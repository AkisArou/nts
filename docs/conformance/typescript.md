...
| ✗ | `Promise.withResolvers()` — the capability object | refuses as ``a global member with no definition here``, and the cost is not the 25 call sites. **Measured 2026-09-13**: 20 refusals in `net`, 30 in `fs`, 29 in `stream`, 29 in `http` — and behind each, a class whose *field initialiser* is one. `web-platform`'s `PipeState<T>`, `TeeState`, `ByteTeeState` each open with `readonly #result = Promise.withResolvers<void>()`, so the constructor refuses, the class gets no type, and every member access on it elsewhere reports ``a member of `X`, a class this compiler has no type for`` — **82 of those in `net`, 82 in `fs`, 82 in `http`, 77 in `stream`**, the largest single root in `net`. Isolated to one variable: a generic class with an ordinary field lowers; the same class with one `withResolvers` field does not. **The representation is settled and the blocker is named.** Record 0337: representing the capability *as its promise* was implemented, agreed with node on 145 cases across two backends, and was reverted because `broadcast.ts` does `state.resolve = pending.resolve` — the members must be **first-class values**, so the right form is the struct of two function fields that record 0336 carries across the library boundary. That form needs **closures with no source node**, since `ClosureInfo` is always tied to one, and that is the whole of what is missing. |
...
| ✅ | a class used as a *value* (`C` itself, passed or returned) | **passing, returning, storing and comparing agree on all three backends as of 2026-09-12** — 116 cases across four functions in `a-class-stored-and-compared`, which was the JVM's last disagreeing example. The token is an address, and a token with no base now takes the signature layout it is *stored into* when there is exactly one; `token_base` gives it the base of its own `typeof` where the program declares that type, and the two coincide on some tokens and not others, which is what made the second look like the general rule. **Constructing through one is a different row and still refuses** — see `calling a class value` above, and the union two rows down. **Constructing through one was a wrong answer that ran**, until it was refused on 2026-09-12: `new C(n)` took its type from the *expression*, which the checker takes from the callee's declared type, so `function make(C: typeof Thing)` built a `Thing` and called `Thing__constructor` with the class argument discarded — `(void)v0;` in the emitted C. One class reaching the site is correct, which is why it was invisible; a second assignable class is built as the first, at the first's size, and `Other`'s constructor was never emitted. Two classes through one site disagreed with node on **18 of 58 cases** and produced a number for each. Refusing it was already the documented intent — the class-token lowering says `new` through such a value "still refuses, as `a computed constructor`" — and a *named* binding slipped past that check because it has identifier text. Building it needs the token to carry the instance descriptor and the constructor, which is the generator's resumption slot with a different member. `tooling/conformance/blockers/a-new-through-a-class-value`, 29 sites in `runtime/node` |
| ✗ | methods and getters on **object literals** | attempted and reverted, and the blocker is not the lowering. A method is `MemberKind::Method` so the literal stores nothing for it; emitting the function and registering it in the hierarchy both work. What fails is the **name**: an anonymous object type has no canonical one, so `layout_of` invents `Type{id}` from whichever `TypeId` the builder saw first — the definition derives it from the literal's type and the call site from the annotation's, and `const s: Stepper = { step() {} }` gives the checker two ids for one shape. So the call emitted `Type14#step` against a function named `Type17#step`. Layouts merge structurally *afterwards*, which is too late for a name already baked into a callee. Needs a program-wide naming authority for structural shapes, which is its own piece of work and renames every anonymous layout. This blocks the idiomatic iterator — `return { next() { … } }` — so it is the next thing worth doing. **Re-probed 2026-09-13 and the row describes the obstacle behind the refusal rather than the refusal.** What is live is unconditional: ``a `method declaration` in an object literal``, and ``a `get accessor` in an object literal``, at the lowering, before any name is formed. The naming argument above is what the *reverted attempt* ran into, which is worth keeping and is not what a reader meets. The probe's third arm is the useful part: an **unannotated** literal — `const s = { step(v) { … } }` with no interface — refuses identically, and it has **one** `TypeId`, so the two-ids collision the row blames cannot arise there. That makes a narrower version reachable without any program-wide authority, and it means the generalisation from the annotated case to all cases was never tested. The authority also already exists in embryo and the row understates it: `layout_of` merges structurally **as it builds**, not afterwards — `Layout::same_shape` finds an existing layout and the type joins it, with a declared name beating a generated one. What does not join the merge is `hierarchy.name`, a per-`TypeId` map, and the qualified method names it produces are themselves compared by `same_shape` — so two ids of one shape emit different method names, the shapes then fail to match, and the merge that would have fixed it never fires. One fact with two derivations, and the second one votes **Third derivation, 2026-09-13, and it outranks both:** dispatch is a property of the **layout**, not of the value. `Layout::methods` is a `Vec<Option<String>>` of *named implementations*, one per slot, and `Layout::types` is a `Vec<TypeId>` precisely because every structurally-equal type shares that one layout. So two literals of one interface type share one table, and one implementation per slot. `{ next() { A } }` and `{ next() { B } }` at the same declared type are two values of one layout, and the table holds one of A and B. A class escapes this because each class is its own layout; an object literal is many values of one. That makes the narrower unannotated version proposed above **silently wrong at the second literal** rather than merely incomplete: it is sound only while exactly one literal has the shape, which nothing checks and no diagnostic would report. The candidate that survives is a different representation -- lower the method to a **field holding a closure**, per value, guarded on the body not mentioning `this` -- which costs a pointer per method per object and changes how every class implementing the interface is laid out. Priced, not built; and note the guard is what makes it *sound*, not merely what makes it narrow. |
