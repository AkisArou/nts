...
import type { SuspendedReason } from "./ReactFiberWorkLoop.ts";
import { isAlreadyRendering } from "./ReactFiberWorkLoop.ts";
...
// `console.createTask`'s task, opaque here: it is only stored and handed to
...
export let yieldReason: SuspendedReason = 0;
export let yieldStartTime: number = -1.1; // The time when we yielded to the event loop

export function startYieldTimer(reason: SuspendedReason): void {
...
export function startHostActionTimer(fiber: Fiber): void {
  ...
  if (blockingUpdateTime < 0) {
    blockingUpdateTime = now();
    blockingUpdateTask =
      isDevelopment && fiber._debugTask != null ? fiber._debugTask : null;
    if (isAlreadyRendering()) {
      blockingUpdateType = SPAWNED_UPDATE;
    }
    const newEventTime = resolveEventTimeStamp();
...
export function trackSuspendedTime(lanes: Lanes, renderEndTime: number): void {
  if (!enableProfilerTimer || !enableComponentPerformanceTrack) {
    return;
  }
  if (isGestureRender(lanes)) {
...
export function recordEffectDuration(_fiber: Fiber): void {
  if (!enableProfilerTimer || !enableProfilerCommitHooks) {
    return;
  }
