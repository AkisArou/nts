'use strict';
// The read direction of the `listen({ fd })` handoff: the worker echoes what it reads.
//
...
function runner() {
  ...
  let ok = false;
  process.on('exit', () => { assert.ok(ok, 'the worker never echoed what it read'); });
...
function worker() {
  const server = net.createServer((conn) => {
    ...
    // **Read before writing.** The write direction was already shown to work by the sibling
    // arm, which never read a byte; node's `http` server on this same handoff receives its
    // connection and then never sees a request, so the read direction is the one in doubt.
    conn.on('data', (chunk) => {
      console.error('worker read ' + JSON.stringify(chunk.toString()));
      conn.end('echo:' + chunk.toString());
    });
    conn.on('end', () => console.error('worker saw end'));
  });
  server.listen({ fd: 3 }, () => {
