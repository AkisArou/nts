...
Re-measuring the published table on 2026-09-15 -- the first full sweep since
`e7342345` on 2026-09-09, **1554 commits back** -- returned `refused` in the
`nts (JVM)` column for every one of `awfy-bounce`, `awfy-list`,
`awfy-mandelbrot`, `awfy-nbody`, `awfy-permute`, `awfy-queens`, `awfy-sieve` and
`awfy-towers`. Exactly those eight and nothing else.

    NTS4004 `Benchmark` could not be written: this class declares the method
    `benchmark()D` twice, which the JVM refuses at load as a duplicate member

`member_forwarders` (`c213fe13`, 2026-09-13, the `s.bump()` surface) declares an
instance method for every function its layout owns. `dispatch_forwarders`
declares one for every entry in the layout's dispatch table. **Every override is
both**, and AWFY's `Benchmark#benchmark` is the plainest possible instance of
one: a member of the layout and slot 0 of its table. The two emitters walked
past each other.

Fixed in `0a9ec047` -- dispatch runs first because it has more to say (it names
the method after the layout that *declared* the slot, and emits the covariant
bridge), and `member_forwarders` then asks the builder what it already holds.
Asking rather than re-deriving which functions occupy a slot, because two
derivations of one fact disagree eventually and this disagreement is a class
that does not load. `awfy-bounce` back to `jvm/Java 1.03x`, jvm floor unmoved at
205 of 205.

### The part worth keeping, which is not the bug

**Four instruments could have caught this and not one of them looks here.**

| instrument | why it missed |
| --- | --- |
| the `jvm` gate step | 205 of 205 *examples*, and no example has the shape -- a method both dispatched virtually and reachable as a member |
| `bench-agree.sh` | does default to `NTS_BACKEND=jvm`, and each of the eight answers "exported nothing with scalar arguments and a scalar result". Checked one at a time: all eight sit outside its floor of 41 |
| `benches.sh` | emitted every case through `emit-c` and `emit-llvm` and not `emit-jvm` -- and its own header is about exactly this hole for the native lanes |
| the published table | regenerated by hand, last on 2026-09-09 |

So **the eight rows this lane's "done" is written about were the eight rows
nothing was watching.** A bar written as "the AWFY rows at or under hand-written
Java" had no instrument standing behind the word AWFY.

The closing move is a rendering floor: `benches.sh` now emits all 61 cases
through the JVM backend too. It cannot be spelled `if ! "$nts" emit-jvm` --
**`emit-jvm` exits zero when it refuses**, rendering what it can and reporting
the rest as diagnostics, so written the obvious way the step would have passed
on every case it failed to emit. Which is the same shape as the bug.

### And a second gap it turned up, still open and not this lane's

`json-stringify-doc` produced no row at all, on any backend. A lowering refusal
added in `c9f3d3a0` on 2026-09-10 -- the one that turned a structural-to-subclass
pointer cast from a SIGSEGV into a diagnostic -- drops that case's `work`.
`emit-c` prints `NTS1001`, **exits zero**, writes C with no entry point, and the
failure surfaces as `undefined reference to 'work'` from the linker with nothing
naming the refusal five lines above it. `benches.sh` does not link, for any
backend, so it does not see this either.

That row's published `5.99 ms` was measured through a cast the compiler has since
established is a crash or a wrong answer. The README's own legend uses this row
as its worked example of "a ratio moving is not this compiler moving".
