...
static void a_reference_tag_is_counted(void) {
    ...
    expect("the erased value keeps its payload alive", (long)nts_live_count(), (long)before + 1);
...
static void an_erased_field_is_released_with_its_owner(void) {
    ...
    expect("the field owns its payload", (long)nts_live_count(), (long)before + 2);
...
static void an_erased_field_holding_a_number_releases_nothing(void) {
    ...
    expect("a number field takes nothing with it", (long)nts_live_count(), (long)before);
    nts_release((NtsHeader *)witness);
}
...
static void erased_array_elements_are_traced(void) {
    ...
    expect("the array owns both payloads", (long)nts_live_count(), (long)before + 3);
