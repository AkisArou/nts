...
pub const NATIVE_WITNESS_NAME: &str = "native_witness.h";
...
fn witness_file(program: &Program, origin: &Origin, diagnostics: &mut Vec<Diagnostic>) -> String {
    ...
    match native_memory::witness(&mut claims, origin, program) {
        Ok(true) => {
            ...
            file.line(origin, "   Include this AFTER the headers that declare these types, in a");
            file.line(origin, "   translation unit that does not include program.h -- the point is");
            file.line(origin, "   to compare what nts believes against what the library says. */");
            // `offsetof` is the assertion mechanism, not one of the bindings, so
            // this include asserts nothing about the consumer's target.
            file.line(origin, "#include <stddef.h>");
            file.blank(origin);
            file.append(claims);
            file.into_text()
        }
...
fn native_prototype(name: &str, target: &nts_core::hir::native::Function) -> String {
    ...
    format!("{} {name}({parameters});", target.result.c_type())
}
...
fn external_prototypes(program: &Program) -> Prototypes {
    ...
    for func in &program.funcs {
            ...
            .map(|value| &func.values[value.0 as usize])
        {
            ...
            let prototype = native_prototype(name, target);
            match seen.get(name) {
