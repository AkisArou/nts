...
import { Forked, NoFlags } from "./ReactFiberFlags.ts";
...
//
// Upstream interleaves the saved values of different types in one array per
// stack; here each saved value has its own array, indexed together.
const forkCountStack: number[] = [];
const forkProviderStack: (Fiber | null)[] = [];
let forkStackIndex: number = 0;
let treeForkProvider: Fiber | null = null;
let treeForkCount: number = 0;

const idIdStack: number[] = [];
const idOverflowStack: string[] = [];
const idProviderStack: (Fiber | null)[] = [];
...
export function pushTreeFork(workInProgress: Fiber, totalChildren: number): void {
  ...
  forkCountStack[forkStackIndex] = treeForkCount;
  forkProviderStack[forkStackIndex] = treeForkProvider;
  forkStackIndex++;
...
function saveTreeId(): void {
  idIdStack[idStackIndex] = treeContextId;
  idOverflowStack[idStackIndex] = treeContextOverflow;
  idProviderStack[idStackIndex] = treeContextProvider;
  idStackIndex++;
}
...
export function popTreeContext(workInProgress: Fiber): void {
  ...
  while (workInProgress === treeForkProvider) {
    forkStackIndex--;
    treeForkProvider = forkProviderStack[forkStackIndex]!;
    forkProviderStack[forkStackIndex] = null;
    treeForkCount = forkCountStack[forkStackIndex]!;
    forkCountStack[forkStackIndex] = 0;
  }

  while (workInProgress === treeContextProvider) {
    idStackIndex--;
    treeContextProvider = idProviderStack[idStackIndex]!;
    idProviderStack[idStackIndex] = null;
    treeContextOverflow = idOverflowStack[idStackIndex]!;
    idOverflowStack[idStackIndex] = "";
    treeContextId = idIdStack[idStackIndex]!;
    idIdStack[idStackIndex] = 0;
  }
}
