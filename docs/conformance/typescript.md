...
| ✗ | a **generator method** — `*named()` or `*[Symbol.iterator]()` on a class | a generator *function* works and `for...of` walks it; a generator **method** is not recognised as one at all, and says `a `yield` outside a generator` — true of the lowering and false of the source. Not about symbol keys: a plainly named `*named()` says it too. The declaration half is about fifteen lines and was written and thrown away, because the *call* site resolves a generator's frame through the callee's **declaration node** and there is no route to one from a receiver type and a member name — `Hierarchy` is keyed by `TypeId` and `String`, and `PropertyRecord` carries no declaration. Landing it alone emits a method nothing can call. `*[Symbol.iterator]()` is how a class is made iterable: 26 sites, against `function*` at 29 and `yield` at 117. `blockers/a-generator-method` |
