...
static void step_run(void *state) {
  ...
  if (!step->report) {
    ...
    for (uint32_t i = 0; i < values->header.length && used < sizeof(line);
         i++) {
      used += (size_t)snprintf(line + used, sizeof(line) - used, "%s%g",
                               i ? "," : "", NTS_ITEMS(values, double)[i]);
    }
...
static void then(NtsPromise *on, const char *label, NtsPromise *next,
                 int report) {
...
/*   Promise.all([Promise.resolve(1), Promise.resolve(2)]).then(v => t("all " +
 * v)); Promise.resolve().then(()=>t("t1")).then(()=>t("t2")).then(()=>t("t3"));
 ...
 * t("race " + v)); Promise.resolve().then(()=>t("t1")).then(()=>t("t2")); node:
 * t1 -> race a -> t2
 *
...
/*   Promise.all([Promise.reject(new Error("boom")), slow]).then(..., e =>
 * t("rejected " + e.message));
 *   Promise.resolve().then(()=>t("t1")).then(()=>{t("t2");
 * r1(9);}).then(()=>t("t3")); node: t1 -> rejected boom -> t2 -> t3
 *
