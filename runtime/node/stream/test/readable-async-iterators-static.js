...
// Every statically representable case from pinned upstream
// `parallel/test-stream-readable-async-iterators.js`. Omitted blocks graft a
// prototype method onto v1 streams, invoke extracted methods through
// Function.call, mutate method slots, or iterate host fs/http stream objects.
...
async function tests() {
...

  {
    ...
    Promise.all(values).then(
      common.mustCall((values) => {
        values.forEach(
          common.mustCall((item, i) => assert.strictEqual(item.value, "hello-" + i), 5),
        );
      }),
    );
...

  {
    ...
    resolved.forEach(
      common.mustCall((item, i) => assert.strictEqual(item.value, "hello-" + i), max),
    );

    assert.rejects(errors[0], { message: "kaboom" }).then(common.mustCall());

    errors.slice(1).forEach(
      common.mustCallAtLeast((promise) => {
        promise.then(
          common.mustCall(({ done, value }) => {
...

  {
    ...
    const next = it.next();
    next.then(common.mustNotCall()).catch(
      common.mustCall((err) => {
        assert.strictEqual(err.code, "ERR_STREAM_PREMATURE_CLOSE");
      }),
    );
...

  {
    ...
    readable.destroy(err);
    await assert.rejects(readable[Symbol.asyncIterator]().next(), (e) => {
      assert.strictEqual(e, err);
      return true;
    });
  }
...

  {
    ...
    const d = b.next();
    r.destroy();
    c.then(common.mustNotCall()).catch(
      common.mustCall((err) => {
        assert.strictEqual(err.code, "ERR_STREAM_PREMATURE_CLOSE");
      }),
    );
...
// AsyncIterator non-destroying iterator
{
  ...
  {
    const readable = createReadable();
    assert.throws(() => readable.iterator(42), {
      code: "ERR_INVALID_ARG_TYPE",
      name: "TypeError",
      message: 'The "options" argument must be of type object. Received ' + "type number (42)",
    });
  }
