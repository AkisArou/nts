...
import { NoLane } from "./ReactFiberLane.ts";
import { ensureScheduleIsScheduled, requestTransitionLane } from "./ReactFiberRootScheduler.ts";
import { clearEntangledAsyncTransitionTypes } from "./ReactFiberTransitionTypes.ts";
import { clearAsyncTransitionTimer, hasScheduledTransitionWork } from "./ReactProfilerTimer.ts";
...
let currentEntangledListeners: (() => unknown)[] | null = null;
...
let pendingIsomorphicIndicator: (() => void) | null = null;
...
export function entangleAsyncAction<S>(transition: Transition, thenable: Thenable<S>): Thenable<S> {
  ...
  if (currentEntangledListeners === null) {
    ...
    const entangledListeners: (() => unknown)[] = (currentEntangledListeners = []);
    ...
    const entangledThenable: Thenable<void> = {
      ...
      then(resolve: (value: void) => unknown) {
        entangledListeners.push(() => resolve(undefined));
      },
    };
...
function pingEngtangledActionScope(): void {
...
export function chainThenableValue<T>(thenable: Thenable<T>, result: T): Thenable<T> {
  ...
  // extra code.
  const listeners: ((value: T) => unknown)[] = [];
