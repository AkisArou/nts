...
| ✅ | `Array.from(xs)` over **anything iterable** | the walk with an append where the body would be, so every shape `for...of` knows arrives for nothing: an array, a typed array, a string by code point, a `Map` or `Set`, a user type with `[Symbol.iterator]`, and a generator. An array source keeps its `slice` — measured, 782 us against 1.05 ms walking — and the three sources that know their length before they start are allocated at it rather than grown |
