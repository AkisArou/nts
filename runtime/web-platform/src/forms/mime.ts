...
export function parseMIMEType(input: string): MIMEType | null {
  ...
  while (at >= 0 && at < source.length) {
    at++;
    while (isHTTPWhitespace(source.charCodeAt(at))) at++;
    ...
    if (source.charAt(at) === '"') {
      ...
      while (at < source.length && source.charAt(at) !== ";") at++;
    } else {
      const valueStart = at;
...
export function parseContentDisposition(input: string): ParameterizedValue {
  ...
  while (at < input.length) {
    ...
    if (input.charAt(at) === '"') {
      ...
      if (!closed) throw new TypeError("Unclosed Content-Disposition parameter");
      while (isHTTPWhitespace(input.charCodeAt(at))) at++;
      ...
      if (!isToken(result)) throw new TypeError("Invalid unquoted Content-Disposition parameter");
    }
