...
// Every behavioral case from pinned upstream
// `parallel/test-stream-iter-broadcast-basic.js`, without the four
// null-prototype observations in its final iterator-result regression.
...
async function testOverlappingNextKeepsEarlierRead() {
  ...
  const secondResult = await Promise.race([
    ...
    setTimeout(common.platformTimeout(50), { settled: false }),
  ]);
  assert.strictEqual(secondResult.settled, false);
  ...
  writer.endSync();
  const finalResult = await second;
