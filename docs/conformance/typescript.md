...
| ✗ | **growing an array by writing past its end** — `const xs = [1]; xs[1] = 2;` | **Aborts.** `nts: refused: index 1 is outside [0, 1)`, exit 134, where JavaScript grows the array to length 2. An abort rather than a refusal, so there is no diagnostic and nothing names the construct. In-bounds writes are right, and `push` grows correctly — `nts_array_grow` exists in the C runtime and is what `push` calls — so the gap is that a *checked index write* has no growing path rather than that growth is unimplemented. **Measured 2026-09-18**: 25 of the slice-1 `test/language` files are blocked upstream of it by `var xs = []; xs[0] = …` at module scope, which refuses as *an empty array literal in a position that does not say what it is* before the write is reached. Not started here because a growing write has to land in C, LLVM **and** JVM together — `bench-agree` has no allowance list — and that is a different piece of work from the lowering gaps beside it. **And the refusal in front of it is load-bearing**: the 26 slice-1 files that stop at *an empty array literal in a position that does not say what it holds* are `var bases = []; bases[0] = …`, and giving the literal a type does not fix them — it turns 26 refusals into 26 aborts. Checked, by annotating one and running it. So the literal and the growing write are one piece of work and have to land together; closing the literal alone would be a change that looks like progress and is not. **The shape it would take, and what it costs.** No backend work is needed: `nts_array_push`/`nts_array_push_ref` already exist in C, are declared for LLVM and handled by JVM, so an append can lower to the call `push` already uses. What it needs is a runtime branch — store when `i < length`, append when `i == length`, abort above that, which is the present behaviour for the sparse case and so no regression. The cost is the reason it is not done here: that branch lands on **every** index write the compiler cannot prove in range, and performance is the reason this project exists. Narrowing it with the interval domain (`facts.rs` already carries `lo`/`hi`, and `proven_array_length` exists) is the obvious mitigation and is itself the design question — so this wants a benchmark and a decision, not an implementation. |
