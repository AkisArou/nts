...
export function getLabelForLane(lane: Lane): string | undefined {
...
export function markRootFinished(
  ...
  while (lanes > 0) {
    ...
    const hiddenUpdatesForLane = hiddenUpdates[index] ?? null;
...
export function markRootEntangled(root: FiberRoot, entangledLanes: Lanes): void {
  ...
  while (lanes) {
    ...
    if (
      ...
      entanglements[index] = entanglements[index]! | entangledLanes;
    }
    lanes &= ~lane;
  }
}
...
export function upgradePendingLanesToSync(
  ...
  while (lanes) {
    const index = pickArbitraryLaneIndex(lanes);
    const lane = 1 << index;
    root.entanglements[SyncLaneIndex] = root.entanglements[SyncLaneIndex]! | lane;
    lanes &= ~lane;
  }
}
...
export function movePendingFibersToMemoized(root: FiberRoot, lanes: Lanes): void {
  if (!enableUpdaterTracking) {
    return;
  }
  if (!isDevToolsPresent) {
...
export function addTransitionToLanesMap(
  ...
  if (enableTransitionTracing) {
    ...
    const index = laneToIndex(lane);
    let transitions = transitionLanesMap[index] ?? null;
    if (transitions === null) {
...
export function getTransitionsForLanes(
  ...
  const transitionsForLanes: Transition[] = [];
  while (lanes > 0) {
    const index = laneToIndex(lanes);
    const lane = 1 << index;
    const transitions = root.transitionLanes[index] ?? null;
    if (transitions !== null) {
...
export function clearTransitionsForLanes(root: FiberRoot, lanes: Lane | Lanes): void {
  if (!enableTransitionTracing) {
    return;
  }
