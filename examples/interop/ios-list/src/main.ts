...
// Swift's `class Fruits: NSObject, UITableViewDataSource`.
class Fruits extends NSObject implements UITableViewDataSource {
...
class AppDelegate extends UIResponder implements UIApplicationDelegate {
  ...
  applicationDidFinishLaunchingWithOptions(application: UIApplication, launchOptions: NSDictionary | null): boolean {
    ...
    // The table holds its data source weakly, as Swift's `weak var
    // dataSource` says: the delegate keeps it.
