...
llvm_rc() { ( NTS_BACKEND=llvm NTS_RC=1; export NTS_BACKEND NTS_RC
  ...
  #
  # 266 -> 267 on 2026-09-19 for `a-library-iterator-result`, and 267 -> 268
  # the same day for `a-narrowed-module-scope-global`.
  backend_examples 268 "through the LLVM backend, counting" "" 14 ); }
...
#
# 267 -> 268 on 2026-09-19 for `a-library-iterator-result`, then 268 -> 269 the
# same day for `a-narrowed-module-scope-global`. The JVM's `exact` floor below
# took both in the same runs and at the same numbers.
llvm() { ( NTS_BACKEND=llvm; export NTS_BACKEND
  backend_examples 269 "through the LLVM backend" "" 14 ); }
...
jvm() { ( NTS_BACKEND=jvm; export NTS_BACKEND
  ...
  backend_examples 269 "through the JVM backend" exact 10 ); }
corpus() {
