...
|---|---|---|---|
| 39 | 42 | 21 | a member on a union whose arms lay their fields out differently — the **discriminant** half is closed: where every arm puts a field at the same index with the same name and representation, the read is licensed by the rule base-first layout already gives a subclass, and `Unerase` is a reinterpretation against a coarse tag so no discriminant has to be tested. What remains is a field whose *representation* differs between arms, and any field sitting behind a disagreement — the agreement is a prefix, not a set. Record 0289 |
