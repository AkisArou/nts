...
// node v24.20.0 `test/parallel/test-blocklist.js`, retaining everything about
// `BlockList` that this profile represents. Two blocks are dropped and only
// two:
//
//   util.inspect(blockList, { depth: -1 })  ->  '[BlockList]'
//   util.inspect(blockList, { depth: null }) includes 'rules: []'
//
// The first wants the depth-exceeded marker to carry the value's constructor
// name; ours is `[Object]`, because a constructor's name is observable
// function metadata and a §13 language non-goal. The second wants node's
// `util.inspect.custom` hook on `BlockList`, and this profile does not
// discover symbol-keyed inspection hooks dynamically. Everything else -- the
// address and subnet rules, every `ERR_INVALID_ARG_TYPE`, `isBlockList`, and
// the whole JSON round trip -- is exercised below unchanged.
