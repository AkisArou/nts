...
pub fn emit(program: &Program) -> Emitted {
    ...
    emit_object_types(&mut object_types, &origin, program, &mut diagnostics);
...
/// nothing.
fn witness_file(program: &Program, origin: &Origin, diagnostics: &mut Vec<Diagnostic>) -> String {
    let mut claims = CodeWriter::new();
    match native_memory::witness(&mut claims, origin, program) {
        Ok(true) => {
