...
| ✗ | **an accessor is absent from every enumeration** — `Object.keys`, `Object.values`, `Object.entries` and `for...in` all omit a getter or setter declared on an object literal, which is an own enumerable property. Measured 2026-09-20: `{ get a() { return 1 }, b: 2 }` gives `"b"` where node gives `"a,b"`, and `Object.values(...).length` is 1 where node says 2. **Wrong answers that compile**, not refusals. The boundary is correct on the other side and must stay: a *class* getter and a method are on the prototype, are not own, and are already excluded. One cause with `{ ...src }`, which was fixed 2026-09-20 — `enumerable_fields` answers `(layout slot, name)` and an accessor has no slot. `examples/a-spread-that-runs-a-getter` carries the seam, the ordering problem and the setter-only case that needs a representation decision |
