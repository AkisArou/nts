...
import { enableSuspenseAvoidThisFallback } from "shared/ReactFeatureFlags.ts";
...
const suspenseHandlerStackCursor: StackCursor<Fiber | null> = createCursor<Fiber | null>(null);
...
export function pushPrimaryTreeSuspenseHandler(handler: Fiber): void {
  ...
  push(suspenseStackCursor, setDefaultShallowSuspenseListContext(suspenseStackCursor.current), handler);
...
export const suspenseStackCursor: StackCursor<SuspenseContext> = createCursor<SuspenseContext>(DefaultSuspenseContext);
