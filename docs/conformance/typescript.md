...
| **a counted loop over a *global* array keeps its bounds checks** — found by the control above rather than looked for. `for (i = 0; i < A.length; i++) A[i]` is `array.get unchecked` when `A` is a parameter (`elementwise`) and `array.get` when `A` is a module global | why the length relationship survives a parameter and not a global. This is a bounds-analysis gap rather than a type one, and it is the first thing `as const` would need before it could be worth anything |
