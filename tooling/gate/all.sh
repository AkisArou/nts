...
llvm_rc() { ( NTS_BACKEND=llvm NTS_RC=1; export NTS_BACKEND NTS_RC
  # 197 -> 198 -> 199 on 2026-09-13. One below its sibling, and the one is named:
  # `this-in-a-field-initializer`, which the run prints rather than absorbing.
  backend_examples 199 "through the LLVM backend, counting" ); }
...
#
# **198 -> 199 -> 200 on 2026-09-13**, for `a-closure-over-a-loop-variable` and
# then `a-symbols-description`. This floor
...
llvm() { ( NTS_BACKEND=llvm; export NTS_BACKEND
  backend_examples 200 "through the LLVM backend" ); }
