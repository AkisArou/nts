...
|---|---|
| `✅` table rows | 166 |
| naming an `examples/` directory | 146 |
| quoting a case count | 14 |
| doing **either** | 148 |
| doing **neither** | **18** |

Forty-five rows were cited on 2026-09-15 and the count moved 62 → 18, of which three are the legend's own rows and can never be cited. Five more say in the row that no example exercises part of what they claim — `var`, `%=`/`^=`/`>>>=`, `ReadonlyMap`/`ReadonlySet`, five of the six type-operator forms, and `xs[next()] += 1` — which the count cannot express and a reader of the row can see. The table
