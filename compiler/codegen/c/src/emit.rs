...
/// A `.c` and not a `.h`: the witness is a complete translation unit that
/// includes the headers its bindings name, so it is compiled, not included.
/// While it was a fragment, what it compared against was chosen by whoever
/// wrote the enclosing file -- and that choice decides the answer.
pub const NATIVE_WITNESS_NAME: &str = "native_witness.c";
...
const WINRT_HELPERS: [&str; 13] = [
    ...
    "nts_string_copy_hstring(",
    // A COM object erased into a value, which the family this file
    // registers counts.
    "NTS_TAG_HANDLE_COM",
];
...
#[must_use]
pub fn support_files(needs_unicode: bool) -> Vec<Support<'static>> {
...
pub struct Emitted {
    ...
    /// C declarations and stable object aliases for the entry modules' exports.
    pub header: String,
    ...
    /// an empty witness reads as "checked" and has checked nothing.
    pub witness: String,
}
...
impl Emitted {
    ...
    pub fn support_files(&self) -> Vec<Support<'_>> {
        ...
        if !self.witness.is_empty() {
            files.push(Support {
                name: NATIVE_WITNESS_NAME,
                contents: &self.witness,
                compiled: false,
            });
        }
        files
...
/// C for `program`, laid out for a target whose C ABI is `abi`.
///
/// HIR is the same for every target. Everything that depends on the target
/// (a `long`'s width, the offsets after it, a `sizeof`) is resolved here from
...
pub fn emit(program: &Program, abi: NativeAbi) -> Emitted {
    ...
    // Whatever a native binding names, when a struct here is that header's and
    // not ours -- see `header::bindings`, which emits the same block for
    // `program.h`. Unlike there the macros are *defined* rather than required:
    // this is a file whose first line we own, which is the only place a
    // feature-test macro can still decide anything.
    native_preamble(&mut writer, &origin, program);
    // The runtime, and otherwise nothing a binding did not ask for -- notably
    // not <stdlib.h>, which declares `div`, a name a TypeScript program is
    // entitled to use.
    ...
    match external_prototypes(program, abi) {
        Ok(prototypes) => {
    ...
    let witness = witness_file(program, abi, &origin, &mut diagnostics);
...
///
/// Separate from the program's own `CodeWriter`: this is not part of the
/// program. It is a set of claims *about* it, for a translation unit that sees
/// the real headers -- which `program.c`, defining those structs itself, never
/// can.
///
...
fn witness_file(program: &Program, abi: NativeAbi, origin: &Origin, diagnostics: &mut Vec<Diagnostic>) -> String {
    ...
    match native_memory::witness(&mut claims, origin, program, abi) {
        Ok(true) => {
            ...
            // definition after that point is simply late.
            for define in &program.native_defines {
                match define.split_once('=') {
                    Some((name, value)) => file.line(origin, format!("#define {name} {value}")),
                    None => file.line(origin, format!("#define {define} 1")),
                }
            }
            let mut included: Vec<&str> = Vec::new();
            for header in &program.native_headers {
                // Kept as authored. `<x.h>` and `"x.h"` differ in where the
                // preprocessor looks, and a project header on the quoted path
                // is not the system one that happens to share its name.
                let spelled = if header.starts_with('<') || header.starts_with('"') {
                    header.clone()
                } else {
                    format!("<{header}>")
                };
                included.push(header.trim_matches(['<', '>', '"']));
                file.line(origin, format!("#include {spelled}"));
            }
            // `offsetof` is the assertion mechanism, and `uint8_t` and friends
            // are how a scalar is spelled here -- neither asserts anything
...
/// The C spelling of one authored foreign declaration.
///
/// One function because two callers need the *same* text: `program.c` declares
/// the symbol with it, and the witness re-declares the symbol with it beside
/// the real header. If those two were built separately they could drift, and a
/// witness that agrees with a prototype nobody emitted checks nothing.
/// Which spelling of a type the caller can use.
///
/// The one thing the two consumers legitimately differ on. `program.c` defines
/// the function-pointer typedefs and should name them; the witness includes the
/// real headers instead of `program.h` and so has none of them in scope. Every
/// other part of the prototype -- order, arity, the `void` for an empty list --
/// is produced once for both, which is the point of sharing this function.
#[derive(Clone, Copy)]
pub(crate) enum Spelling {
    /// `NtsFn_int_int`, defined in `program.h`.
    Typedef,
    /// `int (*)(int)`, which needs nothing defined anywhere.
    Expanded,
}

impl Spelling {
    fn of(self, ty: &nts_core::hir::native::Type) -> String {
        match self {
            Self::Typedef => ty.c_type().into_owned(),
            Self::Expanded => ty.c_type_expanded(),
        }
    }
}

/// Whatever a native binding named, for `program.c`.
///
/// `header::bindings` emits the same headers for `program.h` and *requires* the
/// macros there rather than defining them; here they are defined, because this
/// is a file whose first line we own -- the only place a feature-test macro can
/// still decide anything.
fn native_preamble(writer: &mut CodeWriter, origin: &Origin, program: &Program) {
    if !native_memory::needs_headers(program) {
        return;
    }
    for define in &program.native_defines {
        match define.split_once('=') {
            Some((name, value)) => writer.line(origin, format!("#define {name} {value}")),
            None => writer.line(origin, format!("#define {define} 1")),
        }
    }
...
/// The C data model this program was emitted for, asserted where the C
...
fn external_prototypes(program: &Program, abi: NativeAbi) -> Prototypes {
    ...
    for func in &program.funcs {
            ...
            .map(|value| &func.values[value.0 as usize])
        {
            ...
            let prototype = native_prototype(name, target, Spelling::Typedef);
            match seen.get(name) {
...
fn format_signature(name: &str, returns: &str, params: &[String], public: bool) -> String {
    let visibility = if public { "" } else { "static " };
    let params = if params.is_empty() {
        "void".to_owned()
    } else {
        params.join(", ")
    };
    format!("{visibility}{returns} {}({params})", c_identifier(name))
}
...
mod tests {
    ...
    fn a_function_this_backend_refuses_takes_its_callers_with_it() {
        ...
        let program = Program {
            ...
            public_api: ["refused", "caller", "outer", "unrelated"]
                .into_iter()
        ...
        for gone in ["refused", "caller", "outer"] {
            assert!(!emitted.header.contains(&format!("{gone}(")));
            assert!(
        ...
        assert!(emitted.header.contains("unrelated(void);"));
