...
| module-level statements | done | they ran nowhere. The lowering walked declarations, and `total = bump(41)` is not one, so it was dropped and the program answered as though the line were not there — with no diagnostic, and no example covering it. They are one function now, `module#init`, which the embedder calls before anything else; module evaluation is itself a job, so what it queues is drained at the checkpoint after it. On node's side `await import()` does the same, which is what makes the two comparable |
| top-level statements in more than one module | **not done** | the order is the import graph, and the snapshot does not carry one. Refused rather than run in a guessed order |
| `setTimeout`, `setInterval`, `clearTimeout`, `clearInterval` | done | a capability over the host's `post_delayed`, in the runtime rather than in a host, so both hosts have it and neither implements it. The callback is a closure, so the runtime gets the object and the slot its call occupies. Not observable from TypeScript yet — a timer's effect reaches a program only through a promise its callback settles — so the behaviour is checked in `runtime/c/tests/timers.c` and the lowering in `examples/timers` |
