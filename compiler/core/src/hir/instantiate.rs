...
impl Site for Writer<'_> {
    ...
    fn instance(
        ...
        // A property whose type cannot be substituted keeps the declaration's:
        // a refusal at its use rather than a member gone missing.
        let properties: Vec<PropertyRecord> = properties
            .iter()
            .map(|property| PropertyRecord {
                ty: substitute(self, property.ty, &own, depth).unwrap_or(property.ty),
                ..property.clone()
            })
            .collect();
        ...
        if let Some(bases) = self.snapshot.base_types.get(&declaration).cloned() {
            let bases: Vec<TypeId> = bases
                .into_iter()
                .map(|base| substitute(self, base, &own, depth).unwrap_or(base))
                .collect();
        ...
        if let Some(signatures) = self.snapshot.index_signatures.get(&declaration).cloned() {
            let signatures: Vec<IndexSignature> = signatures
                ...
                .map(|signature| IndexSignature {
                    key: substitute(self, signature.key, &own, depth).unwrap_or(signature.key),
                    value: substitute(self, signature.value, &own, depth).unwrap_or(signature.value),
                    ..signature
                })
                .collect();
