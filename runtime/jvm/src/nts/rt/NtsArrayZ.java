...
public final class NtsArrayZ {
     ...
     */
    public static NtsArrayZ of(long n) {
    ...
    public static NtsArrayZ of(int n) {
     ...
     */
    public static boolean get(NtsArrayZ a, long at) {
        if (at >= 0 && at < a.length) { return a.items[(int) at]; }
        NtsRuntime.outOfRange((double) at, a.length);
        return false;
    }

    public static boolean get(NtsArrayZ a, int at) {
    ...
    public static double push(NtsArrayZ a, boolean value) {
        ...
        // `a.length` is a non-negative `int` over a `boolean[]`, so the only value that
