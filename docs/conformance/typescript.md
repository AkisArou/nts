...
| ◐ | block scope, `const`/`let`/`var` — `const` and `let` hold. **The `var` third was marked ✅ while saying it was untested, and it is wrong.** Tested 2026-09-20: after `for (var i = 0; i < 3; i++) {}` at module scope, a *module-scope* read of `i` gives 3 and agrees with node, but **any function reading it gives 0** — `function f() { return i }` and `const g = () => i` both, where node says 3. A silent wrong answer, not a refusal; the in-function declaration has a blocker and refuses by name, and this one compiles. Pre-existing: a binary built 2026-09-18 answers identically. `declared_in_a_loop_head` in `hir/lower.rs` carries the measurement and what is inferred rather than measured about the cause |
