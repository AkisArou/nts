...
| ✗ | **calling** a class value — `TypeError(m)`, which JavaScript makes `new TypeError(m)`. The token holds nothing and would need a `call` that constructs; nothing in the profile asks for one |
| ✗ | a **union of two class values** — `cond ? TypeError : RangeError` | the diagnosis below is right and was nearly discarded as stale. Comparison **agrees on C and LLVM** — 87 cases across three exports, bound, returned and tested against both arms — so checking those two reads as the feature working. The JVM declines it: `NTS4001 storing a `Ctor_TypeError` where a `Fn16__20` is declared`. The checker collapses the conditional to a *single* constructor type, so whichever token is not that type meets a slot declared for the other — **two genuinely different signatures rather than two ids for one**, which `signature_key` cannot relate and no `Layout.base` can either. A token that is a representation rather than an address would; that is the same change `a class used as a value` wants, and `a-class-stored-and-compared` is the same refusal. Per-backend, so it stays ✗ |
...
| ✗ | the **reverse mapping** (`Colour[1]` → `"Red"`) | needs the table a plain enum emits alongside its members, which is the one part of an enum that has run-time existence. Refused by name **Attempted and reverted, 2026-09-12.** The table itself is easy — the enum's object type carries every member with its literal value, so the names go in an array indexed by the value and a hole is the zeroed slot, which is `undefined` and the answer. What it needs and has not got is a way to say *out of range*: `Colour[-1]` and `Colour[9]` are `undefined` in JavaScript, a bounds-checked read aborts instead, and `lower_branching_value` takes its result type from the node it is given, so there is no way to build a `boolean` select for the range test or a `number` one for the clamped index — the emitted C used an array index typed `bool`. The clean shape is a runtime `nts_enum_name(table, i)` answering the string or null, which is one helper in three backends and therefore cross-lane. Zero enums in `runtime/node`, so it waits |
| ✗ | decorators |
...
| ✗ | `ToPrimitive`, `OrdinaryToPrimitive` — `valueOf`/`toString` dispatch | **not what the 178 refused conversions wanted, which is the finding rather than the row.** `a conversion to string from this type` never named the type, so the sites could not be counted by kind; naming it says **176 of 179 are `unknown`**, 161 in `internal/errors.ts` interpolating one into a message. So the work behind them is `ToString` of an *erased value* — a dispatch on the runtime tag — and `valueOf` is not in it at all. `nts_value_to_string` already exists in all three backends and the lowering already emits it; what gates it is `spells_itself`, which refuses `Unknown` because an `unknown` can hold an object. **Built and reverted on 2026-09-12**: the object tag is answerable (a `toString` slot on the descriptor, `"[object Object]"` for a null entry, a join for an array), and the **function** tag is not — node gives a function's source text and this compiler does not keep it. `tooling/sweep` covers `String(fn)`, so admitting `unknown` makes a case that cannot be answered correctly compile. The tag work is right and blocked on function source retention |
...
| ✗ | `Array.from` with a **mapping callback**, or over an **array-like** | the two-argument form is two features under one name. With an iterable it is `map` fused into the walk; with `{ length: n }` it is not an iteration at all — an array-like is read by index, and `Array.from({ length: 4 })` builds four `undefined`s out of an object with no elements |
