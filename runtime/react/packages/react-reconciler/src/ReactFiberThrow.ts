// What happens when something below a fiber throws during render: a thrown
...
import { markFailedErrorBoundaryForHotReloading } from "./ReactFiberHotReloading.ts";
import {
  getIsHydrating,
...
import { includesSomeLane, mergeLanes, pickArbitraryLane, SyncLane } from "./ReactFiberLane.ts";
import { propagateParentContextChangesToDeferredTree } from "./ReactFiberNewContext.ts";
...
import { noopSuspenseyCommitThenable } from "./ReactFiberThenable.ts";
import {
  attachPingListener,
...
function createRootErrorUpdate(root: FiberRoot, errorInfo: CapturedValue<unknown>, lane: Lane): Update<unknown> {
...
function createClassErrorUpdate(lane: Lane): Update<unknown> {
  const update = createUpdate(lane);
  update.tag = CaptureUpdate;
  return update;
}
...
function resetSuspendedComponent(sourceFiber: Fiber, rootRenderLanes: Lanes): void {
...
function isConcurrentFiber(fiber: Fiber): boolean {
  return disableLegacyMode || (fiber.mode & ConcurrentMode) !== NoMode;
}
...
function throwException(
  ...
  if (value !== null && typeof value === "object") {
    if (typeof (value as { then?: unknown }).then === "function") {
      ...
      const wakeable = value as Wakeable;
      resetSuspendedComponent(sourceFiber, rootRenderLanes);

      if (isDevelopment) {
        if (getIsHydrating() && isConcurrentFiber(sourceFiber)) {
          markDidThrowWhileHydratingDEV();
        }
      }
      ...
      if (suspenseBoundary !== null) {
        switch (suspenseBoundary.tag) {
          ...
          case SuspenseListComponent: {
            ...
            if (isConcurrentFiber(sourceFiber)) {
            ...
            if (isSuspenseyResource) {
              ...
              // no pings.
              if (isConcurrentFiber(suspenseBoundary)) {
          ...
          case OffscreenComponent: {
            if (isConcurrentFiber(suspenseBoundary)) {
              ...
              if (isSuspenseyResource) {
                ...
                const offscreenQueue = suspenseBoundary.updateQueue as OffscreenQueue | null;
  ...
  if (getIsHydrating() && isConcurrentFiber(sourceFiber)) {
    ...
    if (hydrationBoundary !== null) {
      if (isDevelopment) {
        if (hydrationBoundary.tag === SuspenseListComponent) {
          console.error("SuspenseList should never catch while hydrating. This is a bug in React.");
        }
      }
      ...
      markSuspenseBoundaryShouldCapture(hydrationBoundary, returnFiber, sourceFiber, root, rootRenderLanes);
    ...
    const update = createRootErrorUpdate(
      workInProgress.stateNode as FiberRoot,
  ...
  do {
    switch (workInProgress.tag) {
      case HostRoot: {
        ...
        const lane = pickArbitraryLane(rootRenderLanes);
        workInProgress.lanes = mergeLanes(workInProgress.lanes, lane);
        const update = createRootErrorUpdate(workInProgress.stateNode as FiberRoot, errorInfo, lane);
        enqueueCapturedUpdate(workInProgress, update);
        return false;
      }
      case ClassComponent: {
      ...
      case OffscreenComponent: {
        const offscreenState = workInProgress.memoizedState as OffscreenState | null;
...
export { createClassErrorUpdate, createRootErrorUpdate, initializeClassErrorUpdate, throwException };
