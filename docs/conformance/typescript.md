...
| ✅ | **an accessor enumerates** — `Object.keys`, `Object.values`, `Object.entries`, `Object.hasOwn`, `for...in`, `{ ...src }` and **`const { a } = o`** all name and run a getter, as of 2026-09-21. Destructuring differs from the rest on purpose: it is a *read*, so it takes a **prototype** accessor too, where enumeration must not — a prototype member is readable and is not an own property. Was: all six omitted a getter or setter declared on an object literal, which is an own enumerable property. Measured 2026-09-20: `{ get a() { return 1 }, b: 2 }` gives `"b"` where node gives `"a,b"`, and `Object.values(...).length` is 1 where node says 2. **Wrong answers that compile**, not refusals. The boundary is correct on the other side and must stay: a *class* getter and a method are on the prototype, are not own, and are already excluded. One cause with `{ ...src }`, which was fixed 2026-09-20 — `enumerable_fields` answers `(layout slot, name)` and an accessor has no slot. **`"a" in src` answers `true`, correctly** — same question, two derivations, one compiler: `in` asks `declares(type, key)` and enumeration asks the layout, which holds storage and has no entry for a call. `examples/an-accessor-that-enumerates` and `examples/a-spread-that-runs-a-getter` carry it. The discriminator is **where the member is written**, not `PropertyRecord::own`, which is `false` for a class's accessor and a literal's alike. Two things remain: a **set-only** property is refused by name (it enumerates and its value is `undefined`, which the element has no width for), a **method shorthand** on a literal now enumerates by name in `Object.keys`, `for...in` and `Object.hasOwn` (2026-09-21), and is refused by name in `Object.values`/`Object.entries` — its value is a function object and `bound_method` needs a property-access node the enumeration does not have, so a refusal replaced an array that came back one element short. Separately, `Object.values` compiles for a plain literal and is refused for a **class instance or an interface-typed literal** even with uniform `number` fields: read out of `nts types`, the checker's overload gives `any[]` there and `number[]` for the literal, so this is the `any` policy applied correctly rather than an enumeration gap — `Object.keys` on the same class compiles |
