...
| ✗ | a **union of two class values** — `cond ? TypeError : RangeError`. The checker gives that conditional a single object type rather than a union of the two constructors, so nothing sees two representations to erase and the backend declines the function with `NTS2006`. Loud rather than wrong. Zero sites: the idiom is `return TypeError` and `value === TypeError` |
...
|---|---|---|---|
| 39 | 42 | 21 | a member on a union whose arms lay their fields out differently — **designed, not landed.** Where every arm puts a field at the same index with the same name and representation the read is sound, and on C and LLVM it is free: a pointer cast between structs with a common initial sequence is what base-first layout already relies on. The JVM cannot do it — `Unerase` is a `CHECKCAST` there and `Right cannot be cast to Left` — so the HIR has to state the *fact* (these arms agree about this field) rather than the instruction, and carry the arm types and the field index so each backend picks its own. The JVM lane measured the alternatives: an `instanceof` chain at 1759 ns/pass against a synthesised interface at 6213. Record 0289 |
...
| ~~**an exhaustive discriminated union**~~ — **still blocked at the lowering**, and now with a design rather than a question. `switch (s.kind)` over `{kind:"circle";r} \| {kind:"square";side}` is refused: "`kind` on a union, whose members lay their fields out differently". It lowered for one evening on C and LLVM and threw `ClassCastException` on the JVM, so it was reverted rather than left absorbed by a floor. Record 0289 | Lane 1 first, behind the shared-field op. Price the fact after the construct compiles **on every backend** |
