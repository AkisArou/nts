// The begin phase: render a fiber (call the component, process the update
...
} from "./ReactChildFiber.ts";
import { setCurrentFiber } from "./ReactCurrentFiber.ts";
import {
...
import type { ActivityProps, ActivityState } from "./ReactFiberActivityComponent.ts";
import type { Cache, CacheComponentState, SpawnedCachePool } from "./ReactFiberCacheComponent.ts";
import { CacheContext, pushCacheProvider } from "./ReactFiberCacheComponent.ts";
...
import { isCurrentTreeHidden, pushHiddenContext, reuseHiddenContextOnStack } from "./ReactFiberHiddenContext.ts";
import {
  bailoutHooks,
...
import { getRootHostContainer, pushHostContainer, pushHostContext } from "./ReactFiberHostContext.ts";
...
import {
  ...
  warnIfHydrating,
} from "./ReactFiberHydrationContext.ts";
...
import type {
  ...
  OffscreenQueue,
  OffscreenState,
} from "./ReactFiberOffscreenComponent.ts";
import { OffscreenVisible } from "./ReactFiberOffscreenComponent.ts";
import { shouldError, shouldSuspend } from "./ReactFiberReconciler.ts";
...
import type {
  ...
  SuspenseState,
} from "./ReactFiberSuspenseComponent.ts";
import { findFirstSuspended } from "./ReactFiberSuspenseComponent.ts";
import type { SuspenseContext } from "./ReactFiberSuspenseContext.ts";
import {
...
} from "./ReactFiberTracingMarkerComponent.ts";
import {
...
import { stopProfilerTimerIfRunning } from "./ReactProfilerTimer.ts";
import { ReactStrictModeWarnings } from "./ReactStrictModeWarnings.ts";
import type { TypeOfMode } from "./ReactTypeOfMode.ts";
import { ConcurrentMode, NoMode, ProfileMode, StrictLegacyMode } from "./ReactTypeOfMode.ts";
import {
  ActivityComponent,
  CacheComponent,
  ClassComponent,
...
// A legacy context object, as the legacy-context module stores it.
type LegacyContextObject = { [key: string]: unknown };

// A class component's constructor, as the class-component module drives it.
type ClassComponentType = ClassComponentConstructor;

interface SuspenseProps {
  children?: unknown;
  fallback?: unknown;
  suspenseCallback?: unknown;
...
export const SelectiveHydrationException: unknown = new Error(
...
const didWarnAboutBadClass: { [componentName: string]: boolean } = {};
const didWarnAboutContextTypeOnFunctionComponent: { [componentName: string]: boolean } = {};
const didWarnAboutContextTypes: { [componentName: string]: boolean } = {};
const didWarnAboutGetDerivedStateOnFunctionComponent: { [componentName: string]: boolean } = {};
export let didWarnAboutReassigningProps = false;
const didWarnAboutRevealOrder: { [key: string]: boolean } = {};
const didWarnAboutTailOptions: { [key: string]: boolean } = {};
const didWarnAboutClassNameOnViewTransition: { [componentName: string]: boolean } = {};

// Tag-specific projections of a fiber's heterogeneous fields.
type AnyProps = Props;
...
interface MemoType {
  ...
  readonly compare: ((oldProps: unknown, newProps: unknown) => boolean) | null;
}

// A function component, a forwardRef render function or a class's render,
// as the reconciler calls it. The second argument is the ref (forwardRef) or
// the legacy context.
type RenderFunction = (props: unknown, secondArg: unknown) => unknown;

// The static parts of a function or class component the reconciler reads.
interface ComponentStatics {
  ...
  readonly prototype?: { readonly render?: unknown } | null;
}
...
function updateForwardRef(
  ...
  if (isDevelopment) {
    const resolvedRender = resolveTypeForHotReloading(render) as RenderFunction;
  ...
  const nextChildren = renderWithHooks(current, workInProgress, render, propsWithoutRef, ref, renderLanes);
...
function updateMemoComponent(
  ...
  if (current === null) {
    ...
    if (isSimpleFunctionComponent(type) && Component.compare === null) {
      ...
      return updateSimpleMemoComponent(current, workInProgress, resolvedType as RenderFunction, nextProps, renderLanes);
    }
...
function updateSimpleMemoComponent(
  ...
  nextPropsIn: AnyProps,
  renderLanes: Lanes,
): Fiber | null {
  let nextProps = nextPropsIn;
  // TODO: current can be non-null here even if the component
  // hasn't yet mounted. This happens when the inner render suspends.
  // We'll need to figure out if this is fine or can cause issues.
  if (current !== null) {
    const prevProps = current.memoizedProps as AnyProps;
    if (
      shallowEqual(prevProps, nextProps) &&
      current.ref === workInProgress.ref &&
      // Prevent bailout if the implementation changed due to hot reload.
      (isDevelopment ? workInProgress.type === current.type : true)
    ) {
...
function createOffscreenInstance(): OffscreenInstance {
  return {
...
function updateOffscreenComponent(
  ...
  const prevState = current !== null ? (current.memoizedState as OffscreenState | null) : null;

  if (current === null && workInProgress.stateNode === null) {
    // We previously reset the work-in-progress.
    // We need to create a new Offscreen instance.
    workInProgress.stateNode = createOffscreenInstance();
  }
  ...
  if (nextProps.mode === "hidden" || (enableLegacyHidden && nextProps.mode === "unstable-defer-without-hiding")) {
    ...
    if (didSuspend) {
      ...
      if (current !== null) {
        ...
        remainingChildLanes = removeLanes(currentChildLanes, lanesWeJustAttempted);
      } else {
        remainingChildLanes = NoLanes;
        workInProgress.child = null;
      }

      return deferHiddenOffscreenComponent(current, workInProgress, nextBaseLanes, renderLanes, remainingChildLanes);
    }
    ...
    if (!disableLegacyMode && (workInProgress.mode & ConcurrentMode) === NoMode) {
      ...
      const remainingChildLanes = (workInProgress.lanes = laneToLanes(OffscreenLane));
    ...
    if (prevState !== null) {
      ...
      if (enableTransitionTracing) {
        ...
        const instance = workInProgress.stateNode as OffscreenInstance | null;
...
function bailoutOffscreenComponent(current: Fiber | null, workInProgress: Fiber): Fiber | null {
  if ((current === null || current.tag !== OffscreenComponent) && workInProgress.stateNode === null) {
    workInProgress.stateNode = createOffscreenInstance();
  }
...
function updateLegacyHiddenComponent(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  ...
  return updateOffscreenComponent(current, workInProgress, renderLanes, nextProps);
}
...
function mountActivityChildren(workInProgress: Fiber, nextProps: ActivityProps, renderLanes: Lanes): Fiber {
  ...
  const primaryChildFragment = mountWorkInProgressOffscreenFiber(offscreenChildProps, mode, renderLanes);
...
function retryActivityComponentWithoutHydrating(current: Fiber, workInProgress: Fiber, renderLanes: Lanes): Fiber {
...
function updateDehydratedActivityComponent(
  ...
  if (!didSuspend) {
    ...
    if (didReceiveUpdate || hasContextChanged) {
      ...
      if (isCurrentTreeHidden()) {
        ...
        return retryActivityComponentWithoutHydrating(current, workInProgress, renderLanes);
      }
    ...
    reenterHydrationStateFromDehydratedActivityInstance(workInProgress, activityInstance, activityState.treeContext);
  ...
  if (workInProgress.flags & ForceClientRender) {
    ...
    return retryActivityComponentWithoutHydrating(current, workInProgress, renderLanes);
  } else if ((workInProgress.memoizedState as ActivityState | null) !== null) {
...
function updateActivityComponent(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  ...
  if (current === null) {
    ...
    if (getIsHydrating()) {
      ...
      pushDehydratedActivitySuspenseHandler(workInProgress);
      const dehydrated = claimNextHydratableActivityInstance(workInProgress);
  ...
  if (includesSomeLane(renderLanes, OffscreenLane) && includesSomeLane(renderLanes, current.lanes)) {
    // If we're rendering Offscreen and we're entering the activity then it's possible
    // that the only reason we rendered was because this boundary left work. Provide
    // it as a cause if another one doesn't already exist.
    markRenderDerivedCause(workInProgress);
  ...
  const primaryChildFragment = updateWorkInProgressOffscreenFiber(currentChild, offscreenChildProps);
...
function updateCacheComponent(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  prepareToReadContext(workInProgress, renderLanes);
  ...
  if (current === null) {
    ...
    const prevState = current.memoizedState as CacheComponentState;
    const nextState = workInProgress.memoizedState as CacheComponentState;
...
function updateTracingMarkerComponent(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  if (!enableTransitionTracing) {
    return null;
  }
  ...
  const instance = workInProgress.stateNode as TracingMarkerInstance | null;
  if (instance !== null) {
...
function updateFragment(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
...
// The state node of a Profiler fiber.
...
function updateProfiler(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  if (enableProfilerTimer) {
    ...
    if (enableProfilerCommitHooks) {
      ...
      const stateNode = workInProgress.stateNode as ProfilerStateNode;
...
function markRef(current: Fiber | null, workInProgress: Fiber): void {
  ...
  const ref: unknown = workInProgress.ref;
  if (ref === null) {
...
function updateFunctionComponent(
  ...
  if (isDevelopment) {
    const statics = Component as ComponentStatics;
    if (statics.prototype && typeof statics.prototype.render === "function") {
...
export function replayFunctionComponent(
  ...
  const nextChildren = replaySuspendedComponentWithHooks(current, workInProgress, Component as RenderFunction, nextProps, secondArg);
  const hasId = checkDidRenderIdHook();
  if (enableSchedulingProfiler) {
    markComponentRenderStopped();
  }
...
// The parts of a class instance DevTools' forced-error reset touches.
interface ResettableInstance {
  context: unknown;
  updater: { enqueueSetState(instance: unknown, state: unknown, callback: null): void };
}
...
function updateClassComponent(
  ...
  if (isDevelopment) {
    const inst = workInProgress.stateNode as ClassInstance;
...
function finishClassComponent(
  ...
  if (didCaptureError && typeof (Component as ComponentStatics).getDerivedStateFromError !== "function") {
...
function pushHostRootContext(workInProgress: Fiber): void {
  ...
  if (root.pendingContext) {
    pushTopLevelContextObject(workInProgress, root.pendingContext as LegacyContextObject, root.pendingContext !== root.context);
  } else if (root.context) {
    // Should always be set
    pushTopLevelContextObject(workInProgress, root.context as LegacyContextObject, false);
  }
...
function updateHostRoot(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  ...
  const prevState = workInProgress.memoizedState as RootState;
  const prevChildren = prevState.element;
  cloneUpdateQueue(current, workInProgress);
  processUpdateQueue(workInProgress, nextProps, null, renderLanes);

  const nextState = workInProgress.memoizedState as RootState;
  const root = workInProgress.stateNode as FiberRoot;
...
function updateHostComponent(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  ...
  const nextProps = workInProgress.pendingProps as AnyProps;
  const prevProps = current !== null ? (current.memoizedProps as AnyProps) : null;
...
function updateHostHoistable(current: Fiber | null, workInProgress: Fiber, _renderLanes: Lanes): Fiber | null {
  ...
  if (current === null) {
    const resource = getResource(workInProgress.type as Type, null, workInProgress.pendingProps as Props, null);
    if (resource) {
      ...
      if (!getIsHydrating()) {
        ...
        workInProgress.stateNode = createHoistableInstance(
          workInProgress.type as Type,
          workInProgress.pendingProps as Props,
    ...
    workInProgress.memoizedState = getResource(
      workInProgress.type as Type,
      current.memoizedProps as Props,
      workInProgress.pendingProps as Props,
...
function updateHostSingleton(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  pushHostContext(workInProgress);
...
function mountLazyComponent(
  _current: Fiber | null,
  workInProgress: Fiber,
  elementType: LazyComponentType<unknown, unknown>,
  renderLanes: Lanes,
): Fiber | null {
  resetSuspendedCurrentOnMountInLegacyMode(_current, workInProgress);

  const props = workInProgress.pendingProps as AnyProps;
  ...
  if (typeof Component === "function") {
    ...
    return updateFunctionComponent(null, workInProgress, Component as RenderFunction, props, renderLanes);
  } else if (Component !== undefined && Component !== null) {
    const $$typeof = (Component as { $$typeof?: unknown }).$$typeof;
    if ($$typeof === REACT_FORWARD_REF_TYPE) {
      workInProgress.tag = ForwardRef;
      return updateForwardRef(null, workInProgress, Component as ForwardRefType, props, renderLanes);
    } else if ($$typeof === REACT_MEMO_TYPE) {
      workInProgress.tag = MemoComponent;
      return updateMemoComponent(null, workInProgress, Component as MemoType, props, renderLanes);
  ...
  if (isDevelopment) {
    if (
      ...
      (Component as { $$typeof?: unknown }).$$typeof === REACT_LAZY_TYPE
    ) {
  ...
  throw new Error(
    `Element type is invalid. Received a promise that resolves to: ${String(loggedComponent)}. ` +
...
function validateFunctionComponentInDev(_workInProgress: Fiber, ComponentIn: unknown): void {
  if (isDevelopment) {
    const Component = ComponentIn as ComponentStatics | null | undefined;
    ...
    if (Component && typeof Component.getDerivedStateFromProps === "function") {
      const componentName = getComponentNameFromType(Component) || "Unknown";
    ...
    if (Component && typeof Component.contextType === "object" && Component.contextType !== null) {
      const componentName = getComponentNameFromType(Component) || "Unknown";
...
function updateSuspenseOffscreenState(prevOffscreenState: OffscreenState, renderLanes: Lanes): OffscreenState {
...
function shouldRemainOnFallback(current: Fiber | null, _workInProgress: Fiber, _renderLanes: Lanes): boolean {
  ...
  if (current !== null) {
    const suspenseState = current.memoizedState as SuspenseState | null;
...
function getRemainingWorkInPrimaryTree(current: Fiber | null, primaryTreeDidDefer: boolean, renderLanes: Lanes): Lanes {
...
// Attaches the pending transitions (and the tracing markers above) to a
// primary child fragment's queue. Transition tracing only.
function attachPendingTransitions(primaryChildFragment: Fiber, current: Fiber | null): void {
  const currentTransitions = getPendingTransitions();
  if (currentTransitions !== null) {
    const parentMarkerInstances = getMarkerInstances();
    const offscreenQueue = primaryChildFragment.updateQueue as OffscreenQueue | null;
    const currentOffscreenQueue = current !== null ? (current.updateQueue as OffscreenQueue | null) : null;
    ...
    } else if (current !== null && offscreenQueue === currentOffscreenQueue) {
...
function updateSuspenseComponent(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  ...
  if (current === null) {
    ...
    if (showFallback) {
      ...
      if (enableTransitionTracing) {
        attachPendingTransitions(primaryChildFragment, null);
      }
  ...
  if (showFallback) {
    ...
    const nextFallbackChildren = nextProps.fallback;
    const nextPrimaryChildren = nextProps.children;
    updateSuspenseFallbackChildren(current, workInProgress, nextPrimaryChildren, nextFallbackChildren, renderLanes);
    const primaryChildFragment = workInProgress.child!;
    const prevOffscreenState = current.child!.memoizedState as OffscreenState | null;
    primaryChildFragment.memoizedState =
      prevOffscreenState === null
        ? mountSuspenseOffscreenState(renderLanes)
        : updateSuspenseOffscreenState(prevOffscreenState, renderLanes);
    if (enableTransitionTracing) {
      attachPendingTransitions(primaryChildFragment, current);
    }
    primaryChildFragment.childLanes = getRemainingWorkInPrimaryTree(current, didPrimaryChildrenDefer, renderLanes);
    workInProgress.memoizedState = SUSPENDED_MARKER;
    return bailoutOffscreenComponent(current.child, primaryChildFragment);
  }
  if (prevState !== null && includesOnlyRetries(renderLanes) && includesSomeLane(renderLanes, current.lanes)) {
...
function mountSuspensePrimaryChildren(workInProgress: Fiber, primaryChildren: unknown, renderLanes: Lanes): Fiber {
  const mode = workInProgress.mode;
...
function mountWorkInProgressOffscreenFiber(offscreenProps: OffscreenProps, mode: TypeOfMode, _renderLanes: Lanes): Fiber {
  // The props argument to `createFiberFromOffscreen` is `any` typed, so we use
  // this wrapper function to constrain it.
  return createFiberFromOffscreen(offscreenProps, mode, NoLanes, null);
}

function updateWorkInProgressOffscreenFiber(current: Fiber, offscreenProps: OffscreenProps): Fiber {
...
function retrySuspenseComponentWithoutHydrating(current: Fiber, workInProgress: Fiber, renderLanes: Lanes): Fiber {
  // Falling back to client rendering. Because this has performance
  // implications, it's considered a recoverable error, even though the user
  // likely won't observe anything wrong with the UI.
...
function updateDehydratedSuspenseComponent(
  ...
  if (!didSuspend) {
    ...
    if (isSuspenseInstanceFallback(suspenseInstance)) {
      ...
      const details = getSuspenseInstanceFallbackErrorDetails(suspenseInstance);
      const digest = details.digest;
      const message = isDevelopment ? details.message : undefined;
      const stack = isDevelopment ? details.stack : null;
      const componentStack = isDevelopment ? details.componentStack : null;
  ...
  if (workInProgress.flags & ForceClientRender) {
    ...
    workInProgress.flags &= ~ForceClientRender;
    return retrySuspenseComponentWithoutHydrating(current, workInProgress, renderLanes);
  } else if ((workInProgress.memoizedState as SuspenseState | null) !== null) {
    // Something suspended and we should still be in dehydrated mode.
    // Leave the existing child in place.
...
function scheduleSuspenseWorkOnFiber(fiber: Fiber, renderLanes: Lanes, propagationRoot: Fiber): void {
...
function propagateSuspenseContextChange(workInProgress: Fiber, firstChild: Fiber | null, renderLanes: Lanes): void {
...
function findLastContentRow(firstChild: Fiber | null): Fiber | null {
  ...
  let row = firstChild;
  let lastContentRow: Fiber | null = null;
  while (row !== null) {
...
function validateRevealOrder(revealOrder: unknown): void {
  if (isDevelopment) {
    const cacheKey = revealOrder == null ? "null" : String(revealOrder);
    if (
      revealOrder != null &&
...
function validateTailOptions(tailMode: unknown, revealOrder: unknown): void {
  if (isDevelopment) {
    const cacheKey = tailMode == null ? "null" : String(tailMode);
    if (!didWarnAboutTailOptions[cacheKey]) {
      if (tailMode == null) {
        // The default tail is now "hidden".
      } else if (tailMode !== "visible" && tailMode !== "collapsed" && tailMode !== "hidden") {
...
function initSuspenseListRenderState(
  ...
  if (renderState === null) {
    ...
    workInProgress.memoizedState = newRenderState;
  } else {
...
function reverseChildren(fiber: Fiber): void {
  ...
  fiber.child = null;
  while (row !== null) {
    const nextRow: Fiber | null = row.sibling;
...
function updateViewTransition(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  ...
  if (pendingProps.name != null && pendingProps.name !== "auto") {
    ...
    workInProgress.flags |=
      current === null ? ViewTransitionNamedMount | ViewTransitionNamedStatic : ViewTransitionNamedStatic;
  } else {
...
function resetSuspendedCurrentOnMountInLegacyMode(current: Fiber | null, workInProgress: Fiber): void {
  if (!disableLegacyMode && (workInProgress.mode & ConcurrentMode) === NoMode) {
    if (current !== null) {
...
function attemptEarlyBailoutIfNoScheduledUpdate(current: Fiber, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  ...
  switch (workInProgress.tag) {
    case HostRoot: {
      ...
      const cache = (current.memoizedState as RootState).cache;
    ...
    case ActivityComponent: {
      const state = workInProgress.memoizedState as ActivityState | null;
      if (state !== null) {
    ...
    case SuspenseComponent: {
      const state = workInProgress.memoizedState as SuspenseState | null;
      if (state !== null) {
        ...
        const contextChanged = lazilyPropagateParentContextChanges(current, workInProgress, renderLanes);
        const primaryChildFragment = workInProgress.child!;
        const primaryChildLanes = primaryChildFragment.childLanes;
        if (contextChanged || includesSomeLane(renderLanes, primaryChildLanes)) {
    ...
    case SuspenseListComponent: {
      ...
      if (renderState !== null) {
        ...
        // update in the past but didn't complete it. (Upstream also clears a
        // vestigial `lastEffect` field here that nothing reads.)
        renderState.rendering = null;
    ...
    case TracingMarkerComponent: {
      ...
      // Upstream falls through to the LegacyHidden case.
...
function beginWork(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  ...
  if (current !== null) {
    ...
    if (
      ...
      (isDevelopment ? workInProgress.type !== current.type : false)
    ) {
  ...
  switch (workInProgress.tag) {
    case LazyComponent: {
      const elementType = workInProgress.elementType as LazyComponentType<unknown, unknown>;
      return mountLazyComponent(current, workInProgress, elementType, renderLanes);
    }
    case FunctionComponent: {
      const Component = workInProgress.type as RenderFunction;
      return updateFunctionComponent(current, workInProgress, Component, workInProgress.pendingProps as AnyProps, renderLanes);
    }
    case ClassComponent: {
      const Component = workInProgress.type as ClassComponentType;
      const unresolvedProps = workInProgress.pendingProps as AnyProps;
    ...
    case HostHoistable:
      ...
      // Fall through to the singleton and component paths, in order.
    ...
    case HostSingleton:
      ...
      // Fall through to the component path.
      return updateHostComponent(current, workInProgress, renderLanes);
    case HostComponent:
      return updateHostComponent(current, workInProgress, renderLanes);
    case HostText:
    ...
    case ForwardRef: {
      return updateForwardRef(
        current,
        workInProgress,
        workInProgress.type as ForwardRefType,
        workInProgress.pendingProps as AnyProps,
        renderLanes,
      );
    }
    case Fragment:
    ...
    case MemoComponent: {
      return updateMemoComponent(
        current,
        workInProgress,
        workInProgress.type as MemoType,
        workInProgress.pendingProps as AnyProps,
        renderLanes,
      );
    }
    case SimpleMemoComponent: {
      return updateSimpleMemoComponent(
        current,
        workInProgress,
        workInProgress.type as RenderFunction,
        workInProgress.pendingProps as AnyProps,
        renderLanes,
      );
    }
    ...
    case IncompleteFunctionComponent: {
      ...
      const unresolvedProps = workInProgress.pendingProps as AnyProps;
      const resolvedProps = resolveClassComponentProps(Component, unresolvedProps);
      return mountIncompleteFunctionComponent(current, workInProgress, Component, resolvedProps, renderLanes);
    }
    case SuspenseListComponent: {
      return updateSuspenseListComponent(current, workInProgress, renderLanes);
    }
