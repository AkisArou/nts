...
llvm_rc() { ( NTS_BACKEND=llvm NTS_RC=1; export NTS_BACKEND NTS_RC
  backend_examples 90 "through the LLVM backend, counting" ); }
...
llvm() { ( NTS_BACKEND=llvm; export NTS_BACKEND
  backend_examples 90 "through the LLVM backend" ); }
...
jvm() { ( NTS_BACKEND=jvm; export NTS_BACKEND
  ...
  # Seventy-eight of ninety, measured at a commit rather than in the shared tree.
  ...
  # less.
  backend_examples 78 "through the JVM backend" ); }
corpus() {
