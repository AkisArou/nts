...
static uint32_t nts_uv_slot_claim(NtsUvTimer *timer) {
  ...
  if (nts_uv_slots_len == nts_uv_slots_capacity) {
    uint32_t capacity =
        nts_uv_slots_capacity ? nts_uv_slots_capacity * 2u : 16u;
...
static void nts_uv_require_owner(const char *what) {
  if (!nts_uv_is_owner(0)) {
    fprintf(stderr,
            "nts uv host: %s from a thread that does not own the loop\n", what);
    abort();
  }
}
...
static NtsTimerId nts_uv_post_delayed(void *state, NtsTask task,
