...
const contextStackCursor: StackCursor<HostContext | null> = createCursor<HostContext | null>(null);
const contextFiberStackCursor: StackCursor<Fiber | null> = createCursor<Fiber | null>(null);
const rootInstanceStackCursor: StackCursor<Container | null> = createCursor<Container | null>(null);
...
const hostTransitionProviderCursor: StackCursor<Fiber | null> = createCursor<Fiber | null>(null);

function requiredContext<Value>(c: Value | null): Value {
  if (isDevelopment && c === null) {
    console.error(
...
function getCurrentRootHostContainer(): Container | null {
...
function pushHostContext(fiber: Fiber): void {
  const stateHook = fiber.memoizedState as Hook | null;
  if (stateHook !== null) {
  ...
  const nextContext = getChildHostContext(context, fiber.type as string);
