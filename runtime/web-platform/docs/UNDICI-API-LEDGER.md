...
| `Pool`                                       | shared   | Fixed provider dispatchers have common round-robin selection, lifecycle and aggregate stats; exact constructor/options remain.                                                         |
| `RoundRobinPool`                             | shared   | Deterministic fixed-member selection, stats and exact-reason lifecycle exist.                                                                                                          |
| `BalancedPool`                               | shared   | Smooth weighted selection, typed transport-error health, bounded mutation, safe request rerouting and lifecycle exist; exact facade remains.                                           |
...
| DNS caching                                  | shared   | Bounded TTL/LRU storage, miss coalescing, rotation, invalidation, typed provider records and physical-endpoint connector integration exist.                                            |
| Happy Eyeballs                               | provider | Bounded alternating-family connection attempts, stagger/fast-failure behavior and exact cancellation exist; parallel family-DNS timing and production-provider/device evidence remain. |
...
| DNS interceptor                                   | shared     | `DnsCache` plus `DnsConnector` apply the behavior at the typed connector seam without rewriting logical request/TLS identity; exact Undici facade remains.                             |
...
| tracing/diagnostics hooks                         | shared     | Environment-local typed request/header/trailer/error observation exists with credential redaction and no body wrapping; provider connection/body-sent events and exact facades remain. |
...
| diagnostics channels                       | facade   | Node must map the typed shared events without putting `diagnostics_channel` in shared code; provider connection/body-sent events still need plumbing. |
| debug logging                              | facade   | Shared events redact credentials and query values by default; the opt-in Node logger facade remains.                                                  |
