...
| ✗ | an **elision in an array literal** — `[1, , 3]`, `[, , ,]` | refused by name as `an omitted expression`. **Measured 2026-09-18**: 12 of the 413 slice-1 test262 files that reach lowering, and half of those are in the generated `class/dstr` family calling a pattern with `[,]`. Not the same production as a **hole in a destructuring pattern** (`const [, second] = pair`), which works and has `examples/a-hole-in-a-destructuring-pattern` — the census report spent a paragraph establishing that the corpus's elision coverage was all the latter, and 18 files separate the two. A hole is `undefined` at that index, so an array whose element type admits an absence could hold one and a `number[]` could not; that asymmetry is the whole of the work, and nothing in the corpus needed it before test262. |
