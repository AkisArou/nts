...
[ "$(wc -l <"$out/expected.txt")" -eq 47 ] ||
  { echo "macos-classes: the oracle printed $(wc -l <"$out/expected.txt") lines, not 47" >&2; exit 1; }
...
if [ "$differs" != "> answered alive|> fields alive held|> inherited alive|> constructed alive|> replaced alive|> array alive|> maps 1 true true alive|> mapped alive|> object alive|" ]; then
  echo "macos-classes: under NoGc the difference from the oracle was [$differs], not the nine lifetime lines" >&2
  exit 1
fi
