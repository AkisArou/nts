...
import { enableAsyncDebugInfo, enableConditionalUseWarning } from "shared/ReactFeatureFlags.ts";
import { noop } from "shared/noop.ts";

import { callLazyInitInDEV } from "./ReactFiberCallUserSpace.ts";
import { getWorkInProgressRoot } from "./ReactFiberWorkLoop.ts";
import { ReactSharedInternals } from "./ReactSharedInternals.ts";
import { HostRoot } from "./ReactWorkTags.ts";

// The thenables a component has used, by the order of its `use` calls, so
// that a re-render after a ping reads the same ones. Upstream keeps a bare
// array in production and wraps it in development; one shape serves both.
export class ThenableState {
  didWarnAboutUncachedPromise: boolean = false;
  readonly thenables: Thenable<unknown>[] = [];
}

// An error that is thrown (e.g. by `use`) to trigger Suspense. If we
// detect this is caught by userspace, we'll log a warning in development.
export const SuspenseException: Error = new Error(
...
export const SuspenseyCommitException: Error = new Error(
  "Suspense Exception: This is not a real error, and should not leak into " +
    "userspace. If you're seeing this, it's likely a bug in React.",
);

export const SuspenseActionException: Error = new Error(
  "Suspense Exception: This is not a real error! It's an implementation " +
...
export const noopSuspenseyCommitThenable: Thenable<unknown> = {
  then(): void {
    if (isDevelopment) {
      console.error(
...
export function clearUseWarnings(): void {
  lastSuspendedFiber = null;
}

export function trackUsedThenable<T>(
  thenableState: ThenableState,
  thenableInput: Thenable<T>,
  index: number,
  fiber: null | Fiber, // DEV-only
): T {
  let thenable = thenableInput;
  ...
  if (previous === undefined) {
    ...
    if (previous !== thenable) {
      ...
      if (isDevelopment) {
        if (!thenableState.didWarnAboutUncachedPromise) {
  ...
  if (isDevelopment && enableAsyncDebugInfo && thenable._debugInfo === undefined) {
...
export function suspendCommit(): never {
...
export function resolveLazy<T>(lazyType: LazyComponent<T, unknown>): T {
  try {
    if (isDevelopment) {
      return callLazyInitInDEV(lazyType) as T;
    }
    const payload = lazyType._payload;
    const init = lazyType._init;
    return init(payload);
  } catch (x) {
    if (x !== null && typeof x === "object" && typeof (x as { then?: unknown }).then === "function") {
      // This lazy Suspended. Treat this as if we called use() to unwrap it.
      suspendedThenable = x as Thenable<unknown>;
      if (isDevelopment) {
        needsToResetSuspendedThenableDEV = true;
      }
      throw SuspenseException;
...
export function checkIfUseWasUsedBefore(unsuspendedFiber: Fiber, thenableState: null | ThenableState): void {
  if (isDevelopment && enableConditionalUseWarning) {
    if (lastSuspendedFiber !== null && areSameKeyPath(lastSuspendedFiber, unsuspendedFiber)) {
