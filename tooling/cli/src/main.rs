...
fn build(rest: &[String]) -> Result<()> {
    ...
    for (name, product) in chosen {
        ...
        for target in targets_for(name, product, only_os.as_deref())? {
            ...
            if matches!(target.backend.as_str(), "c" | "llvm") {
...
fn compile_program(
    ...
    for source in sources {
        ...
        // An LLVM module has no headers to find and no C to read a define:
...
const PROGRAM_SOURCE_NAME: &str = "program.c";
/// The program as the LLVM backend renders it, compiled in `program.c`'s place.
const PROGRAM_IR_NAME: &str = "program.ll";
...
fn link_c(
    ...
    //
    // `.init_array` is what the platform provides for exactly this, and it runs
    // before any consumer code. An executable does not get one: `main.c`
    // already calls it, and two calls would evaluate the module twice.
    //
    // **It goes in `program.c`, and the sentence that stood here said it did
    // not have to.** That sentence was "it works the same in an archive", and
    // an archive is exactly where it does not: a static library member is
    // linked only if something already references it, `nts_auto_init` is
    // `static` and so publishes nothing, and a translation unit holding nothing
    // but a constructor can therefore never be referenced. `ld` drops it, the
    // `.init_array` entry goes with it, and module evaluation silently does not
    // happen.
    //
    // Measured 2026-09-23 on a `library.staticNative` whose module scope builds
    // a heap array. One variable separates the two arms:
    //
    //     clang caller.o liblib.a                     SIGSEGV, no nts_auto_init in `nm`
    //     clang caller.o -Wl,--whole-archive liblib.a 12.5, which is the answer
    //
    // A wrong answer rather than a link error, which is the trade the paragraph
    // above refuses to make -- and it was made here, by a comment.
    //
    // `program.c` defines every exported function, so a consumer that reaches
    // this library at all has already pulled that member in and the constructor
    // comes with it. Appended rather than written beside it for exactly that:
    // a file of its own is a member with no reason to be linked. The GTK lane
    // found it; `examples/interop/library-module-state` is the arm that fails
    // without this.
    let sources = wrote.sources.clone();
    if library && wrote.initializes {
        append_library_initialiser(out, &sources)?;
    }
...
/// Module evaluation when a library loads, appended to the program's own unit.
///
/// Appended rather than written beside it: an archive member nothing references
/// is not linked, and a unit holding only a constructor never is. The GTK lane
/// found it; `examples/interop/library-module-state` is the arm that fails
/// without this. For an LLVM module the constructor is `llvm.global_ctors`,
/// which is what clang makes of the attribute.
fn append_library_initialiser(out: &Utf8Path, sources: &[String]) -> Result<()> {
    let ir = sources.iter().any(|source| source == PROGRAM_IR_NAME);
    let (program, text): (Utf8PathBuf, &[u8]) = if ir {
        (
            out.join(PROGRAM_IR_NAME),
            b"\n; Generated by nts. Runs module evaluation when the library loads.\n\
             @llvm.global_ctors = appending global [1 x { i32, ptr, ptr }] [{ i32, ptr, ptr } { i32 65535, ptr @nts_auto_init, ptr null }]\n\
             define internal void @nts_auto_init() {\n  call void @module__init()\n  ret void\n}\n",
        )
    } else {
        (
            out.join(PROGRAM_SOURCE_NAME),
            b"\n/* Generated by nts. Runs module evaluation when the library loads.\n\
               Here rather than in a file of its own: an archive member nothing\n\
               references is not linked, and a unit holding only a constructor\n\
               never is. */\n\
...
struct Emission<'a> {
    ...
    /// The program rendered by the LLVM backend for this platform, in place of
    /// `program.c`. Everything else the C emission writes -- the runtime, the
    /// entry point, the loop host -- is the same files either way.
    llvm: Option<nts_codegen_llvm::Platform>,
}
...
fn emit_c(tsconfig: &Utf8Path, out: Option<&Utf8Path>, emission: Emission) -> Result<Wrote> {
    ...
    if let Some(platform) = emission.llvm {
...
/// The LLVM backend's rendering of `program`, written as `program.ll` and
/// compiled in place of `program.c`.
///
/// **The C emission's other files are the LLVM build's too.** The runtime,
/// `main.c` and the loop host are C whichever backend renders the program,
/// and linking them with an LLVM module is what the differential and the
/// macOS fixtures had been doing by hand; this makes it the build's.
///
/// A function the backend declines is fatal, as the C emitter's are: an
/// absent definition is an undefined symbol at the link, which names neither
/// the function nor the construct behind it.
fn write_llvm_program(
    snapshot: &nts_semantic_schema::SemanticSnapshot,
    program: &hir::Program,
    out: &Utf8Path,
    ...
    if !rendered.diagnostics.is_empty() {
        bail!(
            "the llvm backend declined {} function(s), each named above; build this product on the c backend",
