...
int main(void) {
  ...
  // A promise job queued inside the callbacks has not run when the export
  // returns: no foreign loop, so no checkpoint at a callback's return.
  double ran = aJobStaysQueued();
  printf("aJobStaysQueued() = %d\n", (int)ran);
  assert(ran == 0);
