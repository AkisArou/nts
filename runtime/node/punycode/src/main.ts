...
// harder to follow.
//
// Deprecated in node since v7 and still shipped, so still part of the surface —
// including the warning, which node's own test asserts on. Node suppresses it
// when the caller is inside `node_modules`, on the grounds that a dependency's
// use of it is not something the application can fix.
...
declare function nts_is_inside_node_modules(): boolean;

if (!nts_is_inside_node_modules()) {
...
function splitLabels(domain: string): string[] {
  ...
  for (let i = 0; i < domain.length; i++) {
    ...
    normalized +=
      c === 0x2e || c === 0x3002 || c === 0xff0e || c === 0xff61 ? "." : domain[i]!;
  }
...
export function toUnicode(input: string): string {
  return mapDomain(input, (label) =>
    label.toLowerCase().startsWith("xn--") ? decode(label.slice(4).toLowerCase()) : label,
  );
}
