...
| ✗ | a nested function whose name is already taken at the top level — the namespace is flat, so both are refused. The name is not qualified by the function it is written in | **A second, larger thing wears a neighbouring message and is priced as of 2026-09-13**: a nested function reading a local of the function that declares it — `a name from an enclosing scope`, 20 things across 17 modules, the eighth cause in the census. It is a **desugaring rather than a feature**: the same function written as a const-bound expression, or as an arrow, lowers and agrees with node over 58 cases, so a nested `function` declaration is exactly a hoisted `const` binding to a function expression and all the capture machinery is present. What it costs is five coordinated places, each of which is right about module scope and lacks the same distinction — `collect_closures`, `reached_by_name` (which answers `true` for *any* `FUNCTION_DECLARATION`), the named-declaration collection loop, the statement walk that must hoist the binding, and the call sites. 48 nested declarations in `runtime/node`, **20 used as a value**, so lambda lifting reaches at most half and the desugaring reaches all. `blockers/enclosing-scope-name-in-a-nested-function` carries the shape.
