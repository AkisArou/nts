...
| | | |
|---|---|---|
| ✅ | arithmetic | `+ - * / % **` — `examples/math` for `**` on numbers, including `Infinity` and a fractional exponent; `examples/bigint` for `**` on `bigint`. **`**` is not C's `pow`, in two places that are the same place twice**: C99 answers 1 where `Number::exponentiate` answers NaN, both times because the *limit* is 1. The `(±1) ** ±Infinity` half was corrected when `nts_math_pow` was written; the `x ** NaN` half was not, and `1 ** NaN` answered 1 until 2026-09-19 — C99 F.10.4.4 makes `pow(+1, y)` return 1 for any `y`, **even a NaN**, and step 1 of the specification is "if exponent is NaN, return NaN" *before* it looks at the base. Found by the test262 census (`applying-the-exp-operator_A4.js`, which ran and threw) rather than by `examples/math`, whose comment already records the same trap one pool dimension over: **the driver pairs its arguments rather than crossing them**, so a base of 1 had never been handed a NaN exponent. `aNaNExponent` makes the exponent NaN for *every* input for that reason — the first version varied it, and passed on the unfixed compiler. **The JVM needed no change**, measured rather than assumed: Java's `Math.pow` answers NaN for both cases where glibc answers 1, so a guard added there was redundant and its sabotage passed. `NtsRuntime.mathPow` carries the measured table so nobody adds one. |
...
| ✅ | `in` over the **prototype chain** — `"valueOf" in {}` is `true` `examples/an-inherited-name-in-an-in-test`, seven cases on all three backends. **Landed 2026-09-18**, found by running test262 `language/expressions/in/S8.12.6_A2_T1.js`, one of four files in the 2,527-file slice-1 population that compiled, ran, and answered wrongly; it emits no diagnostic, so nothing that ranks refusals could find it. Every path in `lower_in` asked which members the receiver's *type* declares, and an inherited one is declared by none of them. `Object.prototype`'s own names — the seven the specification requires plus `__proto__` and the four Annex B accessors, because node has them and node is the oracle — answer `true` for an object, a class instance or an array. **The precondition is that no object here can have a null prototype**: `Object.create` and `Object.setPrototypeOf` are both refused today, checked rather than assumed, and whoever implements either has to return to `lower_in` because `Object.create(null)` makes every one of these answers wrong with nothing else noticing. TypeScript's **`object`** is covered by name — the non-primitive type, which is what these sites narrow *to* (`value !== null && typeof value === "object" && "k" in value` is how a program duck-types an `unknown`, and it is the most common `in` receiver in `runtime/node`). TypeScript's **`{}`** deliberately is not: it accepts every value but `null` and `undefined`, strings among them, so `"valueOf" in ("a" as {})` throws in node — which also means an empty object literal, whose inferred type is `{}`, still answers `false`. Strings are excluded: `"length" in "abc"` *throws* in JavaScript. A lying cast defeats that — `s as unknown as Record<string, number>` answers `true` where node throws — which is not a loss of reach, since that program previously emitted C that did not compile, but is a change from a broken build to a wrong answer and is written down at the site. It does not make the member callable, and `"push" in []` is still false: `Array.prototype`'s own names are separate work. **The operand this branch was not evaluating**, found 2026-09-19 by reading rather than by the census: `in` evaluates its right operand before it looks anything up, and a constant answer for an inherited name was returned *without touching the receiver* — so `"toString" in make()` never called `make`. A counter the receiver incremented stayed at 0 where node had 1, while the same program with a **declared** key agreed, because that path lowers the receiver like everything else. A constant answer is not a reason to skip the operand; it is the reason it is easy to. **Still wrong**: `"toString" in {}` for an *unannotated* empty literal, whose checker type is literally `{}` and so is excluded for the reason above — the receiver's *representation* would tell an object from a string where its type cannot, but reading it means lowering the receiver before the decision rather than after, and every other path in `lower_in` lowers it for itself. `const o: object = {}` and `const o: Record<string, number> = {}` both work, so nothing is blocked on it. |
...
| ✅ | `new` | user classes, `Array`, typed arrays — `examples/array-buffer` and `examples/data-view` construct both. **`new Uint8Array([1, 2, 3])` as of 2026-09-19** — `examples/a-typed-array-from-an-array`, 203 cases across 7 functions on all five variants; it was refused by name, the constructor taking a length or an `ArrayBuffer` and nothing else. The view is sized to the array and filled with `Place::Element` per index, which is what `a[i] = v` already lowers to, so **the narrowing lives in one place**: `300` becomes `44` and `-1` becomes `255` because the store says so, not because this repeats the rule. An *array* only — any other iterable wants `Iterable<T>`, and `new Uint8Array(anotherView)` wants `nts_view_set`, which copies between two views and is a different operation; both still refuse by name. **`new Uint8Array([])` needed the expectation**, and expecting `Array(u8)` — the obvious spelling — was wrong twice: it duplicates the narrowing, and it manufactures a `u8` array, a representation no TypeScript type asks for. It also produced wrong answers, in the *specialized* copy of the enclosing function and only for small non-negative integer arguments; recorded in the fixture as a latent defect in something else rather than chased, since at `number[]` nothing builds one. **`Uint8Array.of(…)` and `Uint8Array.from(xs)` land with it**, both `not a member of this compiler's Uint8Array` before: `from` with one array argument *is* the constructor's array case and routes to the same code, and `typed_array_shape` is the one place all three ask what a `Uint8Array` is. `of` takes its elements as arguments, so the array is built here with the argument count as its length. **`from`'s mapping function is refused by name** rather than ignored, which would be the silently wrong answer this row is otherwise about. This clears `Uint8Array.of` from `runtime/web-platform`, where it was one of two causes of the `addons` gate step's `http` and `process` regressions; `Object.getPrototypeOf` is the other and remains. The **methods** on a typed array are a separate question and 17 of them refuse; see §8 **`undefined[]` is refused**, as of 2026-09-19 and for a failure worse than a refusal: `[undefined]` has `HirType::Void` as its element, the C backend writes the element type into every access, and `NTS_ITEMS(v, void)[i]` is not C — so the compiler exited 0, wrote the file, and `cc` failed on generated code. Reached from an untyped `([x = 23] = [undefined])` parameter default, which is two `test/language` files; pre-existing, bracketed on a compiler from before any of this. **`Array` alone**: `Promise<void>` shares the arm and is the most ordinary thing in the language, so the test names the container rather than the element. **`never[]` joins it** — `for (const [x = 23] of [[]])` makes one, the inner `[]` having nothing to say what it holds — which is the last three of the 135 and also pre-existing. |
...
| ◐ | `?.`, `?.()`, `?.[]` | member, call and index, through either absence or both. **`o?.m()` on a class emitted C that does not compile, until 2026-09-18.** The merge parameter of an optional call is erased, because one arm is `undefined`, so lowering types the call erased to match; `specialize` then narrows the call to what `m` declares and bridges every reader so the merge edge still receives what it expects. The bridge was `OpKind::Convert`, which is a *numeric* conversion — an erasure is not one — so it emitted `v22 = (NtsValue)v10` and clang said *used type 'NtsValue' where arithmetic or pointer type is required*, on a program every pass before it called well typed. Its neighbours are what date it: `o?.b` and `o?.items` were always right, because only a **call** has a declared return for that pass to narrow, so only a call reached the bridge. The two erasure directions are now named rather than folded in with the numeric one. `examples/an-optional-method-call-on-a-class` is 174 cases across 6 exports on C, LLVM, the JVM and under `rc`, and produces **2 clang errors** on the commit before. Two *optional* links chain — each tests its own receiver — and a **non**-optional link after an optional one is refused and named. **That last clause was false from the day `a?.b()` landed until 2026-09-18, and what stood in its place was wrong answers rather than a refusal.** The rule lived inline in `lower_property_access` and tested one thing — is my object a *property access* whose second child is `?.` — under a comment stating its own precondition: *"all twenty-six optional accesses in the node profile are a single link"*. An optional **call** is a link too and is a `CallExpression`, which that test cannot see, and `lower_method_call` had no test at all. Three shapes reached code generation unrefused and none was right: `a?.m().p` answered `3` where node answers `-1`, and `a?.m().m2()` and `a?.p.m()` **aborted the compiled program**. A fourth, `a?.p[i].q`, was found by the example written to pin the boundary — it disagreed on 5 of 29 cases — and says the rule has to ask about the whole chain rather than about the node in front of it. All four now refuse through one `ends_an_optional_chain`, asked from both link sites; `blockers/a-link-after-an-optional-call` carries them and `examples/an-index-after-an-optional-link` pins the boundary that must keep working. **A seventh shape segfaulted, and it is the one the guard's *position* hid.** A **string** index after an optional link — `a?.label[1]` — was intercepted by `lower_string_index` before anything asked the chain question, so it was neither refused nor right: the compiled program **crashes with signal 11** on 9 of 29 cases, on C, LLVM and the JVM alike. Not a decline, not a wrong number, a crash, and long predating the guard — the dispatch above it had already chosen what kind of index this was before anything asked whether the index runs at all. The chain question now goes first, which is the general form: *what kind* of link it is only matters once it is established that the link runs. **A fifth and sixth are index links, and they nearly went unrefused — twice.** The element read is emitted *after* the merge, so `a?.p[i]` and `a?.m()[i]` read on the absent path. On C that **aborts the program** — `element of a non-array, which the lowering proved was an array` — for every input taking the absent arm, with a constant index as much as a computed one. On the JVM it answers, because an element of `undefined` is `undefined` there and a trailing `??` folds it to the same answer the short-circuit would have given: agreement by coincidence of the surrounding operator, on one backend. **What hid it is that the differential folds a decline into *skipped*, never into *disagreed*** — `agreed on every case` is its last line while the line above it reads `17 case(s) the compiled program declined`. Two of my measurements read only the verdict and reported the shape working on all three backends, and the example I wrote to pin it as working was itself hollow: 17 declines on C, LLVM comparing **3 of 87** cases, `agreed on every case` on both. The JVM lane refused the first diagnosis and sent the IR, which is how it was caught; driving the absent arm *exclusively* is what settles it, and the instrument then says so in words — "no case was checked: all 17 declined, so the two sides were never compared". Both now refuse. `a?.[i]`, a **single** optional index, is untouched and still lowers, because `lower_optional_element` passes the index node *into* the branch. **The row is ◐ rather than ✅ because seven ordinary shapes are refused rather than lowered**, and the repair for all seven is one thing: the rest of the chain belongs inside the present arm rather than after the merge, which `Branch::Member` and `Branch::Element` already do for a single link. **That repair was built and measured on 2026-09-18 and is deliberately not landed.** One test at the innermost link, the rest of the expression re-lowered inside the present arm with that link standing for the narrowed receiver, and the narrowed links held on a **stack** — a single slot let a two-link chain unmark its own first link and recurse to `fatal runtime error: stack overflow`. It passed every shape on C and the JVM, side-effecting arguments included. It is held back because of LLVM: a link inside the arm takes the *whole chain's* type, `T | undefined`, so a read that is `array.get unchecked` in the equivalent unchained program becomes `erase` plus `nts_array_element`, and 17 of 29 cases then decline under LLVM while agreeing under the other two. **The pessimisation is the objection and the divergence is its symptom** — a link in the present arm should be typed by its own result, not by the type of the chain that may short-circuit around it, and whoever takes this should start there rather than at the backend. The blocker carries the design. `addons.sh` is 24 of 24 with 0 regressed across the change, and all 253 examples still agree, so no module and no fixture depended on the shapes that were wrong **Probed 2026-09-13 and the ✅ is wrong for one of the three.** `a?.b()` -- an optional chain whose next step is a **method call** -- refuses by name: ``an optional-chained method call (`a?.b()`)``. `lower_method_call` destructures its callee as `[receiver, member]`, and an optional-chained member access has **three** children because the `?.` is a token of its own between them, so the shape falls through to a refusal. What is green is the neighbouring spelling: `a.b?.()`, a function-valued property called optionally, which is row 162 and lowers. The two look alike and are different nodes. **Measured**: 76 distinct source sites across 24 files in `runtime/node`, and 241 *refusal instances* across five modules -- the larger number counts a site once per specialisation of the function holding it, so the two are different units and neither is the other. Uniform in shape: **zero** of the 76 carry a second `?.`, so none is the `a link after an optional access` case that is separately refused. Tractable and unbuilt: the machinery exists beside it -- `lower_optional_access` and the `Branch::Invoke` arm already do `absence_of` then `present_of` then `lower_branching_value` -- and what it wants is the same three steps with a method call in the present arm, which needs `lower_method_call`'s 183 lines split so the arm can be handed an already-lowered receiver. **Built 2026-09-13, and the row is green again on evidence rather than on assumption.** `lower_method_call` is split: `lower_method_on` takes a receiver that is already lowered, and `lower_optional_method_call` does the three steps `lower_optional_access` does -- `absence_of`, `present_of`, `lower_branching_value` -- with a new `Branch::MethodOn` in the present arm. The arguments are lowered **inside** that arm, so `a?.b(f())` does not call `f` when the receiver is absent, and `examples/an-optional-chained-method-call` has a counter arm proving it. Both arms of the control: the pre-change compiler at `5031bc2c` refuses the example at three sites, and with the change it agrees with node on 116 cases across 4 functions. Corpus, both binaries on one tree: the refusal goes **42 -> 0** in `net`, **56 -> 0** in `http`, **50 -> 0** in `stream`, **58 -> 0** in `fs` and **35 -> 0** in `events`. Root refusals fall only 36 across those five, because clearing one refusal in a function exposes the next one behind it -- a feature can be entirely landed and barely move the headline, which is why the specific message was counted rather than the total. |
...
| ✅ | **growing an array by writing past its end** — `xs[xs.length] = v` `examples/an-array-grown-by-index`, 174 cases on all three backends. **Landed 2026-09-19; it used to abort** — `nts: refused: index 1 is outside [0, 1)`, exit 134, with no diagnostic and nothing naming the construct. `nts_slot_or_grow` is the check inline and the append behind a call, split the way `nts_array_push` is split and for the reason its comment gives. **No cost on the fast path**: `bounds.rs` already eliminates a checked `ArraySet`, so a proven index has no check *and* no growth, and an unproven one was already paying for a check. Measured on `array-mutations` (629.9ns → 630.5ns), `array-predicates` and `arrays` (both unchanged), before and after, on an asserted worktree. **By one only.** `xs[5] = v` on a length-1 array wants four holes, a hole reads as `undefined`, and a dense array of numbers has no room for one — so a sparse write still aborts on C and LLVM. The **JVM diverges here and did before this**: its growable wrapper's `set` grows to any index and fills with the element's zero, which is a wrong answer rather than an abort, and no example exercised it. **Not for a counted element.** `rc.rs` pairs every store of a reference with a load of what the slot held so it can be released, and at `index == length` there is no such slot; `array_write_may_grow` is the one place that rule is written, asked by all three backends. **The length fact is what made this hard.** `allocated_length_is_exact` lets `[1,2,3].length` fold, and a growing store makes that false after it. Invalidating it for any array with a checked store broke `total` in `examples/arrays`, which has no store the source wrote — a literal's *own* initialising stores are `ArraySet`s and cannot grow anything, since `ArrayNew` made the slots and each store names one by a constant. Excluding exactly those is what makes the rule precise, and `squares` keeps every eliminated check because its `xs[i] = i * i` is proven by the guard relation rather than by the constant length. **The other half of the pair landed 2026-09-19**, in `examples/an-evolving-type-at-module-scope`: `var bases = []; bases[0] = …` at module scope, which is the 26 slice-1 files. Two things were in the way and both were one rule asked in two places. `collect_module_scope` took `type_of` and stopped, and `type_of` answers `never[]` for `[]` — a `Some`, so the `or_else(evolved_type)` beside it never ran; `never[]` is now recognised in one place, `is_an_unsettled_array`, by the three readers that each spelled it themselves. Then `refused_initializers` lowered every module initializer a *second* time with a bare `lower_expression` and so with **no expectation**, refused the literal for want of a type the global already had, and marked the symbol unsupported — so the real lowering, which passes the global's type, never ran. Both now read the same `module.types` entry. **Two guards keep this from widening the abort above.** `growth_can_fill`: the writes that settle such a type are exactly the writes that grow, so a counted element would build the literal and then abort at the first store. It is asked of the element type rather than of the writes, which makes it wider than its hazard — a `push`-filled `string[]` is safe and stays refused — and narrowing it means asking whether any write to the name goes through an index. **`written_as_a_dense_prefix` is the second, and the census is what said it was needed**: six `test/language` files went from `unsupported` to **SIGABRT** the first time a settled type reached them. `applying-the-exp-operator_A11.js` opens `var exponents = []; exponents[3] = Infinity; exponents[2] = …` — dense when it finishes and sparse at every step, and growth is by *one*. So every indexed write must be a constant and each must be the next slot after the last; a compound assignment is rejected outright, since it reads the slot before writing it and could never be the write that creates one. A refusal replaced by an abort is a worse answer, not a better one. Nothing regressed by either guard: every untyped empty literal at module scope refused before. **A `.length`-only program stays refused** too, because the checker widens an evolving array at an *element* read and there is nothing else to ask; deriving the element from the writes instead would be a second derivation competing with the checker's. |
| ✅ | **a bitwise operand that does not fit an `int32`**, fixed 2026-09-21 in all three backends. `"".codePointAt(0) ?? (-1 >>> 4)` answered **0** on C and **134217727** on the JVM against node's 268435455, and LLVM declined it. JavaScript defines `&`, `|`, `^` and the shifts through `ToInt32`/`ToUint32`, which wrap modulo 2^32; each backend narrowed with its own language's conversion --- C's `(int32_t)` (undefined out of range), the JVM's `d2i` (saturates), LLVM's `fptosi` (poison) --- and **each comment cited another backend as justification**. All three now call ECMAScript's conversion, and LLVM additionally grows the float shifts it used to decline. Needed a fuzzer because `-1 >>> 4` alone is constant-folded and a second site computing it narrows the first, so the bug disappears from any reduction that adds one. `examples/a-bitwise-operand-that-does-not-fit-an-int32`. Was: `"".codePointAt(0) ?? (-1 >>> 4)` gives 0 where node gives 268435455. The trigger is an **unsigned shift of a negative value** as the right operand --- `?? (16 >>> 2)` and `?? (-16 >> 2)` are both correct, and so are `?? -1`, `?? -7`, `?? (5 | 0)`, `?? (2 + 3)` and `?? 268435455`. **Adding `export function b() { return -1 >>> 4 }` to the same file makes it correct**, which places it in a specialisation decision taken differently when the value appears twice, not in the nullish lowering. An unrelated function does not fix it, so it is the shift that matters and not the count. `codePointAt` itself is right: `"".codePointAt(0) === undefined` is `true` and `?? -1` gives `-1`. Much improved rather than new --- the 2026-09-18 binary fails **all six** right-operand spellings here, one of them answering `nan`. Found 2026-09-21 by `fuzz-expressions.mjs` at seed 68 |
| ✗ | **growing an array of references by an index write aborts, in three spellings of four** — `const xs = []; xs[0] = "a"` gives `nts: refused: index 0 is outside [0, 0)`, exit 134, with **zero compile-time refusals**. Measured 2026-09-21 across all four positions: only *inferred, at module scope* is refused properly, by `growth_can_fill`; annotated-at-module-scope and both in-function spellings abort. Two controls place it: an array of **numbers** grows by index write everywhere and agrees with node, and **`push`** works everywhere for references too — so what is unsupported is the growing *index write* on a counted element, not growth and not references. Pre-existing (a 2026-09-18 binary aborts identically). Extending `growth_can_fill` is not the fix: it is asked of a declaration and the other three have a fine declared type. The distinguishing fact is what the program later does to the name — a program-wide write analysis, or a bounds check that refuses a write provably past the end, which would also catch `xs[9] = v` on a populated array |
| ✅ | `Promise.withResolvers()` — the capability object | **landed 2026-09-16, as the representation [[0337]] reverted.** `PromiseWithResolvers<T>` holds nothing besides the promise — `resolve` and `reject` are the two settles the runtime already performs with the promise as the receiver, and `promise` is the promise — so it is *represented as* `Promise<T>`: no layout, no allocation, no closure. `examples/promise-with-resolvers` is 203 cases across seven exports and agrees with node, and the arm that earns it is `ordering`, where `consume` suspends at its `await` and node answers `1,2,4,3` rather than `1,3,2,4` — a resumption folded into the call to `resolve` would show. **That example proves nothing on its own and is not what landed this**: it was written by the author of the change from the change's own model, which is precisely [[0337]]'s lesson. The corpus is: `addons.sh` gives **24 of 24 still build, 0 regressed**, controlled against the pinned binary giving the same. **22 distinct sites cleared** across `stream`, `fs`, `zlib`, `util`, `http` and `web-platform`; the NTS1001 totals moved by 1 to 13 per module, because a cleared blocker publishes the next one. **Five distinct sites still refuse** and say so by name — ``a promise capability's `resolve` used as a value rather than called`` — which is `broadcast.ts`'s `state.resolve = pending.resolve` and its four siblings. Those need a closure with no source node, which is [[0336]]'s decomposed form and is now in nobody's way. What [[0337]] measured as the representation failing was a **latent defect in the reject path** it made reachable, recorded as [[0339]]. **The optional spelling refused until 2026-09-18.** `cap.resolve(v)` lowered and `cap?.resolve(v)` did not, because the settle was recognised *syntactically* — a callee that is a property access with exactly **two** children — and `a?.b()` has three, the `?.` being a token of its own. So it fell past the recogniser and out the bottom of the method-call path as *`resolve` on a promise, which has no method table here*, which is also how it was found: the split that made that message name the receiver's representation put 9 of the 37 promise-receiver sites under `resolve`/`reject` rather than under `.then`, and they are a different gap entirely. Teaching the recogniser about three children would have been the wrong fix — the settle must happen **inside the present arm**, which is built by `lower_optional_method_call` and hands a narrowed *value* to `lower_method_on`. Both spellings now meet at `settle_lowered`, and `names_a_promise_capability` looks through a union with an absence, because a capability that may be missing is still one and `named` of `PromiseWithResolvers<T> | undefined` answers nothing. **12 distinct sites cleared** — `fs` 9, `child_process` 2, `assert` 1, each module to zero — all of the corpus shape `this.#writeRequests.dequeue()?.reject(...)`: a queue of waiters, settled if there is one. `examples/a-capability-settled-through-an-optional-link` is 174 cases across 6 exports on C, LLVM, the JVM and under `rc`, and pairs every optional settle with its direct twin; the arm that earns it is `theArgumentIsNotEvaluatedWhenAbsent`, since without it the two paths differ only in a number and a settle that ran on the absent side could still produce the right one. On the binary one commit earlier the same example compares 29 cases across 1 of its 6 exports. `addons.sh` 24 of 24, 0 regressed |
| ✅ | a nested object literal assigned where an **optional** property is declared | it gets its own anonymous type, laid out with a pointer where the declared one has a tagged value. **Re-probed 2026-09-13: it refuses, and nothing segfaults.** The row said reading it back segfaults, which is a wrong answer that runs and would rank this far higher than it deserves; what is live is three refusals by name. The shape matters, because a reduced probe passes: `{ tag: 1, inner: { a, b } }` read straight back, and the same with `inner` absent, **compile and agree with node over 58 cases**. Three harder arms do not — a union of two literals where only one has the property, and `"inner" in o`, both give `an erased value where a concrete representation is wanted`; and passing the object to a function that reads it gives `a `Outer` where `an anonymous type` is wanted, which is a pointer cast between two structs that do not agree about where their shared fields are`. **That second message is the refusal census's number one row** — 48 things, 66 sites, 20 modules, filed as `annotated-const-read` — so this row is a route into that cause rather than a defect of its own, and the two should not be built separately. The first is the census's `an erased value where a concrete representation is wanted`, 20 things across 19 modules. Pre-existing; see the anonymous-type row in §4  **Re-probed 2026-09-17 and two of the three arms have gone false.** `"inner" in o` compiles, and passing the object to a function that reads it compiles; both agree with node across 116 cases on C, LLVM and the JVM. **Not claimed by any change of mine**: the compiler at `914b09d8`, before the slot work below, answers identically — so something between 2026-09-13 and now closed them and nobody came back to this row, which is the most perishable kind of claim there is (prose saying *this does not work* is the sentence a reader is most likely to act on, by not trying). What is still live is **one** arm, re-measured rather than quoted: a union of two literals where only one has the property, `n > 0 ? { tag: 1, inner: {…} } : { tag: 2 }`, which gives `an erased value where a concrete representation is wanted`. Marked ◐ rather than ✗ on that basis.  **Closed 2026-09-18.** The one live arm was a conditional between two literals of different shapes: the checker's type for it is a union, which erases, and the erased value then met the declaration. The merge type is now taken from the **arms** where they already agree on a concrete representation — they do, because every literal is lowered expecting its slot, so erasing to merge and unerasing to store was work to undo work. **Asked of the arms and not of the slot**, which is the correction that matters: a first version took the slot's type whenever the checker's erased, and that refused four exports of `examples/callback-fields` — `h.fn?.(n) ?? -1` is a union containing a genuine *absence*, where the slot is a number, the value may be `undefined`, and what resolves it is the `??` rather than the slot. The arms disagree there and it stays erased. Corpus effect, measured: the `an erased value where a concrete representation is wanted` sites go **31 to 29**. Marked ✅. |
| ✅ | a literal built at **the type of the slot it is written into**, not at its own and cast — and the two silent miscompiles that were behind it | some literals decide nothing on their own: `[]` has element type `never`, and `{ a: 1 }` has exactly the field it wrote, which is a different *shape* from an interface that also declares an optional one. `lower_module_binding` has always lowered a deferred initializer expecting the global's type, and a call argument at its parameter's; **a local declaration, a `return`, an object literal's property value, an array literal's element and a `push` argument did not**, so the same source compiled at module scope and refused inside a function. One fact, six derivations, and five of them wrong. **Two positions did not refuse — they gave wrong answers, on every binary, for as long as they had existed.** `takes([{ a: 1 }, { a: 2, b: 3 }])` into `Opts[]` built `obj#13` and `obj#14`, one field each, and stored both into an array of a two-field `obj#1`; `takes` read field 1 off an object with no field 1 and answered **2 where node answers 105**. Nothing refused, so no refusal census could see it and every static check called the program fine. It was found by writing the *controls* for the refusing positions and running them — they were the two arms I expected to pass. **The JVM found the sixth.** `xs.push({ a: 1 })` handed `nts_array_push_ref` a pointer it asks nothing about; C and LLVM stored it, and the JVM, which relates classes by name, threw `ClassCastException: nts.gen.Type67 cannot be cast to nts.gen.Opts`. Two backends agreeing is not evidence about something neither of them looks at. `property_parts` is split so the **name finds the slot before the value is lowered**, which is the order the other five wanted too. **All nine slots now do it**, the last three through one extraction rather than three fixes: `coerce_to_slot` computed the type of a `Place` on its way to coercing and did not expose it, so `slot_type` is that match with the coercion removed — asked once before the value exists, to build it right, and once after, to convert what could not be. An array or view **element** is the one arm not shared, because it *narrows* rather than coerces (a `double` into a `uint8_t` slot is the language's modulo, not a C assignment), which is a fact about the conversion and not about the type. Asking the place before lowering the value is also the order the evaluation rule already imposed: `xs[i()] = v()` evaluates the array, then the index, then the value. **It closed a blocker nobody aimed at**: `blockers/type-parameter-optional`, `class Slot<T> { value: T | undefined = undefined }` — the shape `web-platform/src/streams/fifo.ts` is written in — had been filed as a *generics* gap for months and was a field initializer lowered at nothing: `= undefined` takes its type from where it sits, and it was being told nothing, so `T | undefined` had no reference to stand in for. It is now a `compiles` regression guard. `examples/a-value-built-at-the-slot-it-meets` is **522 cases across 18 exports on C, LLVM and the JVM, and under reference counting on C and LLVM**; on the compiler before it, **14 refusals and two exports that compile and disagree with node**. `blockers/a-structural-cast-that-is-not-a-prefix` moved its expectation forward in the same change: `const xs: Named[] = [new Thing(n)]` no longer builds an array to convert, so the element store refuses instead and names the actual cast — `a `Thing` where a `Named` is wanted` — where the array message had two `Managed(Object(TypeId(…)))`. |
| ✅ | an `async` function's `return`, built at the **promise's payload type** | `async function make(n): Promise<Opts> { return { x: n } }` refused with ``an anonymous type` where a `Opts` is wanted`, while the identical `return` in a synchronous function lowered — the literal had simply not been told what it was. A `return` has three destinations wanting three different types: a **callback** hands its value to an iteration's accumulator, whose type the loop decides and the return cannot see; a **plain** return wants the function's own; an **`async`** settle hands it to a promise, and the payload is written on `AsyncResult` for exactly this reason — that field's own comment says it is what decides whether settling emits `nts_promise_fulfill_number`, `_reference` or `_void`. When the plain path was taught to lower at the declared type, the async path was **excluded rather than given its own answer**, which was a guess that the third destination was as unknowable as the callback's, and it was one field away. `examples/an-async-return-at-its-payload-type` is **232 cases across 8 exports on C, LLVM and the JVM and under reference counting**; six refusals on the compiler before it, and four controls — the synchronous twin, a number payload, a `void` payload, and a callback return, which is the destination this deliberately leaves alone. |
| ◐ | an array method on an array of **erased** elements — `(number \| string)[]`, `unknown[]`, `T[]` where nothing pinned `T` | refused, correctly, and **named wrongly**: the guard asks whether the element is a double, a reference or a bool, and anything else was reported as `on a typed array`. An erased element is not a typed array. **27 of the 29 distinct sites in the profile were the erased kind**, counted by splitting the message and re-running rather than by reading them — `ctx.seen.includes(value)` in `util/inspect`, `names.map(…)` in `internal/errors`, `enqueue(value: T)` in `streams/fifo`; two are genuinely typed arrays. A reader sent to look for a typed array in the other twenty-seven finds nothing and has no next step, which is what a message that cannot distinguish its own causes costs. The two now say different things, and the erased one names the work: the `_value` family. `at`, `pop`, `shift`, `concat`, `set_length`, `includes_str` and `index_of_str` have one; `push`, `slice`, `reverse`, `join`, `splice`, `fill`, `index_of` and `includes` do not, and each is a C helper plus a row in the LLVM table and two in the JVM's. ◐ rather than ✗ because the family is half built. |
...
| ✅ | **a member with no storage still enumerates** — a getter, a setter or a method shorthand written in an object literal is an own enumerable property, and `Object.keys`, `Object.values`, `Object.entries`, `Object.hasOwn`, `for...in`, `{ ...src }` and `const { a } = o` all account for it as of 2026-09-21. Was: all seven dropped it silently — `{ get a() { return 1 }, b: 2 }` gave `"b"` where node gives `"a,b"`, and `Object.values(...).length` was 1 against 2. **Wrong answers that compiled**, on every backend, because the zero they produced is what the slot holds. `"a" in src` was right throughout, and that is what said the information was never missing: `in` asks `declares(type, key)` and enumeration asked the *layout*, which holds storage and has no entry for a call. The discriminator is **where the member is written**, not `PropertyRecord::own`, which is `false` for a class's accessor and a literal's alike. A **class** member is on the prototype: excluded from enumeration, and read by `const { d } = inst`, which is a read rather than an own-property question. Carried by `examples/an-accessor-that-enumerates` and `examples/a-spread-that-runs-a-getter`. **Still refused, by name:** a set-only property in `Object.values` (its value is `undefined`, which the element has no width for), and a method shorthand in `Object.values`/`Object.entries` (its value is a function object, and `bound_method` needs a property-access node an enumeration does not have). **Still refused for an unrelated reason:** `Object.values` over a class instance or an interface-typed literal, where the checker's overload gives `any[]` and the `any` policy declines it — `Object.keys` on the same class compiles. An accessor supplied where a declared type wants *storage* — `const o: { readonly a: number } = { get a() {…} }` — is refused too, and used to answer the slot's zero: `blockers/an-accessor-where-the-type-declares-storage` |
...
| ✅ | `instanceof` | against a class this program declares — **including one reached through an `import`** — or one of the seven provided error classes. The set of classes that satisfy it is closed when the program is built, so it is a comparison and not a walk. The import case was refused for as long as this row has existed: a reference to an imported name resolves to a symbol declared at the *import site* and a class's type is filed under the declaration's, so the lookup found nothing and said the compiler had no class for it. **18 sites in `runtime/node`**, `chunk instanceof Buffer` among them. Record 0157 — `examples/open-typed-values`, twenty uses, and `examples/two-classes-one-descriptor` for two classes sharing a descriptor **`instanceof Array` and `instanceof Object` landed 2026-09-20** — `examples/an-instanceof-against-a-builtin`, 203 cases across 7 functions on all five variants, eight refusals on the compiler before it. Neither is a class here and neither can be: an array is a runtime object with a length and no layout, and `Object` is not a declaration at all, so the class search has nothing to find and *"an `instanceof` against something this compiler has no class for"* was true about the representation and useless about the question — the same argument `instanceof_native` already makes for a typed array. **`Array` is answered by `decide_is_array`, the function `Array.isArray` already asks**, rather than a second rule beside it: the existing one is right about two cases a representation test gets wrong — a **tuple** is an `Array` in the language however this compiler lays it out, and a *typed* array is not one although it shares the representation — and it answers an erased operand at run time, which `instanceof` inherits for free. `Object` is the other half and is not a class test: it asks whether a value is an object, and a **string** and a **symbol** are managed values here and *primitives* in the language. That arm looks unreachable and is not — TypeScript rejects a primitive left operand with `TS2358`, so no direct spelling reaches it, but **a type parameter does**, since a generic is specialized per instantiation. `throughATypeParameter` is the arm for it. **Sharing the derivation found a live defect in the other caller**: `Array.isArray(x)` inside a generic answered `false` for every instantiation, because `Substitution` maps a `TypeId` to a *representation* rather than to another `TypeId`, so the checker's type at the argument is still `T` and fell to the catch-all. It now decides from the representation there, asking the snapshot for the one thing a representation cannot say — whether an object is a tuple. That was found by probing `instanceof` against node and had no corpus file reporting it. **`Function` and `RegExp` landed the same day.** A function value here is a closure class — an object with one method — so `f instanceof Function` is a question about the representation, and `f instanceof Object` is `true` for the same value, which is what JavaScript says: one test over one representation rather than two. `RegExp` is the other kind of answer, and it has a precedent: **nothing a compiled program holds can be one**, because a regular expression has no representation here, so the test is not unanswerable — it is answerable and the answer is `false`, exactly as `instanceof_native` already argues for `WeakMap`, `WeakSet` and `WeakRef`. **Guarded on the fact rather than asserting it**: the day a regular expression represents, this falls through to the refusal it has today rather than quietly answering `false` about a real value. Of the 16 files the row ranks, 12 moved on to their next blocker. |
...
| ◐ | block scope, `const`/`let`/`var` — `const` and `let` hold. **The `var` third was marked ✅ while saying it was untested, and it is wrong.** Tested 2026-09-20: after `for (var i = 0; i < 3; i++) {}` at module scope, a *module-scope* read of `i` gives 3 and agrees with node, but **any function reading it gives the global's zero** — `function f() { return i }` and `const g = () => i` both, where node says 3. All three head forms: `for...of` over `[10, 20, 30]` answers 0 where node says 30, and `for...in` answers `undefined` where node says `"b"`. A `var` the loop *body* assigns is correct, which is the control that locates it in the head binding. It is the zero rather than "the loop's first value" as previously recorded — `for (var i = 0; …)` is the one shape where those coincide. A silent wrong answer, not a refusal; the in-function declaration has a blocker and refuses by name, and this one compiles. Pre-existing: a binary built 2026-09-18 answers identically. `declared_in_a_loop_head` in `hir/lower.rs` carries the measurement and what is inferred rather than measured about the cause |
...
| ✅ | a **rest element whose target is a pattern** — `[...[x, y]]`, `[...{ length }]` `examples/destructuring`. **Landed 2026-09-18.** Both are ordinary JavaScript: the tail is built exactly as it is for a name, then destructured again one level down. `bind_rest` searched its children for an `IDENTIFIER`, found neither, and refused as *a rest element with no name* — true of what it looked for and misleading about the program, which names the tail's *parts* rather than the tail. 20 of the 413 files that reach lowering in the slice-1 test262 population, all in the generated `class/dstr` family; 4 of 10 sampled went to `strict-pass` and the rest publish `next` or the private-method blocker. `[...{ length }]` needed a second thing — an object pattern over an **array**, which has no fields — so `length` is answered directly from `OpKind::Length`. Only `length`, and only for an array: an object pattern naming a numeric key (`{ 0: x }`) is a different read and stays refused rather than guessed at. |
| ✅ | an **elision in an array literal** — `[1, , 3]`, `[,]`, `[, , ,]` — `examples/a-hole-in-an-array-literal`, 261 cases across 9 functions on all five variants; nine refusals on the compiler before it. **Landed 2026-09-20.** Not the same production as a **hole in a destructuring pattern** (`const [, second] = pair`), which has worked since 2026-09-17 — the census report spent a paragraph establishing that the corpus's elision coverage was all the latter. A hole reads as `undefined`, so a `number[]` cannot hold one and storing anything there would be a wrong answer; the refusal said exactly that, and what was missing was the room. **`undefined` and `void` both represent as `HirType::Void`, which is right for a function's result and wrong for a slot that is read back.** `in_a_slot` is that distinction: at the two places a representation becomes a slot — an array's element and a tuple's position — a `Void` becomes `Erased`, the one representation with an `undefined` in it. A *numeric* placeholder would not do, which is the difference from `suspend::yielded_slot` one construct over: there nothing can read the slot, and here `method([x = 23] = [,])` reads index 0 to decide whether the default fires, so a `0` would leave `x` at `0` where JavaScript says 23. **The corpus reported this under a message naming something else** — 36 files as *a parameter of unrepresentable type (a tuple)*, the tuple being `[undefined]` — plus 20 more naming the hole itself; **32 of the 36 and 16 of the 20 now pass**. A second defect was behind them: a parameter's **default** was lowered with a bare `lower_expression` while the argument loop three lines above used `lower_expecting` and carried the comment saying why, so `function f(p: number[] = [])` refused with *an array literal that is not an array* — `[]` is typed `never[]`, the checker saying the slot decides, and nothing was telling it what the slot is. **What this is not: a hole is *absent*, not present-and-`undefined`.** `1 in [1, , 3]` is `false`, `Object.keys` gives `["0", "2"]`, and `forEach`, `map` and `join` all skip it — so every one of those would disagree with node. Each of them refuses today, measured rather than assumed, and for its own reason rather than for this one, which is exactly the shape that goes wrong silently later. `tooling/conformance/blockers/a-hole-is-absent-not-undefined` pins those refusals so the person who lands `forEach` on an array of erased elements finds this. A sparse representation is the real answer and is a different data structure from the dense array this compiler has, so the honest scope is: **right for an indexed read, wrong for anything that asks whether the index exists.** **One message was wrong the moment arrays of erased elements existed**: `[...xs]` over one landed on `lower_array_copy`'s catch-all and was reported as *"a copy of a typed array"*, a construct the program does not contain. It names the element now, in the phrasing the array methods already use for it — `forEach`, `map` and `join` all refuse an array of erased elements with the same sentence — so the family reads as one thing. Found by a probe sweep rather than by a corpus row. |
| ◐ | `try`/`catch`/`finally`, a bare `catch { }`, and a `throw` of any type | **within one function, and across a call to a plain function, and through a chain of them, as of 2026-09-23 — record 0343: the callee is compiled a second time as a raising copy that records the value and returns, and the call tests afterwards, so every frame out is left by an ordinary `return` and `rc` releases what it holds. The copy set is a greatest fixpoint — every plain raiser is assumed copyable and any that reaches one that is not is removed — so a method, a constructor, an accessor and anything reaching one keep the refusal; `examples/a-throw-that-stays-in-its-function` holds both refusing arms beside the four that must keep compiling. Previously refused outright as of 2026-09-12 — except that an accessor was not counted as a call until 2026-09-18.** The guard matched `CallExpression` and `NewExpression`, and a getter read is a `PropertyAccessExpression`, so `try { return c.checked } catch { }` compiled and the throw crossed exactly the boundary this refusal exists to name: **6 of 29 cases declined** where node answers `-1`, and a setter did the same under an assignment. Nothing reported it — `emit-c` was silent, and the differential folds a decline into *skipped*, so its last line said `agreed on every case` directly below `6 case(s) the compiled program declined`. It is `examples/accessors`' own opening sentence, *an accessor looks like a property and **is** a call*, being false of a guard's population; that example states the hazard for storage and this was the same mistake about control flow. `reads_an_accessor` answers by the member symbol's declarations, since nothing in the syntax of `c.checked` says which it is, and `blockers/an-accessor-inside-a-try` carries both accessors plus a plain field in the same position as the arm that fails if the guard is widened to every member access. It cost nothing: `addons.sh` 24 of 24 with 0 regressed, 254 of 254 examples still agreeing. A `throw` lowers to a jump to the enclosing handler *block*, which is a branch inside one function; a callee has no edge back to its caller's handler, so a throw raised inside a call abandoned the program where node's `catch` runs. Measured on all three backends: `try { return deep(n) } catch { return -1 }` gave `10 case(s) the compiled program declined` against node's -1, and the JVM emitted no exception table at all. **The row was ✅ and was true of the half it had been tested on** — same-function `throw`/`catch`/`finally` agrees on 58 cases and still does. Refused rather than left wrong, and the node lane priced that against its own axis rather than my guessing: 25 of its 49 compiled-lane files are in modules with no such `try` at all, and **the defect is masked by the boundary** — a module must publish a function before a caught throw inside it is reachable, and `assert` fails 13 of 13 on names that do not exist. So the refusal is cheapest now and grows dearer as publishing improves. Only calls that can **reach a `throw`**, which is a whole-program fixpoint over the call graph, not a test for "is it compiled": the coarse version refused `new ArrayBuffer(bounded(n))` and cost `examples/array-buffer` six tests. Excluded for stated reasons rather than by tuning — a provided error's constructor is emitted inline and a runtime helper aborts rather than throwing, so `try { throw new RangeError(m) } catch { }` keeps working; an `async` callee never raises *synchronously*, so `try { await failing(n) } catch { }` keeps working and `examples/async-catch`'s eight functions still agree; and an unresolved callee — `fns[0]()` — joins the set, because what it reaches is the one thing this refusal exists because the compiler cannot establish. `examples/a-throw-that-stays-in-its-function` holds all four arms and `compiler/core/tests/throw_across_a_call.rs` asserts each by name. Closing it properly is cross-call exceptions: an exception table and a handler block on the JVM, and on C **not** the `NtsLanding` `setjmp` pair that already carries a throw across the napi boundary — its own docstring says why, since a `longjmp` skips the releases the reference-counting provider inserted between the throw and the frame it lands in, so every frame thrown through leaks what it held. Unwinding the counts is the feature, not the jump |
...
| ✅ | a default in an **assignment** pattern — `[a = 7] = xs`, and the `for ([a = 7] of xss)` head that reaches the same code — `examples/a-for-of-that-assigns`, 464 cases across 16 functions on all five variants. **Landed 2026-09-19.** The row above is the *binding* form (`const [a = 7] = xs`), which has always worked; this is the same syntax where the targets are existing variables, and it refused as ``assignment to a computed target``. The element node is a `BinaryExpression` with `=`, so `place_of` was handed the whole of `a = 7` and answered truthfully about a question that should not have been asked. It is refused **outside a loop too**, bracketed against the pre-session binary, so this is not about the head. The value comes from `defaulted_array_element`, the helper the binding path already uses — one derivation of when an element is absent and what happens then, asked by both, rather than a second rule that can drift from it. **A default is an expression and it runs only when the element is absent**, which is what `defaultRunsOnlyWhenAbsent` pins — and that arm found the second half of the loop-carried problem: `a = (flag = 1)` *writes* `flag` on the iterations where the element is missing, and a head's writes are loop-carried. `names_written_by` collects targets and cannot see an assignment nested in an expression; `assigned_symbols` finds assignments and cannot see a bare name. Both are asked, and with only the first this was `NotDominated` — invalid HIR, no output, no diagnostic. **Array form only.** An *object* assignment pattern's default (`{ x: a = 1 } = o`) is refused by name, because there is no length to say whether the element is absent, and a rest is refused as before. **No file moved, and that is the finding**: over the 112-file candidate set the pass count held at 10 while ``assignment to a computed target`` went 31 → 12 — every one of those 19 files published the blocker behind it (a tuple parameter, an array of `any`). A feature can be entirely landed and move no headline, which is why the specific message was counted rather than the total. **The three causes behind the remaining 12 are cleared as of 2026-09-20**, and all three were the binding path's rule learned by the assignment path. A **nested** pattern (`[[a, b]] = xss`, `({ p: [a, b] } = o)`) is the same function one level down. A **rest** element (`[a, ...rest] = xs`) takes its tail from `rest_tail`, extracted so `bind_rest` and this ask one thing — the tail is a *fresh array*, which `theTailIsFresh` pins by writing to it and reading the source. A **hole** (`[, a] = xs`) binds nothing and still occupies a position, and the two facts travel together: skipping without counting writes `xs[0]` into `a`, which `twoHoles` is the arm for. Across the 18 files the corpus ranks under this message, 12 now pass. **Six of the rest were parentheses** — `(x) = 5`, `(x)++`, `for ((v) of xs)`, the grammar's *cover* production, which test262 names `target-cover-id`. `place_of` unwraps them, and there rather than at any one caller: *where a thing writes* is one question and that is the function which answers it — a caller unwrapping for itself would be a second derivation and the next caller would not have it, which is how five files across assignment, both increments and both decrements came to share one refusal. `examples/a-parenthesised-assignment-target` is 290 cases across 10 functions, nine refusals on the compiler before it, and carries a doubly-parenthesised arm because an unwrap that ran once passes every other. What still refuses is a default in an **object** assignment pattern (`{ x: a = 1 } = o`), which has no length to say whether the element is absent, and a shorthand, whose name resolves to the property. |
...
| ✅ | a **destructured** parameter with a whole-parameter default — `function g({ a = 1 }: Opts = {})`, called as `g()` | the options bag, ordinary JavaScript, and it did not build. A default is materialised at the **call** rather than in the callee, and `default_of` located it by finding the parameter's *name* among the children and taking the last child when it was something else — searching for an `IDENTIFIER`. A destructured parameter's name is a **binding pattern**, so the search failed, the parameter was reported as having no default, and the call passed no argument at all. **It produced no refusal.** The arity check is in `verify`, so the program came out as `invalid HIR: CallArgumentCount { expected: 1, found: 0 }` and `emit-c` **exited 0 having emitted nothing** — not a refused function but a silently unbuilt program, which a refusal census cannot see and an exit status cannot either. It needed three things at once — destructured, defaulted, and a call that omits it — and each alone lowered, which is the whole reason it lasted. **Zero sites in `runtime/node`**, and that is the point rather than an excuse: a census ranks what the corpus is blocked on, not what the language is missing, and this was found by writing the shape and *running* it. `examples/a-destructured-parameter-with-a-default` is **232 cases across 8 exports on C, LLVM and the JVM, and under reference counting on C and LLVM**; on the compiler before it the whole file fails to build, so its five controls were each run alone against the old binary — `invalid HIR` emits nothing, so a control inside the file would only have restated the finding. |
| ✅ | an **element default where the array is too short** — `const [a = 9] = []` `examples/an-element-default-over-a-short-array`, 145 cases on all three backends. **Landed 2026-09-18, and it aborted the process before this.** The default is the answer for exactly this case and the read was emitted first, with the default applied to its result — so the checked `ArrayGet` the default exists to avoid ran anyway and the program died with `nts: refused: index 0 is outside [0, 0)`. `defaulted_array_element` puts the read inside the arm where the index is in range, which is the rule `a?.[i]` already follows: the thing a test guards against must not happen before the test. Six files in the test262 corpus are this, every one named `-init-exhausted` — the suite writes the case down separately because it is the one implementations get wrong. **The control is worth reading**: on the previous compiler `nts check` over the new example prints `17 case(s) the compiled program declined`, then `checked 3 of 145 cases`, then `agreed on every case`, and exits 0. An abort folds into *declined*, declined folds into *skipped*, and a fixture comparing 3 of 145 reads exactly like one comparing all of them. **Completed 2026-09-18 for an *erased* element**, which is the common case and needed a second test: an unannotated `[a = 9]` is inferred as `(number | undefined)[]`, so the element can be present *and* `undefined` — `f([undefined])` is 9 exactly as `f([])` is, and test262 writes the two as separate files (`-init-exhausted`, `-init-undef`). The first pass cleared the exhausted family and left `-init-undef` refusing. Two branches, and the second already existed: produce the element as an erased value, `undefined` when the array is short — both arms erased, nothing to reconcile — then hand it to `defaulted_when` and `Branch::Present`, which test an erased value's tag and read it back at the type the branch establishes. `lower_branching_value_at` takes the merge type explicitly, because a branch this compiler synthesises has no node whose type to read. 6 of 16 sampled test262 files went from refused to `strict-pass`; the rest publish their next blocker, `next` or an array elision. An element with **no** default read past the end still aborts, deliberately: the language says `undefined` there and this compiler has none to hand back. |
...
| ✅ | higher-order functions and closures that only *read* what they capture `examples/closures` carries them, under the representation this row depends on: a closure is an object with one method. **An array of them produced by a conditional did not emit until 2026-09-18.** A signature is written on a declaration and never allocated, so nothing constructs one and nothing gives it a layout; `lower_branching_value` knew that and laid one out for a conditional whose own type *is* a signature — testing `Managed(Object(_))` and nothing else. So `cond ? [() => n] : []` went unlaid: both arms build the array at one signature type id and the merge parameter carries another — the checker's for the declaration, the arms' for the literals — and only the arms' had a layout. The C emitter then said `NTS2006 an object type with no layout: type 9`, **a message about a type rather than a line, from the backend, on a program the lowering reported as complete**. `materialize` is the walk that already answers this for an array's element, a map's value and a set's member, and the merge now uses it instead of looking only at the top level; `collect_layouts` merges the two ids by shape once both exist. Found by a ten-line probe, not by the corpus: it clears **0 corpus sites** — the 35 `NTS2006` sites in `runtime/node` are a different sub-cause and `stream` 12, `fs` 30, `util` 2 and `http` 13 are unmoved — so it is offered as correctness and not as reach. `examples/an-array-of-closures-from-a-conditional` is 174 cases across 6 exports on C, LLVM, the JVM and under `rc`; against the commit immediately before it, asserted by `rev-parse` rather than assumed, the same example declines 4 functions with three distinct `NTS2006`s. Not `examples/closure-merge`, which is the other half: two arrows reaching one **variable**, where the hazard is which closure class the merge takes rather than whether it has a layout. **Including one closure capturing another, as of 2026-09-17.** That refused until then, and not at lowering: a capture is typed twice — the enclosing function stores the value and takes the value's own type, the body reads the field back and took the *checker's* type of the name. Those agree for a captured number and not for a captured arrow, whose checker type is its function type and has no layout, so the store said `managed<closure#0>` and the read said `managed<obj#4>` and the C backend answered ``an object type with no layout: type 4``. The two sides' `Field` lists are merged by `collect_layouts`, whose comment calls that merge *"the check that the two sides agree"* — it merged them without noticing, because the field's name and index agreed and only its type did not. The read now asks what the store answers, through `closure_bound_to`: a name bound to a **`const`** arrow has that closure's layout, which is `closure_typed_global`'s soundness argument word for word. `examples/a-closure-capturing-a-closure` is 174 cases across six exports agreeing with node, where the pre-change binary refuses fourteen times; its arms go three deep, capture a closure beside a number, and share one captured closure between two capturing ones. A **reassignable** variable is refused by name — two arrows are two layouts — where it used to reach the backend and be told about `type 4`; `blockers/a-closure-captured-through-a-reassignable-variable` holds it |
| ✅ | a **method used as a value** — `const g = c.twice` — `examples/a-method-used-as-a-value`, 321 cases across 11 functions on all five variants. **Landed 2026-09-19**; it refused as *`twice`, declared by `C` with a type that has no representation (a function type)*, because a method is not storage — it lives in the dispatch table, so the layout has no field for it. **175 files** of the slice-1 `test/language` population, split across two ranked rows that look unrelated, and the smallest program showing it uses neither `private` nor `static`. **A read does not bind the receiver, and that is the whole difficulty**: `c.twice` is the function itself, not `c.twice.bind(c)`, so calling it later has `this === undefined`. Measured against node before anything was built — a method reading `this.k` *throws* where the first version of this answered `30`. So a body that reads `this`, or `super`, or that contains an arrow inheriting `this`, is **refused by name**. The receiver is still carried, because it *selects* the body: `c.twice` where `c` holds a `D` must reach `D`'s override, which is a virtual dispatch and needs the object — sound exactly while the body cannot tell, which is what the refusal guarantees. Three shapes and only one needs an object: a **static** method has no receiver and is the free-function case under another name, one instance for the program; an **instance** method gets one object per read, which is also what the language does since `c.twice !== c.twice`. **Calling the result of a getter lands with it** — `new C().f(n)` where `f` is a getter returning a function, which was refused as *a method `f` with no declaration in the hierarchy*: a true sentence about a question that should not have been asked, since the hierarchy has `get f` and no method of that name. A *field* holding an arrow already worked when called that way, which is what says the machinery was there and only the getter was not reaching it. The two together are the shape the corpus writes — `get method() { return this.#method; }` then `new C().method(…)`. `getterRunsOnce` pins the operation count at one. **The fourth link, and 34 more files: a destructuring parameter.** `get method() { return this.#m; }` gives the read an anonymous function type that nothing else in the program names, and a type reaching HIR without a layout is *invalid HIR* rather than a refusal — `emit-c` prints `refusing to emit code from invalid HIR`, writes nothing and exits 0. It took a destructuring parameter to show it: with a plain parameter the function type is one the program already names. The layout is materialized at the **call**, not at the read, because a read whose closure is refused must not bring the class into existence — doing it in `member_of` built a `call` for a method that had already been declined. **A generator method used as a value is refused by name**: calling a generator produces its *frame*, whose type is synthetic and per-declaration, while the call site sees the abstract `Generator<…>` a wrapping closure is declared with, and the two disagree; without the refusal the emitted C does not compile. **`f.name` landed 2026-09-20.** Two ways a value is a named function here. A **closure class** is one per declaration and final, so `ClosureInfo::node` is the declaration it was made for — that covers a function value read directly. A value whose static type is a **function type** covers the rest, and it reaches further than I first recorded. **I wrote here that it could not**: that the `private-*-method-name` files read `.name` through a getter, so "one function type can hold any function with that signature" and following the value back would be dataflow. That was wrong, and one probe settled it — an *unannotated* getter returning `this.#method` has the private method's **own** function type, whose symbol is declared by the method, so the name is one lookup away. **14 of those 17 files now pass.** What made the claim look true was my own fixture: I wrote the getter with an explicit `(n: number) => number`, which really is an anonymous type with no declaration behind it, and read that refusal as the general case — a fixture differing from the corpus in one incidental way can confirm a limitation that is not there. A genuinely anonymous signature still refuses and should. The three files left are `array.toString !== Array.prototype.toString`, which needs function identity for a prototype method. Private names need `member_name_of` rather than `declared_name`, since the name node is a `PRIVATE_IDENTIFIER`. An **arrow** is not covered: `const beta = () => 1` has `beta.name === "beta"` by `NamedEvaluation` off the *binding*. That is the `private-gen-meth-*` family. **The refusal was removed on 2026-09-20 and nothing was done to this code to remove it** — 40 files, of which 36 now pass. `suspend::yielded_slot` made the abstract `Generator<…>` representable for an uninhabited element, and a frame's layout already had that class as its *base*, so the two are one pointer and `compatible` has always permitted the upcast — at a return as much as at an argument. The refusal was written against a state of the world that a change three commits later removed, and **nothing would have said so: a refusal does not fail when it stops being necessary.** It was found by re-running the census row, not by reading the comment that explained it, which is the general shape — clearing one refusal publishes what stood behind it, and sometimes what stood behind it was another refusal. |
...
| ✅ | a function value chosen by a **branch**, where the other arm is `undefined` or `null` — `const g: ((x: number) => number) | undefined = n > 0 ? (x) => x * 2 : undefined` | the row above is the *property* form and this is the value form, and it refused for a reason that is neither representation nor narrowing: **nothing had laid the signature out**. A function type gets its layout from a declaration — a parameter, a field, a variable's annotation — and something calls `materialize` at each. `lower_absent` takes its type from where the literal *sits* instead, which is the one place a type arrives without a declaration being consulted, so the `undefined` was pushed at a type no layout answered for. Two type ids for one written signature is the ordinary case here — the annotation's and the union member's — and `signature_name` exists to merge them, but only for ids something asked `layout_of` about. `collect_layouts` then merges the two by name. **Signatures only, and that is a totality argument rather than a preference**: `layout_of` answers unconditionally for a function type where for anything else it can refuse, and refusing at the literal would stop a program whose value the later passes go on to eliminate. The C backend was naming the type (`NTS2006 an object type with no layout`) rather than the literal that produced it, which is why this read as a representation gap for as long as it did. `examples/a-function-or-nothing-from-a-branch` is **232 cases across 8 exports on C, LLVM and the JVM, and under reference counting on C and LLVM**; four of its nine exports refuse on the compiler before it and five are controls that lowered on both — the parameter, the field, the `let` built by assignment, and a conditional between two functions with no absent arm. **Corpus effect: zero, and the first number here was wrong.** It said `addons.sh` went 24 of 26 to 25 of 26 with `child_process` building for the first time. It did not. That compared *my working tree's* 25 against *the gate tree's* 24 and attributed the difference to the change — two trees, one bracket. The pre-change compiler run in my tree with the same tsgo also says 25, and the reason it says 25 at all is that `tooling/conformance/loads.sh` loads whatever `.node` files are in `target/node`: `child_process.node` there was four days old, from 2026-09-13, while every module that actually built carried that run's mtime. The build half said `24 of 24 still build, 0 newly building` in the same output. So this change publishes **no module**, and it is justified by the fixture and by what it lets the next thing reach, which is the only justification a lowering fix is owed. |
| ✅ | `parseInt` and `parseFloat` | neither is `Number(s)`, and the difference is that both **stop at the first character their grammar does not admit**: `Number("12abc")` is NaN and both of these answer 12. `parseFloat` is not `strtod` on the whole string either — `parseFloat("0x10")` is 0 where `strtod` reads a hexadecimal float and answers 16, `"inf"` is NaN where `strtod` accepts it, and `"1e"` is 1 because an exponent needs a digit after it. The longest admitted prefix is measured first and `strtod` is handed only that, because rounding a decimal string to the nearest double is what it is for. `examples/parse-int` and `examples/parse-float`  **`Number.parseFloat` was missing until 2026-09-17 and `Number.parseInt` was not.** Each is the same function object as its global in JavaScript and reaches the same helper here, which is deliberate — two spellings reaching two helpers is how they come to disagree — and the `Number` side of that had thirteen of its fourteen members and said `a global member with no definition here` for the fourteenth. The asymmetry was in the *fixtures* first: `examples/parse-int` has carried a `throughNumber` arm since it was written and `examples/parse-float` did not, so a pair of fixtures that do not ask the same questions of each other let the implementations drift. `parse-float` now carries the arm, asked as an **identity against the global** rather than against a literal, because the failure it guards is the two spellings reaching different helpers and a value comparison would pass if both were wrong in the same way. **Only these two are aliased**: `Number.isNaN` and `Number.isFinite` are *not* their globals — the whole reason those spellings exist is what they do to a value that is not a number — and aliasing them would be a silent wrong answer rather than a missing one. |
...
| ◐ | a closure over a `for` loop's own variable, which JavaScript rebinds per iteration | **the common shape landed 2026-09-13; a body that writes the variable, and `var`, still refuse.** It was established on 2026-09-12 that this was refused rather than silently wrong, and the probe for that is still the right one: four closures over `let i` sum to `0+1+2+3 = 6` under per-iteration binding and `4*4 = 16` under one shared binding, and node answers 6. **Copying is exact here rather than an approximation.** The specification copies the binding before each iteration and runs the increment in the copy, so iteration k's binding keeps iteration k's value for ever — and the value `i` holds where the closure is built *is* that value. The old rule reached the opposite conclusion by asking whether the name was written **anywhere**, and a counter is written by its own `i++` in every loop ever written, so it refused every loop to catch the rare one. It now asks whether the loop's **body** writes it. `examples/a-closure-over-a-loop-variable` is 116 cases across four functions on C and LLVM, against 29 across one before. **The arm that earns it is `deferredOne`**, which calls its closure after the loop: value capture gives 0 and one shared cell gives 3, where every immediately-called arm agrees under either. Without it the example would pass under both implementations. What still refuses: a body that writes the variable — `i += 10` after the capture lands in the binding the closure holds, so node answers 10 and a copy answers 0 — and **`var`**, which has one binding for the whole loop. That second one is the row's own earlier sentence becoming load-bearing: this refusal used to cover `var` by not asking which keyword wrote it, recorded then as an over-refusal costing nothing with seven ambient `var`s and none in a loop beside it. It was the only thing between `var` and a wrong answer once `let` was narrowed, and nothing said so. A shared cell is probably right for `var` and is unmeasured, which with zero sites is not a reason to stop refusing it. `blockers/a-closure-over-a-loop-variable` and `blockers/a-closure-over-a-loop-var` hold the two, each with the same program one keyword or one statement different **At module scope it was answering from a global nothing writes, and fixed on 2026-09-20** — `examples/a-closure-over-a-module-scope-loop-head`, 174 cases across 6 functions on all five variants, and the pre-session binary **aborts 17 times** on it. `collect_module_scope` walks every `VARIABLE_DECLARATION` outside a function and gives it a global; a `for (let i = …)` head is one of those nodes and is not a module binding. The global was never written — `lower_for` binds the head variable as a **block parameter**, whatever the keyword — while a closure in the body took `reached_by_name`'s shortcut (a name with a global needs no capture) and emitted `global.get` against it. So `for (let i = 0; i < 3; i++) { … () => i … }` answered **0** where node answers 3, and the `for...of` form **segfaulted**, reading a null managed pointer. The identical loop *inside a function* was always right, because there the collector never looked and the ordinary capture path ran: **two derivations of where `i` lives, agreeing everywhere except the one place one of them had no business answering.** A wrong answer that runs — `nts check` cannot see it (its harness drives exported functions and this is a statement) and neither can the census (it ranks refusals, and this compiled and returned a number); a probe sweep against node found it. `declared_in_a_loop_head` is asked by **both** `collect_module_scope` and `is_module_scope`, because closures are collected before the module scope is and cannot consult its result — one syntactic rule, since two would disagree again. **`var` is *not* skipped, and that was learned the expensive way.** For one commit it was, on the reasoning that `lower_for` binds the head variable as a block parameter whatever the keyword so the global was unwritten either way. **A full census run said otherwise**: three files reading the head's `var` *after* the loop — `for (var i = 0; i < 10; i++) {}` then `if (i !== 10) throw` — passed before and refused after, so the global was being written and the claim was false. The gate could not have caught it: no example reads a `for` head's `var` after the loop, and it took comparing 4,812 files against the same run a few commits earlier. `var` hoists, so its head declaration really is a module binding. What stays unfixed is a closure over a `for` loop's `var` at module scope, which answers the loop's first value rather than its last — the status quo rather than a regression. **A `const` declared inside a module-scope loop *body* still refuses** (``i`, a name from an enclosing scope``), with no closure in it at all: that declaration gets a global too and its initializer is deferred to the top of `module#init`, where the loop's `i` does not exist. It is the same defect one level out — a declaration nested in a **block** at module scope is not a module binding either — and it moved from a wrong answer to a refusal here rather than being fixed. |
...
| ✅ | generators (`function*`, `yield`) | the `async` state machine with a different protocol: the element goes in the frame and the suspension is an ordinary `return`, because what resumes it is the caller standing there rather than the event loop. There is no `Generator<T>` object — the **frame is the iterator** `examples/generators` carries them, walked by a `for...of` — and **`g.next()` on a generator held rather than walked, as of 2026-09-19**. `a-generator-method` had recorded what was missing: the resumption answers whether the generator finished and leaves the element in the frame's `yielded` slot, *"exactly the two halves of an `IteratorResult` — so what is missing is not the step but the object"*. The object arrived the same day, when `IteratorResult<T>` became a provided layout, and this is the step it was waiting for. Two things the HIR had to be **read** for rather than reasoned about: the resumption answers `done`, not "produced a value" — inverting it made every arm answer the opposite — and it is named two ways, directly for a frame made by a call here and through a slot otherwise, which `generator_walk` already decides; asking again wanted a *layout* for a synthetic frame type the snapshot has never heard of. `next(v)` sending a value into a suspended `yield` is refused by name, its resumption having no parameter for it. `examples/generator-unsupported` carries what a `function*` is still refused for — **now four**, the fourth being a `for...of` over a generator with **no `yield` in it**. Its element type is `never`, the walk reads the frame's `yielded` slot inside a loop body the resumption can never enter, and a value of type `never` reaching code generation is *invalid HIR*: no output and no diagnostic naming anything. Pre-existing — the same program fails identically on a compiler from before any of this — and now refused by name. **Moved to `begin_generator`**, which already refused `Generator<void, …>` for the same reason and was missing the `never` spelling that a `function*` with no `yield` statement infers; refusing there rather than at a reader is what makes the sentence about the generator instead of about a loop. A **`next()`** on one is refused separately and deliberately: that is a different function, and `generator_element` answers from the type argument rather than from whether the generator was accepted, so the call built the read anyway and stored a `never` — `StoreType { expected: Int { bits: 32 }, found: Never }`. Two refusals naming one fact beat one refusal at whichever reader ran first, which is what the first version did and it named a `for...of` at a program that had none. **125 of the 135 slice-1 files at *a value of type `never` reached code generation*** are this; the row went from 135 to 0. Giving the slot a width was tried and is wrong: the field `suspend.rs` builds is typed from the generator's `yield`s, so a different answer disagrees with it. **A generator that yields nothing lowers as of 2026-09-20** — `function* g() {}`, whose element the checker types `never`, and `Generator<void, …>`, which is driven entirely by what the caller passes to `next(v)`. Both were refused by name, and that was **176 files** of the slice-1 `test/language` population, of which **125 now pass**; the remaining 51 publish the blocker behind them, 40 of them a generator method used as a value. Neither spelling can ever fill the frame's `yielded` slot, and a layout has a fixed shape, so the slot exists and needs a width. **The reason the first attempt failed is that the element has four independent derivations** — whether `Generator<T>` represents at all, the concrete frame's slot, the read out of the frame, and the abstract `Generator<…>` a frame extends — and a placeholder applied to fewer than all of them is a store and a load that disagree: two of them gave `BrokenBase { layout: "silent#frame", base: "Generator0" }` and one gave `StoreType { expected: Int { bits: 32 }, found: Float { bits: 64 } }`. `suspend::yielded_slot` is now the one rule and all four ask it, together with a fifth caller — the `IteratorResult` layout, whose `value` slot receives what the frame's slot holds, since `generator_next` copies one into the other. Sound because nothing can read it: `refuse_unguarded_iterator_value` permits a `.value` read only where the checker has ruled the finished arm out, and for an uninhabited element that narrowing gives a value of type `never`. `examples/generators` carries four arms — an empty walk, `next()` answering done twice, **a body that still runs** (a generator that yields nothing is not one that does nothing), and a generator method with an empty body, which is the shape the corpus writes 156 times — plus the short `Generator<never>` spelling, which moved out of `examples/generator-unsupported` along with two of its arms. That example still refuses `yield;` **with no operand**, which was the same refusal and is now a separate and narrower one. |
...
| ✅ | a base class's **field initialisers run through `super()`** | they did not, and it is the worst-shaped defect of 2026-09-17: `class Base { tag = 1 }` with `class Derived extends Base { constructor() { super() } }` answered **0** where node answers **1**, on every backend, silently. `lower_super` resolves which class above declares the constructor to run, found that none does, and the comment there concluded there was nothing to run — true of a base with only *methods* and false of every base whose fields have defaults. **Two things hid it.** A derived class with no constructor of its own gets a synthesised one that walks the whole chain, so `class Derived extends Base {}` was right; and giving the base an explicit constructor makes the lookup succeed, so the obvious minimal reproduction does not reproduce. It needs a base with only initialisers *and* a derived class that writes a constructor. Every ancestor is initialised now, **outermost first** — the order the constructors would have run in, which is what lets a derived initialiser read what a base one stored — and reaching that arm means the whole chain above is constructorless, because `Hierarchy::constructor` walks up. **Found by a control**: it was the arm of `examples/an-error-subclass-that-carries-a-field` written to pass, for the *other* fix in the same commit. |
| ✅ | `static { … }` in a class body | it lowered to **nothing at all** — not refused, *ignored* — so `class C { static a = 0; static { C.a = 5 } }` answered 0 where node answers 5, on every backend and with no diagnostic anywhere. On the compiler before this, `examples/a-static-block` produces **zero refusals and seven exports that disagree with node**, which is the clearest statement of the difference: a refusal census cannot see a construct that is skipped, and `nts check` says "agreed on every case" for every program that does not read the value back. **The frontend knew the node all along** — `syntax::name_of(176)` is `class static block declaration` — and only the lowering had never been told; `super::unaccounted`, the conservation law for a construct that is silently absent rather than refused, does not catch this one because a static block declares no symbol for it to miss. It runs in the **same loop** as the static field initialisers, because the specification runs both in one source order: `class C { static a = 1; static { C.a += 1 } static b = C.a }` gives `b` what the block produced, and two loops would be two orders. `fieldBlockField` is the arm whose answer depends on the interleaving rather than on the block running at all; `twoBlocksInOrder` distinguishes `1 * 2 + 3` from `(1 + 3) * 2`. **232 cases across 8 exports on C, LLVM and the JVM, and under reference counting.** Found by running ordinary class shapes against node rather than by reading refusals — the same sweep that had already listed static blocks as *compiling*, which they did. | **Completed 2026-09-18 for a class whose *only* static member is a block.** `runs_a_static_initializer` decides whether a class joins the ordered module-statement list, and asked only for a static **property** with an initializer — so `class C { static { seen = 1; } }` answered false, never reached `lower_static_fields`, and its block did not run: `let seen = 0;` stayed 0 where node says 1, silently and on every backend. Every arm of `examples/a-static-block` had a static field beside its block, which is exactly what hid it — the field put the class in the list and the block came along. The predicate stays narrow the other way: a class with no static member must **not** join, or a file with no module evaluation gains an empty `module#init` and one more export, which `examples/delete` once did. `noStatics` is that control.
| ✅ | an **empty** array literal whose annotation names a function type — `const fs: (() => number)[] = []`, filled by `push`, then called | it refused with `NTS2006 an object type with no layout`, **at the call rather than at the declaration**, which is why it read as a representation problem. It is not: `lower_empty_array` takes its type from the annotation and `[]` has no elements to lay out, so the element type was written down and walked by nothing. A non-empty literal works because its elements are materialised on the way in, and the same array as a **parameter** works because parameters are materialised too — an empty literal is the one position with neither. One `materialize` call, which walks containers, so an array of arrays of signatures is reached as well. It is how a program collects listeners, and it is what `for (let i …) { fs.push(() => i) }` needs to be assertable: `examples/a-closure-over-a-loop-variable` gains two arms where the closures **outlive the loop** rather than being called inside it, which is the only shape that can tell per-iteration capture from a shared cell *and* keep the closures anywhere. `blockers/an-empty-array-of-closures` turns its guard from `lacks-c` to `emits-c` rather than being deleted — its own text said that when this landed the function would appear and the guard would fail, and it did. **Corpus effect: none, measured.** That blocker's 61 occurrences across 36 locations are unchanged at 475 and 35 after the fix: they are a different shape wearing the same diagnostic — `once<A extends unknown[]>(fn: (...args: A) => void)`, a generic over a rest tuple, which refuses as ``A`, captured above its own declaration`. |
| ✅ | a **parameter property** on a class descending from a provided one — `class MyError extends Error { constructor(public code: number) }` | the field is declared in the constructor's parameter list rather than in the class body, and that one level was the whole gap. A class descending from a provided one takes that class's fields rather than the checker's flattened view — `stack?` and `cause?` are in the flattened list and are not fields here — and keeps only what it declares *itself*; `own_member_names` built that set by walking the class's children for a property or method declaration, which a parameter property is not. So the field was dropped and reading it refused as ``a type that has no representation``, **for a `number`**: a sentence about the type, for a field the compiler had removed, which is the failure this ledger already records for a function-typed property. Only a parameter carrying `public`, `private`, `protected` or `readonly` declares anything — a plain one must keep declaring nothing, and `plainParameterIsNotAField` is that control. The first fix repeated the mistake it was fixing: the constructor's parameters hang off a `List`, so walking its children directly found the list and not the parameters, one level shallow again. `examples/an-error-subclass-that-carries-a-field` is **348 cases across 12 exports on C, LLVM and the JVM, and under reference counting**; on the compiler before it, seven refuse and **four compile and disagree with node**. |
| ✅ | methods, `get`/`set` accessors, `static` methods `examples/accessors` carries them, and states the thing that makes them worth a row: an accessor looks like a property and **is** a call. **A getter with no return annotation was broken until 2026-09-18**, on a class and in an object literal alike, for as long as getters had existed. `get doubled(): number` worked; `get doubled()` — the ordinary spelling, since TypeScript infers the property type from the body — lowered as `-> void` with its `return` dropped, and the call site read the property type and assigned from a `void` call. `emit-c` reported `nothing refused` and wrote C that clang rejects with `use of undeclared identifier`. The cause is that **a getter's node type is the type of the property it defines, not a function type**, so `declared_return`'s signature branch could never match one and the branch under it reads the *written* annotation — which is exactly why annotating it worked and inferring it did not, and why this row and `examples/accessors` both looked settled: every getter in that example is annotated. Two derivations of one fact disagreeing, with nothing between them. 984 workspace tests passed over it. `examples/a-getter-with-no-return-annotation` pins both spellings side by side — 232 cases across 8 exports, agreeing on C, LLVM, the JVM and under `rc`, and **14 clang errors on the binary one commit earlier**, which is the arm that says the example measures something. A getter whose property type has no representation is now refused as `a getter returning …` rather than silently made void. |
...
| ✗ | an object-literal **method shorthand that captures** — `{ read() { return base + 1 } }` where `base` is a local | the row above landed the method; this is the one thing it cannot do. **The other two spellings work**: `{ read: () => base + 1 }` and `{ read: function () { … } }` both lower, so this is one syntax of three rather than "closures in object literals". **33 distinct sites**, measured 2026-09-18, and they are one idiom — an object literal standing in for an interface whose methods read the locals of the function that built it: `pull(controller) { … position … }` in `web-platform/streams/readable.ts`, `read(maxBytes) { return reader.some(maxBytes) }` in `http1/transport.ts`. It is the largest cluster of `a name from an enclosing scope` and fourteenth overall by distinct site. **Why it is not small.** A method has no environment and a closure carries one, so a capturing method must become *storage* — and storage is a property of the **type** while capture is a property of the **literal**. One layout serves every literal of a type, and two literals of one type may differ in whether their methods capture, so the question is "does *any* literal of this type capture", which is whole-program. `blockers/an-object-literal-method-that-captures` carries the three routes and what each costs, with the per-type pass argued as the confined one. |
| ✗ | **two** object literals built at one *named* interface, each declaring its method | a member's function is named after the type the literal is built at — deliberately, since that is what makes a call through the interface find it — so both are `T2#twice`. **It used to be invalid HIR**, which is the worst way to fail: the second function was pushed anyway, `verify` reported `DuplicateFunction`, and `emit-c` printed *refusing to emit code from invalid HIR*, wrote nothing and **exited 0** — with the only visible diagnostic being the cascade ``f` cannot be compiled because it calls `T2#twice`, which was refused above`, and nothing above it, because nothing was. A false sentence and a silently unbuilt program. It refuses by name as of 2026-09-18. **Closing it is dispatch and nothing less**: two literals at one interface are two implementations of it and the call site has only the interface, so each literal wants its own class implementing `T2` and a virtual slot. The **anonymous** case was solved on 2026-09-17 by the opposite move — register every anonymous object type separately and decline `__object` as a nominal identity — and that answer is unavailable here, because `T2` *is* a nominal name and both literals genuinely have it. `blockers/two-literals-at-one-interface` carries it, with `oneLiteral` as the control that must stay a static call. |
...
| ✅ | `try`/`catch`/`finally`, `for…in` and labelled statements **at module scope** `examples/module-scope-control-flow`, three cases on all three backends. **Landed 2026-09-18.** All three lowered inside a function body from the beginning — `lower_try`, `lower_for_of` with `Over::Keys`, `lower_labeled` — and all three were refused one scope out, because `is_module_statement` is an allow-list and they were not on it. Nothing decided that: the list was three entries short of its own lowering, and the refusal read as though the construct were unsupported (`a \`for...in\` statement, which has code in it`). The allow-list shape is right — a kind on neither list is refused rather than silently skipped, which is the defect it was written to fix, when `total = bump(41)` at module scope was dropped and the program ran as though the line were not there — it just has to be complete. Its doc comment had also come adrift: `runs_a_static_initializer` was inserted between the comment and the function it describes, so rustdoc put a paragraph about module statements at the top of a predicate about static fields. |
| ✅ | a **`switch` at module scope** whose clauses assign a module-scope name `examples/a-module-scope-switch`, 33 cases on all three backends. **Landed 2026-09-18, and it panicked the compiler** — `no entry found for key`, from `carried_now` indexing the binding table for a name that is a *global*. `lower_switch` gives every assigned name a block parameter per clause; a global needs none, because it is memory read with `GlobalGet` and written with `GlobalSet`, and `begin_loop` has applied exactly that rule to a loop's carried set since a loop assigning a global was fixed. A symbol that is neither bound nor a global is now refused by name rather than indexed. **A panic is worse than a refusal in a way worth naming**: `emit-c` died, so there was no diagnostic, no artifact and nothing identifying the construct — the probe that found it reported `C DID NOT COMPILE`, because a missing output directory and bad C fail `cc` identically. The cause was a comment: `assigned_symbols` claimed "a global is skipped here", it never was, and `lower_switch` was written trusting it. |
| ✅ | a **labelled block** at module scope, left with `break label` `examples/module-scope-control-flow`. **Landed 2026-09-18, and the commit earlier that day which allowed labelled statements at module scope published a panic behind the refusal it cleared** — for a few hours `block: { g = 1; break block; }` at module scope died with `no entry found for key` where it had previously been refused. Same cause as the `switch`, which panicked *before* the session: `carried_now` indexing the binding table for a name that is a global. Three constructs carry names across their arms — a loop, a `switch`, a labelled block — each collecting them with `assigned_symbols`, and **only the loop filtered globals out**. `carried_locals` is now the one place that decides it, and it says why the filter cannot live in `assigned_symbols` itself: a `for (let i = 0; …)` head at module scope is collected as a module binding, so filtering early stopped `i` being carried and the loop never advanced. Both panics were found by running module-scope statements against node rather than by reading the code. |
| ✅ | a label on **any** statement — `lbl: n = 1;`, `lbl: if (…) {…}`, `lbl: try {…}`, and a labelled `for…in` `examples/module-scope-control-flow`. **Landed 2026-09-18.** The specification puts a label on any statement and `break lbl` leaves it; `lower_labeled` had an allow-list of five kinds and refused the rest as *a label on something that is not a loop* — 14 files in the slice-1 `test/language` population, most in `asi` and `statementList` where a label on an expression statement is the subject of the test. 11 of 14 went to `strict-pass`. Only `continue` needs a loop, and the shape a non-loop label gets has no latch, so the checker's rejection of `continue` to such a label is backed by there being nothing to continue to. **`for…in` was missing from the list too**, so a labelled `for…in` refused while the same loop written `for…of` compiled — `lower_for_of` handles both through its `Over::Keys` arm and only the list did not say so. Found by widening the conformance population from `test/language/expressions` to `test/language`, which is what the shape's 14 files are: none of them is an expression. |
...
| ✅ | a module-scope `var` **declared more than once**, each declaration assigning at the position it is written `examples/module-scope-redeclaration`, five cases on all three backends. **Landed 2026-09-18, and it was wrong in two independent ways.** `collect_module_scope` keyed on the *symbol*, so each declaration pushed its own global and `scope.variables` — which every read and write resolves through — named only the last, leaving the earlier storage written and unread: `var c = 7; var c: number;` answered 0 where node says 7. And a declaration whose initializer folded to a constant emitted no code, on the grounds that the value was already in `Global.initial` — which is written once, before any statement runs, so `var a = 5; a = 99; var a = 5;` left `a` at 99. Silently, on every backend, with no diagnostic. The fix is one storage location per symbol, a redeclaration deferred even when its value folds, and `lower_module_binding` taking the expression from the declaration it is lowering rather than from `deferred`, which holds one node per symbol and so wrote the last declaration's value at every position. **The function form was always right** — a local written twice has no `initial` to fold into — which is why 58 differential cases over it never saw this. Found by running a test262 file about `+` (`language/expressions/addition/S11.6.1_A2.4_T1.js`) that redeclares its subject partway through. |
| ✅ | **a module-scope global read where the checker narrowed it** — `let x: number | undefined = undefined; x ??= 5; const n: number = x;` `examples/a-narrowed-module-scope-global`, 203 cases across 7 functions on all five variants. **Landed 2026-09-19**; it refused with *an erased value where a concrete representation is wanted*. `number | undefined` has no spare bit, so it is stored **erased** — a tag beside a payload — and reading one as a `number` needs the unerase that `narrowed` inserts. Three branches read a name and only two called it: the local binding at the top of `lower_identifier` narrowed, the global *member* branch narrowed, and the plain module-scope global returned the slot's declared representation whatever the checker had established. The same three lines were an ordinary local one scope in — the same shape as the row below, which is that question about a global's *type* where this is about its *value*. **A sweep over the obvious spelling would have missed it**: `string | undefined` is a nullable pointer rather than an erased pair, so it has nothing to unerase and lowered throughout; only `number | undefined`, `boolean | undefined` and `number | null` reach the refusal. `keepsZero` and `keepsFalse` are in the fixture so that a narrowing fix cannot quietly turn `??=` into `||=`. |
| ✅ | a module-scope declaration with **no annotation and no initializer**, whose type the assignments settle — `var x; x = -1; x += -1;` `examples/an-evolving-type-at-module-scope`. **Landed 2026-09-18.** `lower_variable_statement` reads an evolving type back with `evolved_type` and always has; `collect_module_scope` asked `type_of` alone, so the same three lines were an ordinary local inside a function and refused one scope out. Two derivations of one question, completed in only one of them. It is 36 of the 413 files that reach lowering in the slice-1 test262 population, every one a compound assignment, and 12 of 12 sampled went from refused to `strict-pass`. **It ranked twice and under a name that was not its own**: the read that follows the missing global refuses again as `reading a name before it is bound`, which the census report had attributed to the temporal dead zone — `NTS1004` is the dead-zone diagnostic, it is about cross-module evaluation order, and it cannot fire on a single-file program. Note the reads must stay **in the flow**: an evolving type read from inside a function is `TS7005` on any compiler, so the fixture settles each value into an annotated constant at module scope, which is also what the test262 files do. |
| ✅ | **destructuring at module scope** — `const [a, b] = pair`, `const { x, y } = point` `examples/module-scope-destructuring`, 261 cases on all three backends. **Landed 2026-09-18.** `collect_module_scope` declared no storage for a pattern's names — deliberately, its comment said, because "inventing one name for several is worse than declaring none" — and the read that followed then found no binding either, so the refusal was `a name from an enclosing scope`, which reads as a scoping limitation and is the *second* failure. The same declaration inside a function has always worked. 37 files of the slice-1 `test/language` population are this, every one a `const`/`let`/`var` `dstr` test and **not one of them under `expressions`**, which is the directory the lane had measured four times; 10 of 20 sampled went to `strict-pass`. The reads are `bind_pattern`'s, unchanged — defaults, renames, nested patterns, holes and rest elements all come free — and `pattern_names` is the single walk both halves ask, because a collector and a binder disagreeing about which names exist leaves a global nothing writes. The binding is **dropped after the store**: it is a temporary of the destructuring and the global is the storage, and left in place it shadowed the global for the rest of `module#init` — a later `a = 100` wrote the binding while every function reading `a` read the global, which is why the fixture reads every name through an exported function. |
| ✅ | a function-scope `var` **declared after it is assigned** — `v = 2; var v: number;` `examples/a-var-declared-after-it-is-assigned`, 145 cases on all three backends. **Landed 2026-09-18, the same day and the same mistake one scope in.** A `var` binding belongs to the function, not to the statement that spells it, so a declaration with no initializer evaluates nothing — the storage was already there. This bound a placeholder zero over the earlier write and answered 0 where node answers 2, silently and on every backend. `let` and `const` keep the placeholder and must: their binding *begins* at the declaration, a read before it is "used before being assigned", and the checker rejects that program — so there is no earlier value to preserve. Found by a probe sweep against node, not by a census: the shape emits no diagnostic, so nothing that ranks refusals could see it. |
| ✅ | the sign of a zero produced by **underflow** — `1 / (-0.1 * Number.MIN_VALUE)` is `-Infinity` `examples/negative-zero`, three folded arms and one runtime control. **Landed 2026-09-18.** Neither operand is zero: the product is too small to represent and IEEE keeps the sign. Every clause of `facts::mul`'s negative-zero rule asked whether an *operand* was or might be zero, so none fired, and `Facts::new` spells a `-0` bound as `0` deliberately — the two compare equal and an interval means the same either way — so the sign had nowhere left to live. `fold` then saw an ordinary singleton zero and wrote `0.0` into the program. `fold`'s guard was already correct and was being handed a fact that had lost the distinction, which is why the fix is in the fact. One of exactly **four** files in the 2,527-file slice-1 test262 population that compiled, ran, and answered wrongly. The first version of the fixture took the operand as a parameter and **agreed on the broken compiler**: a folding defect needs a foldable fixture, and a runtime operand never reaches the folder. **And the sign lost through an erased slot, fixed 2026-09-19.** `sign(-0)` where `sign` takes an `unknown` answered `+Infinity`: the erased parameter and the whole chain behind it narrowed to `i32`. Two readers were short. `zero_sign::observed` listed a call argument and stopped, so the *erase* was observed and the `neg` behind it was not — the lenient test applied to the one operation that makes a `-0` out of a `+0`; an `Erase` now observes what it erases, because an erased value is on its way somewhere this function cannot see the shape of. And `width_of`'s arms for `+ - * %` and `Neg` asked whether the **operands** were integral and never asked about the result, though `0 * -5` is `-0` from two operands as integral as values get; both now ask `provable(id)` too, which routes through the same `observed` set and so is lenient exactly where it was — `arrays`, `strings` and `bitwise` specialise the same number of values as before, counted. **Found through the test262 census and only after its harness stopped comparing with `!==`**: `assert.sameValue(x, -0)` was passing for `+0`, so three `test/language` files had been reported as passes. The new arms take constants, because a *parameter* is never narrowed by this pass and the first version of them agreed on the unfixed compiler. |
...
| ✅ | `enum` — numeric members, explicit and implicit, negative and fractional | the checker has already done the arithmetic and gives the member access a *literal* type, so `Colour.Red` is an **immediate**. There is no object: `tsc` emits a table per enum and reads a property per use, and the emitted C here is byte-identical to writing the numbers. The old note said `Colour.Red` resolves `Colour`, which is a type and not a value — true, and not the obstacle: the enum is not used as a value, the member is `examples/enums` carries them, under the claim that decides their representation: an enum is a set of named constants and nothing at run time. **Across a module edge this was false until 2026-09-23, and all four enum rows below rested on it.** Every enum example in this repository was a single file, so each of them folded a member whose declaration was in the same module; `import { Align } from "./kinds.ts"; Align.Center` refused. An import names an **alias** at the use site and an alias carries no `ENUM` flag, so both enum paths -- `enum_member` and `enum_reverse_member` -- failed their flag test on the unresolved symbol and fell through to `describe_name`, which *does* resolve the alias, sees a genuine enum, and reported ``Align` as an object rather than a member, which is the reverse mapping`. That sentence was wrong twice over: the failing sites are plain member reads, and the reverse mapping was the half that already worked. The rule is stated at `lower.rs:1549` -- *"Imports name their alias at the use site. Classify the declaration it denotes"* -- and of the four sites testing `SymbolFlags::ENUM`, two followed it and the two enum paths did not. `examples/an-enum-across-a-module`, **41 cases across nine exports**, eight refusals on the pre-change binary |
| ✅ | `const enum` | the same substitution, which is what TypeScript's own erasure of one does. Refused across a module edge until 2026-09-23 with the rest -- and a `const enum` is the form where that could not be worked around, since it has no object at run time to load from even in principle. A plain `enum` differs only in also emitting the reverse-mapping object, which nothing compiled here reads `examples/enums` carries the enum forms together; a `const enum` differs from the others only in having no table left at run time, which is the same claim the plain row makes about all of them. |
| ✅ | a **string** enum member (`Label.Short`) | a constant too, and a *managed* one: it takes the interned static a string literal gets rather than an immediate. The checker gives the member access the same `Literal(String("s"))` type it gives the literal, so the two do not merely agree — they **share one static**, verified in the emitted C. Empty members, `const enum` members, and a member defined as another all fold the same way `examples/string-enum` carries it, on the point that makes it worth separating from the numeric case: a string member is a constant like a numeric one and a different thing to store. Refused across a module edge until 2026-09-23; `stringMember` in `examples/an-enum-across-a-module` is the arm. |
| ◐ | the **reverse mapping** at a constant index is answered as of 2026-09-13; at a computed one it is not | a numeric enum emits a table alongside its members mapping each value back to the member's name, so `Colour[1]` is `"Green"`, and at a constant index that is a string the compiler already holds — folding it is exact. **At a computed index it is not**: `Colour[n]` for an `n` no member has answers `undefined` in JavaScript while TypeScript, under this project's settings, types the whole expression `string` — so a lookup producing the declared type would be wrong precisely where the program is asking a question it cannot answer statically. Closing it needs the answer to carry absence, which is the `string \| undefined` representation the optional-property work settled for *fields* and nothing has settled for this. The same split as `Object.hasOwn` against `Object.keys` and `Array.from` over an iterable against over an array-like: **the form that names its key is answerable and the form that computes one is a different feature.** The numbering is TypeScript's rather than the position — a member with an initializer takes it and one without takes the previous plus one, so `enum E { A, B = 5, C }` is 0, 5, 6 — and the lowering carries a running total; `withAGap` is the arm that fails if the rule is read as "position". A **string** enum has no reverse map at all and that is the specification's rule, so the lowering tests each member's folded value rather than the enum's declaration. `examples/an-enums-reverse-mapping`, **145 cases across five exports** on C, LLVM and the JVM, with `examples/enum-reverse-map-unsupported` holding both refused forms. **Zero corpus demand, measured**: `runtime/node` declares **no enums at all**, 0 across every module against a control matching 2. A specification row closed with no axis movement. **And it was closed for one module only**, which nothing here detected until 2026-09-23: `enum_reverse_member` recovers the member values by walking the symbol's `declarations`, and an unresolved import alias declares one import specifier with no `ENUM_MEMBER` child -- so `Align[6]` across an edge refused with *at a value no member has* for a value a member has. `reverseAtConstant` and `withAGap` in `examples/an-enum-across-a-module` are the arms, the second of them at the member carrying the explicit initializer. The zero-corpus-demand measurement stands and is worth reading twice: this fix clears nothing in `runtime/node`, which declares no enums at all. Its demand is **generated bindings** -- `nts bind-gir` writes one enum per GObject type into its own module, so every one of them is read across an edge by construction |
...
| keyed | `Map` ◐, `Set` ◐ — empty construction, `get`, `set`, `has`, `delete`, `size`, `add`, `clear` | **iteration over either** — `keys`, `values`, `entries`, `forEach`, `for...of`, and construction from an iterable of entries, which needs the same protocol; `WeakMap`, `WeakSet` | |
| structured | `ArrayBuffer` ◐ — length, maximum, `resizable`, `detached`, `slice`, `resize`, `transfer`, `transferToFixedLength`. `DataView` ◐ — construction over a buffer, tracking and fixed windows, `buffer`, `byteOffset`, `byteLength`, and every `get`/`set` — the eight numeric widths with explicit endianness, and the `BigInt64`/`BigUint64` pair | `JSON`, `Atomics`, `SharedArrayBuffer` | **A fixed-length view over a *resizable* buffer reported a stale length until 2026-09-18, on every backend.** `nts_view_length` computed the tracking case from the buffer — under a comment saying a stored length is *"right until the first resize and silently wrong afterwards"* — and returned the stored one from the branch immediately above it. So `tracks.length` was right and `tracks.subarray(0, 2).length` was not: node answers **0** for a view whose extent no longer fits, and it is a **test rather than a latch** — grow the buffer back and the view is in bounds again, measured as `cut.length` going 2, 0, 2 across `resize(0)` and `resize(8)`. Element reads follow for free, since they already bounds-check against this. `nts_dataview_byte_length` had the same two-branch shape and needed a **different** answer, also measured rather than assumed: node **throws a `TypeError`** for an out-of-bounds `DataView` where it answers 0 for a typed array. This runtime has no throw that reaches a handler, so it declines by name instead of inventing a number — returning 0 would disagree with node just as surely and would do it silently. The rule is implemented twice, `runtime/c` (which serves C **and** LLVM — only C was reported) and `runtime/jvm`, with the same two-branch shape in both; the JVM lane landed their half the same day and `examples/a-fixed-view-over-a-resizable-buffer` is 116 cases across 5 exports agreeing on all three backends and under `rc`. Its `offsetCut` arm is the one that earns it: at 3 bytes a view at offset 2 is out of bounds where one at offset 0 is not, so a fix testing `length` and forgetting `byteOffset` fails there and passes everywhere else. The element-read arm is deliberately absent — before the fix `view[0]` **disagreed** on 14 of 29 cases and after it the same 14 **decline**, silent wrong values becoming honest ones, and what remains is the separate asserted-index family. Found by the JVM lane, who asked of the partial-comparison list a question its count does not: whether the same examples decline the same on C. Nine of ten did; the tenth was this. And it is **not a decline at all** — it is a *disagreement on an input the pool never drives*, which is a second hole the partial instrument cannot see: declines are counted, unreached inputs are not. `DataView` is `ManagedType::DataView`, which carries nothing: its width is chosen per *access* by the method called, so `getUint8` and `getFloat64` are two calls on one type rather than two types. The bigint pair works because a bigint already crosses the boundary as a value — `__int128` on C, `NtsBigInt` on the JVM — so it needed an arm rather than a mechanism |
| memory | | `WeakRef`, `FinalizationRegistry` | |

**The `keyed` row read as a whole-family gap until 2026-09-24 and was two rows.** `Map` and `Set` are one insertion-ordered hash table under two names, storing `NtsValue`s — the same sixteen bytes an erased value already is — so a key and a value are erased at the call and `get` hands the slot back whole, its `V | undefined` and an absent key being the same bytes. `examples/map-and-set` is **725 cases across 25 exports, agreeing on every one** — every one of which constructs empty, `new Map<K, V>()`, because `new Map(entries)` wants the protocol too. What the key *type* buys is the probe loop: a `Map<string, V>` is built with the string hash and compares with the runtime's string equality, never reading a tag, and only a genuinely heterogeneous key type pays for being one. The gap that is left is **iteration**, which is the protocol rather than the container and is refused by name rather than by silence — and it is most of what remains worth having, since the 141 of 177 `Map` call sites in the node profile that are `get`/`set`/`has`/`delete`/`size` already work. `WeakMap` and `WeakSet` are a different question again and stay refused deliberately: `decompose.rs` declines the cheap table because *"representing it as this table would turn a documented weakness into a silent leak"*, and RFC §16 prices the family at the collector across four passes.
...
| ✗ | an **unpaired surrogate in a string literal** does not survive the frontend — `"\uD800"` becomes three U+FFFD code units | **Diagnosed 2026-09-18** from test262 `greater-than/S11.8.2_A4.12_T1.js` and `less-than/S11.8.1_A4.12_T1.js`, which are `"\uDC00" > "\uD800"` and were two of four files in the slice-1 population that ran and answered wrongly. **The comparison is not the defect** — the row above stands. `length`, `charCodeAt` and ordering are correct UTF-16 for every well-formed string, astral pairs included, and the emitted literal is `{ 65533, 65533, 65533 }` with `length` 3: the three WTF-8 bytes of the lone surrogate were each replaced by a lossy UTF-8 decode at the transport boundary, before any of this compiler's string handling runs. So the fix is in how `frontend-ts` carries a literal, not in strings — msgpack strings are UTF-8 and a lone surrogate has no UTF-8 spelling. Silent: the program compiles and runs, with a different string than the source wrote. |
| ✗ | the **wrapper objects** — `new String(…)`, `new Number(…)`, `new Boolean(…)` | **Measured 2026-09-18**: 24 of the 198 non-generated slice-1 test262 files that reach lowering and are refused, the largest such row after `eval` and the two fixed that day. Refused by name — `a \`new\` of unrepresentable type (\`String\`)` — so it is visible rather than silent. These are objects, not primitives: `typeof new String("a")` is `"object"`, `new Boolean(false)` is truthy, and `new String("a") === "a"` is false. Supporting them means a boxed representation per primitive plus `ToPrimitive` unwrapping at every operator and coercion site, which is why the cost is out of proportion to the demand — no code written this decade uses them deliberately, and `String(x)` / `Number(x)` **without** `new` are the ordinary conversions and already work. Recorded so the row is a decision rather than an absence; test262 exercises them heavily because they are specified, which is what puts them this high. |
| ✗ | `using` and `await using` — **refused as of 2026-09-18, and accepted before that** `examples/using-unsupported`. Nothing in this compiler had heard of the construct: `VariableKind::from_flags` knew `Let = 1` and `Const = 2` and not `Using = 1 << 2`, so a plain `using` read as **`var`** and an `await using` — which the frontend spells `Const \| Using`, its own source noting the two "would otherwise be mutually exclusive" — read as **`const`**. The declaration lowered as an ordinary binding and `[Symbol.dispose]` was never called: `function f() { using r = new R(); } f();` left `disposed` at 0 where node says 1, silently and on every backend. Refused rather than implemented because disposal is a scope-exit call in reverse declaration order, on the normal path and on a throw, with `await using` awaiting it — and **zero sites in `runtime/node` or the example corpus** write it, while the 190 test262 files declaring `explicit-resource-management` are better served by an honest `unsupported`. Refused at the **statement** as well as the symbol: a refusal recorded against a symbol is reported where the name is *read*, and a `using` binding exists for its disposal rather than its reader, so `using r = new R();` mentions `r` once and the symbol-keyed refusal was correct and silent. |
...
| | | |
|---|---|---|
| ✅ | a `for...of` or `for...in` head that **assigns** rather than declares — `for (v of xs)`, `for (o.k of xs)`, `for ([a, b] of pairs)` — `examples/a-for-of-that-assigns`, 319 cases across 11 functions on all five variants. **Landed 2026-09-19**; `for_of_head` looked for a `VariableDeclaration` and refused when there was none, which is 36 files of the slice-1 `test/language` population. The language permits any assignment target there, and the write is the one an assignment statement makes: `place_of` already knows every target shape, and a destructuring target goes to `assign_pattern`, which `[a, b] = p` already uses — neither was taught the shape twice. **The part that is not the write**: a head assigns, so what it writes is *loop-carried*. `for (v of xs) { }` writes `v` once per iteration and the body never mentions it, so `assigned_symbols(body)` finds nothing; left out it showed as `NotDominated` — invalid HIR, no output, no diagnostic. `survivesTheLoop` and `emptyLeavesItAlone` pin it and disagree with each other on purpose. A property or element target needs none of that, being storage that outlives the loop. **`for ({ x } of objs)` still refuses**, and identically to `({ x } = o)` outside a loop: a shorthand's symbol is the *property's*, not the variable's. Bracketed as unchanged. 10 of the 36 files pass; the rest stop at that shorthand and at other targets. **A default in the head** — `for ([a = 7] of xss)` — landed 2026-09-19 and has its own row above, since it is refused outside a loop too. |
...
| ✅ | over an **array**'s `entries()`, `keys()` and `values()` — `for (const [i, v] of xs.entries())` — the counted loop reading its own cursor | the same argument the `Map` row above makes, one container over: a counted walk already keeps the index it steps by, so all three spellings are that one loop rather than three walks, and no iterator is stepped and no `[index, element]` pair is built to be taken apart immediately. `Walk::Counted` carries which of the position and the element the head binds, which is why `keys()` needs no second walk — its element *is* the index, so its element type is a number rather than the array's. **The fold and the reading had to agree**: `sequence_source` folds `xs.entries()` back to `xs` before the arity is known, and `head_for_the_sequence` was then rewriting the two names into a destructuring of one element — correct over an array of tuples, which is the identical source, and wrong here. The name the fold carries out is the only thing that still distinguishes them by that point. **With one name it is refused**, because the element then *is* the pair; `tooling/conformance/blockers/an-entries-walk-binding-one-name` asserts that **by message** rather than by "something refused", since the failure mode is a loop that quietly walks elements and binds each to the name. **The fold has three call sites** — `for...of`, `yield*` and `Array.from` — and all three pass one name, so `yield* xs.keys()` and `Array.from(xs.values())` fall out of it. Two exports drive those rather than leaving them asserted, because a fold verified only through the site that motivated it says nothing about the other two. `examples/an-array-walked-with-its-index` is **319 cases across 11 exports on C, LLVM and the JVM, and under reference counting on C and LLVM**; nine refusals on the compiler before it, against three controls that lowered on both — an array of tuples destructured, the `Map` form, and the plain walk. Corpus effect: **none measured**, and not claimed — the idiom is 2 distinct sites in `runtime/node`, both in `util/deep-equal.ts`, and neither module moved. |
...
| ✅ | a **rest** or a **nested pattern** in an array head — `for (const [head, ...tail] of rows)` | the row above binds an *object* pattern off the element and this is the array half, which refused until 2026-09-17 for a reason that was about ordering rather than about destructuring. `[a, b]` has two readings and the sequence decides which, but the head was read **before** the sequence — so an element a *positional* head could not be was refused there, as ``a destructuring element that is more than a name``, and never reached the second reading. `const [head, ...tail] = row` worked in a declaration and the identical pattern in a head did not. **An element that is more than a name is itself the answer**: a walk's positional values cannot carry a rest or a nested pattern — a `Map` hands over a key and a value and there is nothing to gather — so the brackets must be a destructuring of the single element, which `bind_pattern` has always done. Two refusals became a routing decision, and over a `Map` the case now falls to the one-name refusal, which names the pair and says to bind `[key, value]` — a better sentence for that case than the one it replaced. **A default in the head is not claimed**: it compiles by the same route and nothing can make it fire, because `number[]` types every index `number` (so the default is only reachable by an out-of-range read, which node answers `undefined` for and this declines) and `{ a: number; b?: number }[]` is refused as an array literal, which is the optional-property representation rather than this. `examples/a-for-of-head-that-destructures` is **232 cases across 8 exports on C, LLVM and the JVM, and under reference counting on C and LLVM**; three refuse on the compiler before it and five are controls that lowered on both — two plain names over a `Map`, an elision over a `Map`, two names over an array of tuples, and `entries()`. The `Map` arms are the load-bearing ones: a change routing *everything* to `bind_pattern` would pass the three new arms and break those. |
...
| ✅ | **`IteratorResult<T>` from `lib.d.ts`** — `examples/a-library-iterator-result`, 261 cases across 9 functions on C, C+rc, LLVM, LLVM+rc and JVM. **Landed 2026-09-19.** The layout is *provided* rather than read, the way `builtin::error_fields` provides `Error`'s and for the reason that module's header gives — one `bool` slot for `done` and one element slot for `value`, per instantiation. **Both blockers dissolve rather than being worked around**: no arm is ever decomposed, so `shared_field` never meets `done?: false`'s presence bit; and the return arm's `value` gets no slot, so the `any` that `TReturn` defaults to never reaches HIR, which is what `docs/any-unknown.md` requires rather than a way past it. Adding the two arm names to `decompose.rs`'s `is_carried` was tried once and reverted at 2–4 cleared sites per module; it lands here with the layout that makes it mean something. **The slot the source cannot fill.** `{ done: true, value: undefined }` is how every iterator finishes and a `number` slot has no room for `undefined`, so it takes the element representation's zero — honest only because no read reaches it: `for...of` reads `value` on the not-done edge by construction; `r.value` is permitted only where the checker has already narrowed `r` to the yield arm, which is **TypeScript's own rule asked rather than re-derived** (a `flow.rs` relation on the `done` field would have been a second derivation of one fact, disagreeing the first time the two were spelled differently); `const { value } = r` is refused, having no narrowed receiver node to ask; and `r.done = false` is refused, because it moves the guard without moving what the slot holds. **The last two were probe.sh disagreeing with node** — `0` where node says `undefined` — with the first refusal already in the tree and looking like it covered the question. `refuse_unguarded_iterator_value` is the one place that decides it, asked by both readers of the slot. **What this does not close**, measured the same day: `Iterator<T>`, `Iterable<T>` and `IterableIterator<T>` as annotations are still refused, and carrying those three names next stops at *a method `next` with no declaration* — interface dispatch for a library interface, which is the next link and a larger one. So the 255-site family figure below is untouched: the root is represented and the chain above it is not. **The provided layout is per instantiation and its *name* was a constant, which stopped `http` and `process` from building** — caught by `addons.sh` on 2026-09-20, three commits after it landed, and by nothing before that. `IteratorResult<Buffer[]>` and `IteratorResult<unknown>` are two layouts, and a layout's name is what every backend names its type after, so one translation unit held two `struct NtsObj_IteratorResult` definitions: ``error: redefinition of `NtsObj_IteratorResult` ``, from clang, a compilation stage away from the choice that caused it and naming no source line. **The name was also what three call sites compared against to recognise the provided layout**, which is why it was a constant — one string doing two jobs that pull in opposite directions. Recognition asks the *type* now, through `iterator_result_element`, the same predicate the provider itself asks; the name carries the instantiation's id, which is the `Type{id}` convention every anonymous layout already uses. **The doc comment on the provider said "two layouts and two names" while the code beside it wrote one constant** — a comment asserting what the code does is a claim, and this one was false from the day it was written. `verify` now reports `DuplicateLayout` for any two layouts that share a name and disagree, so the next one is a named failure in the compiler rather than a clang error in a build log. Its first message reported the two field *counts*, which are **equal** here — `{ done, value }` either way, differing in what `value` holds — so it now names the field and both types. Sabotaged back to the constant to confirm it fires. |
...
| ◐ | spread over an iterable; `new Map([[k, v]])`, `new Set([...])` | **the constructors take an array as of 2026-09-19** — `examples/a-table-built-from-an-array`, 290 cases across 10 functions on all five variants. The previous wording here said they *"still want an array"*, which reads as a restriction and was not one: they took **nothing**, and an array argument refused exactly as a `Set` argument did. The contents are now a loop over the array calling `nts_set_add` / `nts_map_set` — the same externals `lower_table_method` already emits for `s.add(v)` and `m.set(k, v)`, so **no backend changed** and none can disagree about it; a helper added for one red-gates the other two and there is no allowance list for that. The index read is emitted *checked* and `bounds.rs` eliminates it, verified on the artifact: the prepared HIR reads `array.get unchecked` and the C is a plain `NTS_ITEMS(v, double)[i]`. **A `[K, V]` has two representations** and handling one was a hole — members that differ are an object layout, members that agree are an *array* of two slots, so `new Map<number, number>` refused beside three string-keyed arms that passed. **An arbitrary iterable is still refused, by name**: that needs `Iterable<T>` to be representable, which is library-interface dispatch and the larger piece the whole iteration family sits behind. `new Map<K, V>([])` also still refuses — a Set's empty literal is lowered at `T[]`, and a Map's element is a two-field tuple with no representation at that point. The spread — `[...someSet]` — is untouched and wants the same protocol. |
...
| ✅ | a **structural cast that is not a prefix** — a class passed where an interface is wanted | a compiled reference is a pointer, so this is a pointer cast, sound only where the target's fields are the source's first fields. It is now a **copy of the callee over the concrete type**, reading each field at that class's offset — no cast and no dispatch. The plain version stays, because a structural copy is an addition where a generic instantiation is a replacement. Chosen over laying the interface out like its implementor because a prefix buys the JVM nothing (it relates classes by name) and because on **ART there is no inline cache**: a monomorphic interface call is 1.97x a direct read, not HotSpot's 1.04x, so the case C2 makes free is the one that loses most without it. Every one of the 63 sites in the profile is monomorphic. A field or an array of the interface type still has no concrete type to specialise over. Record 0294 `examples/a-structural-cast-that-is-not-a-prefix` carries it, and `examples/a-structural-cast-that-is-a-prefix` the case that does line up — the pair is what shows the prefix is the deciding property. **The residue is priced, 2026-09-24.** A field or an array of the interface type is what the structural copy cannot reach, and the only repair left open for it is broad indirection — every field access through an interface-typed receiver. `nts receivers` counts them: **4,174 of 17,521 field accesses in `runtime/node`, 185 files — 23.8%, or 26.6% of the 15,681 through a type this program lays out itself; the 1,840 library receivers are two different things and nothing can say which, so the share is bracketed**, against **10 of 694 across the 61 benchmark cases**, with every hot loop at zero. So the corpus cost is real and no current benchmark row would detect it, which means "the benchmarks did not regress" cannot be the evidence that it is free. Of the 4,174, only 158 are through an interface a class names in `implements` and at most 883 through one whose members any class covers, so 79%–96% of the broad rule's cost would be paid on records and options bags no class can inhabit. And a **sound** narrow rule -- every interface a class could inhabit by member *name*, plus any nothing could examine -- costs **883, 5.0%**, a 4.7x reduction, because names over-approximate real assignability and that is the safe direction. Record 0294 rules out the *complete* satisfier set, which is what decides who is safe to **leave alone**; a sound over-approximation decides who **must** be made indirect, and that one is computable. `blockers/method-syntax-in-an-interface` carries the full table. |
...
| ✅ | `findLast`, `findLastIndex` and `reduceRight` — the same loops, reversed | their forward twins all worked, which made this an asymmetry rather than a feature: `find`, `findIndex` and `reduce` are compiled as a loop with the callback's body inlined, and these three are that loop starting at the other end and stepping the other way. Nothing else differs — same callback shape, same delivery, same result. So the direction is a flag the loop reads in **exactly three places** (where the cursor starts, what stops it, how it moves) rather than three more `Iteration` variants that every match would have had to say were the same; `iteration_result` was split out to make that claim checkable, since every arm of it reads what the loop left behind and none can see how it walked. **The seeds were already right and must not follow the direction**: `find` carries the index it stopped at seeded with the *length* and `findIndex` seeded with *-1*, and the reason the first is not -1 is that `at(-1)` is the **last** element in JavaScript — so a `findLast` matching nothing must still answer `undefined` and a `findLastIndex` must still answer -1. `matchesNothing` is the arm that fails if either were made directional. `reduceRight` with no seed is the one place the cursor starts *two* steps in from the edge, because the outermost element becomes the accumulator. The loop stops with `>= 0` rather than `> -1` deliberately: the same test, and the second spells it in the constant that already means "not found" three functions down. `examples/an-array-walked-backwards` is **290 cases across 10 exports on C, LLVM and the JVM, and under reference counting on C and LLVM**; seven refuse on the compiler before it, and three are controls — the forward twins on the same data, `reduce` in both forms, and the methods with no backwards form at all. **Zero sites in `runtime/node`** for all three, which is why a refusal census never ranked them. |
| ✅ | `toReversed()` — and **not** as a helper of its own | the specification says *a new array with the elements in reverse order*, which is `reverse()` on a copy, so it is composed from `slice` and `reverse` rather than given a third helper that would be a second place deciding what copying and reversing mean. That costs **no runtime surface**, so C, LLVM and the JVM answer for it the day it lands instead of two answering and the third red-gating — which is the argument for composing wherever the pieces exist. `examples/arrays` and `examples/array-references` carry it on each element path, and the arm reads the **source** back afterwards: `reverse` returns the same array it reversed, so `reversedHead` beside it cannot tell the two apart, and reading the source is the only thing that fails if this were wired to the in-place helper. `compiler/core/tests/bounds_checks.rs` moves 4 to 8 in the same change and that is the test working — an arm indexing a returned array has to show up there or the count is measuring nothing. **`with`, `toSpliced` and `toSorted` are still refused**: the first throws a `RangeError` out of range and a runtime helper here has no way to throw, the second needs `splice`'s insert form which is itself refused, and the third needs `sort`. |
| ✅ | `codePointAt` answers **`undefined`** for an index that is not there | and it answered NaN, on every out-of-range call, silently — `?? 0` takes the one and not the other, and `String(v)` is `"undefined"` against `"NaN"`. **Its sibling `charCodeAt` answers NaN and is right to**, which is the whole reason this lasted: the helper is written in terms of that one, so the sibling's answer reads as a precedent. They are different questions with different answers, exactly as `at`, `charAt` and `s[i]` are three answers for an index that is not there. NaN is not a code point, so the absence is distinguishable without a second helper — the lowering builds the two-armed value itself and **no runtime surface is added**, so C, LLVM and the JVM all have it the day it lands. Only where the result is erased, so a caller who narrowed it away pays nothing, which is the rule `pop` and `at` already follow. **`examples/strings` had carried `s.codePointAt(i)!` since it was written and could not have caught this**: the `!` asked the question and threw the answer away, and the arm passed because the harness renders node's `undefined` and the helper's NaN identically. It now brings the index into range so the `!` is true, and `pointAtOrAbsent` beside it asks without narrowing. |
| ✗ | `x!` where `x` **is** `undefined`, at a number | node answers `undefined` and this answers **`0`**. `const v: number \| undefined = cond ? 1 : undefined; return v!` diverges on every case, and has since long before 2026-09-17 — it was found by `codePointAt` starting to answer `undefined` properly and routing `examples/strings` through it. **The compiler disagrees with itself**, which is the reason to fix rather than to document: `xs.at(9)!` and `xs.pop()!` on an empty array agree with node, because the absent value comes back from a runtime helper as a real `NtsValue` and unerases at run time; a *folded* `undefined` in the middle end is rewritten to `const 0 : f64` when the merge it feeds is only ever read as a double. One fact, two derivations, and the constant fold is the one that disagrees with the runtime it is folding. The program has lied either way — `!` promised the value was there — so no answer is correct, and the argument for NaN is not that it is right but that it is what the other path already produces and what node's rendering agrees with. Not fixed here because the fold is in the middle end and the blast radius is every erased merge, not because it is small. |
| ◐ | `sort()` and `toSorted()` with **no comparator**, on an array of **strings** | the default order is the specification's and not a natural one: every element is converted to a string and the strings are compared by UTF-16 code unit. An array that already holds strings needs no conversion, which is why that half landed and the numeric half did not — and it is the opposite of the split `blockers/sort-array-of-references` was filed under, which read it as a *reference* problem. **All three sites in `runtime/node` are this form**: `keys.sort()` and `output.sort()` in `util/inspect` (which is `util.inspect(value, { sorted: true })`, a public option) and `strings.toSorted()` in `readline`. No comparator means no callback and no bridging, so it is a plain runtime helper. **Stable, by a merge sort rather than `qsort`** — the specification has required stability since ES2019 and `qsort` leaves the order among equal elements to the libc, so the same program would answer differently on two machines and the differential would catch it on exactly one. The JVM gets it from `Arrays.sort(Object[])`, which is TimSort; `String.compareTo` is UTF-16 code-unit order and agrees, and the one pair that could separate code-*unit* from code-*point* order — a surrogate pair against `\uE000` — is in the fixture, with both putting the pair first. The Java half was written with a lambda first and `runtime_jar.rs` rejected the jar: a lambda is `invokedynamic` through `LambdaMetafactory`, and at the Android floor the bootstrap runs at first execution. Natural ordering needs no comparator at all. `toSorted` is `sort` on a copy, composed from `slice` for the same reason `toReversed` is. **The JVM needed two table rows, not one, and said so**: a *growable* array is a class with a backing store and a length rather than a Java array, so its helpers are looked up by stem in a second table — `xs.sort()` worked and `xs.push(v); xs.sort()` declined, and so did a program that pushed a *different* reference array anywhere, because growability is decided for reference arrays together. `NtsArrayL.sortStr` sorts `items` between 0 and `length`, since sorting the whole backing store would order the unused tail in with the live elements. Only the JVM names its methods, so only the JVM could report a missing row; C and LLVM pass a pointer and ask nothing. **Refused, and each held by a blocker that asserts the message**: the numeric form (`[3, 1, 10].sort()` is `[1, 10, 3]`, so answering `[1, 3, 10]` would be a wrong answer where this is a missing one) and the comparator form, whose blocker carries the three routes to it and what each costs. `examples/an-array-sorted-by-the-default-order` is **203 cases across 7 exports on C, LLVM and the JVM, and under reference counting on C and LLVM**, all seven refusing on the compiler before it; every string in it is written with `\u` escapes, because the peer lane's probe of this same question diverged once on literals that did not survive identically into its two arms. |
| ✗ | **JVM only**: `indexOf`/`lastIndexOf` whose result is used as an integer, on an array the program **grows** | a run-time `NoSuchMethodError`, not a refusal. `const xs = [3, 1]; xs.push(n); (xs.at(0) ?? 0) * 10 + xs.indexOf(1)` dies with `'int nts.rt.NtsArrayD.arrayIndexOfI(NtsArrayD, double)'`; C and LLVM agree with node on the same program. **Two tables, and the call is assembled from both**: `ops.rs` takes the *owner* from `found` — which under `shape.grows` is `growable_external`, answering `nts/rt/NtsArrayD`, whose method is `indexOf` — and the *method name* from `intcall::integral_helper`, which answers `arrayIndexOfI`, a name that exists only on `NtsRuntime`. It needs the narrowing *and* a grown array, which is why `xs.indexOf(1)` alone is fine. Pre-existing: identical on `c19d8b05`. **Worse than a missing row**, which is a loud `NTS4001` at build time — here both tables answer and disagree, so it links and dies later. Two repairs and the choice is a measurement: decline the narrowing under `grows` (correct, loses the 22.8% `intcall` was built for, on grown arrays only, and any program it then refuses red-gates the `exact` JVM floor), or add `indexOfI`/`lastIndexOfI` to the growable classes and a row naming them (keeps it, four Java methods, and makes the two tables agree rather than one defer). A guard at the call site alone is **not** enough: the value stays in `narrowed`, so the emitter reports `emitting %26 moved the operand stack from 0 to 1` — `intcall::narrowed` has to know about `grows`, which means threading the shape into `held_values`. |
| ✅ | **JVM**: a runtime helper that returns a **bare Java array**, in a program that grows one — `"a-b-c".split("-")` beside any `push` | a growable program holds every array as an `NtsArrayL` wrapper, and `nts_str_split` answered a `[Ljava/lang/String;` straight into a slot typed as the wrapper: `ClassCastException: class [Ljava.lang.String; cannot be cast to class nts.rt.NtsArrayL`, at **run time**, on a program C and LLVM both agree with node about — both pass a pointer and ask nothing, so only a backend that names the type it stores into can notice. **`split` is the only helper this reaches, measured one at a time rather than reasoned about**: `slice`, `concat`, `splice`, `reverse`, `map`, `filter`, `Array.from`, `Object.keys`, `Object.values`, `toReversed` and `toSorted` all already answered a wrapper. The wrap is the one the **bound member** path already makes, with the same overload rule — a `[Ljava/lang/String;` calls `adopt([Ljava/lang/Object;)`, and array covariance is what makes the argument legal; `adopt` already existed, and `javac` said so three seconds after I wrote a second one. **It needed two unrelated pieces of a program to meet**: growability is decided for the whole program, so a `push` anywhere is what puts every array into a wrapper — which is why `examples/a-split-in-a-program-that-grows-an-array` carries a `grown` array that never touches the split. 174 cases across 6 exports on C, LLVM and the JVM and under reference counting; it throws on the compiler before it. |
...
`const` type parameters, `String.raw`, `replaceAll`, shorthand
properties, `implements`, local *function* declarations including recursive
ones, IIFEs, and the comma operator.

**`using` was on that list and came off it on 2026-09-18.** It compiled, so a
probe that asked whether the program builds said yes — and `[Symbol.dispose]`
was never called. The arm that would have failed is the one nobody wrote: read
the effect back. See the ✗ row below.
...
#### `toString` on an array (fixed)

`[1, 2].toString()` is `"1,2"`, and the specification says how: `Array.prototype
.toString` calls the array's `join` when it is callable, which for an ordinary
array it is. The separator is the comma `join` already defaults to, so the
lowering is a **rename before dispatch** rather than a second implementation —
one that could disagree with `join` about a hole, an empty array, or how a
number is formatted. Only with no arguments: `toString` takes none, and a call
that passes one is something else and keeps its own refusal.

It refused as ``toString`, where an array has only `length``, which is a true
sentence about the layout — an array has no methods in it — and a misleading one
about the program.

**It moves no corpus row, and that was established rather than assumed.** The 7
files carrying this message, which surfaced behind `instanceof Array`, do not
call it:
...
was correct on C, LLVM and the JVM and **answered freed memory under `rc`**.
`examples/an-element-of-a-mapped-array` carries it; the pre-fix binary
disagrees with node on **41 of its cases**.

**The cause.** `map` allocates its result with
`nts_array_new_uninitialized`, and `rc::load_slot` was reading each slot
*before* writing it so that it could release whatever the slot held. Nothing
held anything — the slots had never been written — so `nts_release` was handed
whatever the allocator last left there and decremented a header belonging to
something else.

The runtime states the invariant in its own words: that allocator exists
because "every slot is written", and `NTS_POISON` fills the storage with `0xA5`
precisely so a slot nobody wrote stops reading as zero. This pass made that
false in the one build where the slots are pointers. **It looked benign for a
long time**, and the same comment says why — a *fresh* page is zero and
`nts_release(NULL)` returns, which is exactly the measurement recorded there. A
reused allocation is not zero.
...
filled somewhere the pass cannot see, the release comes back and is as correct
...
looked like different defects. `ys.join(",")` and `ys.length` pass because they
read the storage inside the runtime rather than handing an element back, which
made *"how the result is read"* look causal when it was a symptom.

**The record of getting it wrong is kept deliberately.** The fixture was first
named `a-mapped-tuple-in-a-concatenation`, after a boundary saying the defect
needed two or more elements, a number conversion and a **tuple** element. Each
was measured; all three were wrong, because every comparison changed more than
one thing at a time. What found it was not re-reading the reasoning but
continuing to shrink the reproduction after the boundary already looked
established — *"I can state the boundary"* is not the stopping condition,
*"I cannot remove anything else"* is.

**Pre-existing.** The binary from before the enumeration-order work fails it
identically; that example was simply the first in the corpus to write such a
shape.
...
#### The iteration family is 255 sites; the root is closed and the next link is dispatch
...
**The root was closed on 2026-09-19, and neither blocker was sneaked past.**
The layout for `IteratorResult<T>` is *provided* rather than read, per
instantiation — `{ done: bool, value: T }` — which is what `builtin.rs` exists
for and what its header already described. Providing it means no arm is ever
decomposed, so `shared_field` never meets the optional modifier; and the return
arm's `value` gets **no slot**, so `TReturn`'s `any` is not represented rather
than being represented as something. The table row above carries the whole
account, including the four rules that keep the finished arm's zero unreadable
and the two of them that were **probe.sh disagreeing with node** rather than
reasoning.

**The 255 is not 255 fewer.** Measured the same day, immediately after:

```text
  next(): IteratorResult<number>            lowers
  [Symbol.iterator](): Iterator<number>     refused: a function returning `Iterator`
  xs: Iterable<number>                      refused: a parameter of unrepresentable type
  it: Iterator<number>                      refused: a parameter of unrepresentable type
```

Adding `Iterator`, `Iterable` and `IterableIterator` to `is_carried` was then
tried, exactly as the two arms were, and stops one link further on:

```text
  a method `next` with no declaration in ...
  a method `__@iterator@22` with no declaration in ...
```

Which is **interface dispatch for a library interface** — a parameter typed
`Iterator<number>` means a virtual call, and these interfaces declare methods
that no class in the program declares. That is the next link, it is larger than
this one, and the three names were reverted again rather than committed: a
name-list entry that clears nothing is still a claim the list does not support.

So the family's shape is now one root represented, one chain above it, and the
number to watch is *sites that clear*, which this did not move. What it moved is
the shape a hand-written iterator can be written in: `for...of` over a class
whose `next()` is annotated the way `lib.d.ts` annotates it.
