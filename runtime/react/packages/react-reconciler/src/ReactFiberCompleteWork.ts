// The complete phase: after a fiber's children are done, create or update
...
import { getHostContext, getRootHostContainer, popHostContainer, popHostContext } from "./ReactFiberHostContext.ts";
import {
...
import type { OffscreenQueue, OffscreenState } from "./ReactFiberOffscreenComponent.ts";
import type { RootState } from "./ReactFiberRoot.ts";
import { createScopeInstance } from "./ReactFiberScope.ts";
import type { RetryQueue, SuspenseListRenderState, SuspenseState } from "./ReactFiberSuspenseComponent.ts";
...
import { suspendCommit } from "./ReactFiberThenable.ts";
...
import { ConcurrentMode, NoMode, ProfileMode, SuspenseyImagesMode } from "./ReactTypeOfMode.ts";
import {
  ActivityComponent,
  CacheComponent,
  ClassComponent,
...
function offscreenCacheOf(fiber: Fiber | null): Cache | null {
  ...
  const state = fiber.memoizedState as OffscreenState | null;
  if (state !== null && state.cachePool !== null) {
    return (state.cachePool as SpawnedCachePool).pool;
  }
  return null;
}
...
function markCloned(workInProgress: Fiber): void {
...
function doesRequireClone(current: Fiber | null, completedWork: Fiber): boolean {
...
function appendAllChildren(parent: Instance, workInProgress: Fiber, needsVisibilityToggle: boolean, isHidden: boolean): void {
  if (supportsMutation) {
    ...
    while (node !== null) {
      if (node.tag === HostComponent) {
        ...
        appendAllChildren(parent, node, /* needsVisibilityToggle */ true, /* isHidden */ true);
      } else if (node.child !== null) {
        node.child.return = node;
        node = node.child;
        continue;
  ...
  if (supportsPersistence) {
    ...
    while (node !== null) {
      if (node.tag === HostComponent) {
        ...
        appendAllChildrenToContainer(containerChildSet, node, /* needsVisibilityToggle */ true, /* isHidden */ true);
...
function updateHostComponent(current: Fiber, workInProgress: Fiber, type: Type, newProps: Props, _renderLanes: Lanes): void {
  if (supportsMutation) {
    ...
    if (requiresClone && (!passChildrenWhenCloningPersistedNodes || hasOffscreenComponentChild)) {
...
function preloadInstanceAndSuspendIfNeeded(
  ...
  const maySuspend =
    (enableSuspenseyImages || (workInProgress.mode & SuspenseyImagesMode) !== NoMode) &&
    (oldProps === null ? maySuspendCommit(type, newProps) : maySuspendCommitOnUpdate(type, oldProps, newProps));
...
function scheduleRetryEffect(workInProgress: Fiber, retryQueue: RetryQueue | null): void {
...
function updateHostText(current: Fiber, workInProgress: Fiber, oldText: string, newText: string): void {
  if (supportsMutation) {
...
function cutOffTailIfNeeded(renderState: SuspenseListRenderState, hasRenderedATailFallback: boolean): void {
  if (getIsHydrating()) {
...
// Don't count time spent in a timed out Suspense subtree as part of the base
// duration.
function subtractTimedOutPrimaryDuration(workInProgress: Fiber, isTimedOut: boolean): void {
...
function completeDehydratedActivityBoundary(
  current: Fiber | null,
  workInProgress: Fiber,
  nextStateIn: ActivityState | null,
): boolean {
  let nextState = nextStateIn;
  ...
  if (nextState !== null) {
    ...
    if (current === null) {
      ...
      subtractTimedOutPrimaryDuration(workInProgress, nextState !== null);
      return false;
    }
    emitPendingHydrationWarnings();
  ...
  if (current !== null && current.memoizedState !== null) {
    const prevState = current.memoizedState as ActivityState;
...
function completeDehydratedSuspenseBoundary(
  current: Fiber | null,
  workInProgress: Fiber,
  nextStateIn: SuspenseState | null,
): boolean {
  let nextState = nextStateIn;
  const wasHydrated = popHydrationState(workInProgress);
...
// Host Components act as their own View Transitions which doesn't run
// enter/exit animations. We clear any ViewTransitionStatic flag bubbled from
// inner View Transitions.
function clearBubbledViewTransitions(workInProgress: Fiber): void {
...
function completeHostHoistable(current: Fiber | null, workInProgress: Fiber, newProps: Props, renderLanes: Lanes): null {
  ...
  const nextResource = workInProgress.memoizedState as Resource | null;
  if (current === null) {
...
function completeHostSingleton(current: Fiber | null, workInProgress: Fiber, newProps: Props, renderLanes: Lanes): null {
  ...
  if (current !== null && workInProgress.stateNode != null) {
    ...
    if (!newProps) {
      ...
      bubbleProperties(workInProgress);
      clearBubbledViewTransitions(workInProgress);
      return null;
    }
    ...
    if (wasHydrated) {
      ...
      const instance = resolveSingletonInstance(type, newProps, rootContainerInstance, currentHostContext, true);
...
function completeHostComponent(current: Fiber | null, workInProgress: Fiber, newProps: Props, renderLanes: Lanes): null {
  ...
  preloadInstanceAndSuspendIfNeeded(
    ...
    current === null ? null : (current.memoizedProps as Props),
...
function completeSuspenseList(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  ...
  if (renderedTail === null) {
    ...
    if (!didSuspendAlready) {
      ...
      const cannotBeSuspended =
        renderHasNotSuspendedYet() && (current === null || (current.flags & DidCapture) === NoFlags);
      if (!cannotBeSuspended) {
        ...
        while (row !== null) {
          ...
          if (suspended !== null) {
            ...
            const retryQueue = suspended.updateQueue as RetryQueue | null;
            ...
            pushSuspenseListContext(
              workInProgress,
              setShallowSuspenseListContext(suspenseStackCursor.current, ForceSuspenseFallback),
            );
            if (getIsHydrating()) {
...
function completeWork(current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes): Fiber | null {
  ...
  switch (workInProgress.tag) {
    ...
    case HostRoot: {
      ...
      if (current !== null) {
        previousCache = cacheOf(current);
      }
      const cache = cacheOf(workInProgress);
      ...
      if (current === null || current.child === null) {
        ...
        if (wasHydrated) {
          ...
          if (current !== null) {
            const prevState = current.memoizedState as RootState;
            if (
    ...
    case HostHoistable: {
      if (supportsResources) {
        return completeHostHoistable(current, workInProgress, newProps as Props, renderLanes);
      }
      // Fall through to the singleton and component paths, in order.
      if (supportsSingletons) {
        return completeHostSingleton(current, workInProgress, newProps as Props, renderLanes);
      }
      return completeHostComponent(current, workInProgress, newProps as Props, renderLanes);
    }
    case HostSingleton: {
      if (supportsSingletons) {
        return completeHostSingleton(current, workInProgress, newProps as Props, renderLanes);
    ...
    case ActivityComponent: {
      const nextState = workInProgress.memoizedState as ActivityState | null;

      if (current === null || current.memoizedState !== null) {
        const fallthroughToNormalOffscreenPath = completeDehydratedActivityBoundary(current, workInProgress, nextState);
    ...
    case SuspenseComponent: {
      const nextState = workInProgress.memoizedState as SuspenseState | null;
      ...
      if (
        current === null ||
        (current.memoizedState !== null && (current.memoizedState as SuspenseState).dehydrated !== null)
      ) {
      ...
      const prevDidTimeout = current !== null && (current.memoizedState as SuspenseState | null) !== null;

      if (nextDidTimeout) {
        const offscreenFiber = workInProgress.child!;
        const previousCache = offscreenCacheOf(offscreenFiber.alternate);
        const cache = offscreenCacheOf(offscreenFiber);
        if (cache !== previousCache) {
          // Run passive effects to retain/release the cache.
      ...
      bubbleProperties(workInProgress);
      subtractTimedOutPrimaryDuration(workInProgress, nextDidTimeout);
      return null;
    }
    case HostPortal:
      popHostContainer(workInProgress);
    ...
    case SuspenseListComponent:
      return completeSuspenseList(current, workInProgress, renderLanes);
    case ScopeComponent: {
      if (enableScopeAPI) {
        if (current === null) {
    ...
    case LegacyHiddenComponent: {
      ...
      const nextState = workInProgress.memoizedState as OffscreenState | null;
      ...
      if (enableLegacyHidden && workInProgress.tag === LegacyHiddenComponent) {
        ...
        if (current !== null) {
          const prevState = current.memoizedState as OffscreenState | null;
      ...
      const offscreenQueue = workInProgress.updateQueue as OffscreenQueue | null;
      if (offscreenQueue !== null) {
        const retryQueue = offscreenQueue.retryQueue;
        scheduleRetryEffect(workInProgress, retryQueue);
      }

      const previousCache = offscreenCacheOf(current);
      const cache = offscreenCacheOf(workInProgress);
      if (cache !== previousCache) {
        // Run passive effects to retain/release the cache.
        workInProgress.flags |= Passive;
      }
