 ...
 *
 * The obvious refactor -- `appendRow(out, row)` returning the accumulator -- is **50x slower**,
 * and that was measured rather than feared. `out = f(out, x)` has two live references while `f`
 * runs, the caller's local and the parameter, so `nts_str_append`'s `reserved == 1` test fails
 * and every append copies the whole buffer. On this document that is about 534MB of memcpy for
 * one serialization: 62.81ms against 1.25ms, and the arithmetic matches at roughly 10GB/s.
 *
 * So the accumulator never crosses a parameter. Everything that appends to it is written here,
 * which is not how a person would factor this and is exactly how a compiler would emit it -- a
 * generated serializer has no reason to introduce a call boundary the source never asked for.
 *
 * `quoteJSONString` and `numberText` still return strings, so their results are still allocated
 * and copied in. Removing *those* needs an append-in-place form of both, which is a change to
 * shared code and a separate question.
 */
function serializeDocument(doc: Document): string {
  ...
  for (let at = 0; at < rows.length; at++) {
    ...
    for (let tag = 0; tag < tags.length; tag++) {
      if (tag !== 0) out += ",";
      out += quoteJSONString(tags[tag] as string);
    }
    out += '],"meta":{"level":';
    const meta = row.meta;
