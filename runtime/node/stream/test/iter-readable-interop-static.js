...
// Every supported case from pinned upstream
// `parallel/test-stream-iter-readable-interop.js`. The omitted assertion
// directly inspects the class prototype; the byte consumer comparison below
// checks values without distinguishing Buffer from Uint8Array by prototype.
...
async function testByteModeBytes() {
  ...
  assert.strictEqual(result.byteLength, data.byteLength);
...
async function testBatchingBehavior() {
  ...
  assert.ok(batches.length < 10, `Expected fewer batches than chunks, got ${batches.length}`);
...
async function testErrorPropagation() {
  ...
  await assert.rejects(text(from(readable)), (err) => err.message === "test error");
}
...
async function testObjectModeError() {
  ...
  await assert.rejects(text(from(readable)), (err) => err.message === "object error");
}
