...
| refused a construct | 43 |
| rejected by the typechecker | 86 |
| **the frontend fell over** | **1** |
| **invalid HIR or a panic** | **0** |

Of the 97 that typecheck, **55%** lower completely. The typechecker rejects the rest by design — a compiler's test suite is largely programs that are supposed to fail.
