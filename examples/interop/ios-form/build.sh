...
"$nts" bind-objc --sdk "$sdk" --target x86_64-apple-ios17.0-simulator --module objc:UIKit \
  ...
  --class UIView --class UITextField --class UIScreen --class NSNotificationCenter --class NSNotification \
  --class NSOperationQueue --class NSTimer --class NSString --function UIApplicationMain \
  --protocol UIApplicationDelegate --protocol UITextFieldDelegate --protocol UIKeyInput --out "$out/uikit.d.ts" --values "$out/uikit.values.ts" >/dev/null
for generated in uikit.d.ts uikit.values.ts; do
  ...
  diff -u "$source/types/$generated" "$out/$generated" >"$out/$generated.diff" || {
    ...
    echo "ios-form: types/$generated is not what nts bind-objc writes; NTS_REGENERATE=1 rewrites it" >&2
    exit 1
  }
done
