...
out: *"a call inside a `try` whose `throw` would not reach this handler"* —
followed by which of four things the callee is, and, where the callee is a plain
function that calls one of them, the leaf it reaches.

**Quoted to the colon on purpose.** The tail is a classification that has already
been extended once and will be again, and a document that quotes a whole
diagnostic is right the day it is written and wrong the next time anyone improves
the message. Four readers of the previous wording had to be found by hand, two of
them prose. A
...
| --- | --- |
| `deep(n)`, a plain TypeScript call | `NTS1001 a call inside a `try` whose `throw` would not reach this handler`, naming the leaf it reaches |
...
| `number` | `int`, `long`, `float`, `short`, `byte`, `char` | a **call**, not an instruction, and deliberately **not** `d2i`. Corrected 2026-09-15 by reading the bytecode: it is `invokestatic NtsRuntime.toInt32:(D)I` and its siblings. `d2i` saturates -- `2**31` becomes `2147483647` -- where JavaScript wraps it to `-2147483648`, and node is the oracle. The two agree on every value anyone tries by hand and disagree on exactly the ones a boundary is tested with, so the instruction this row used to name is the one that would be wrong. A small `static` that HotSpot inlines, so the *runtime* cost is near an instruction; the emitted code is a call. `float` is the exception and really is one instruction (`d2f`), because there the JVM's own conversion is the specified one |
| `boolean` | `boolean` | nothing |
| `bigint` | `long` | a field read; `NtsBigInt.lo` is the value |
| `bigint` | `BigInteger` | **the binding does not compile**, and no call is needed to see it. Corrected 2026-09-15: a single method taking `BigInteger` makes the whole generated `.d.ts` fail with `TS2694 Namespace 'java' has no exported member 'math'`, because a curated prelude carries `java.lang` and `java.util` and not `java.math`. So the cost is not a copy -- the same shape as the `Map` row two lines down, where a price was quoted for something unreachable. What the copy would cost, once `java.math` is bound, is still an allocation per crossing: `NtsBigInt` is two `long` fields and `BigInteger` is arbitrary precision |
| `T[]` (non-growing program) | `T[]` of the same element | **nothing.** It is already that array. |
| `T[]` (non-growing) | an array of a *different* width -- `number[]` to `int[]` | **unreachable**, corrected 2026-09-15. `typed_array` maps every primitive array element to a typed array -- `[I` is `Int32Array`, `[D` is `Float64Array` -- and only a *reference* element or `boolean` falls through to `T[]`. Both of those are already the width they need, so a `T[]` never meets a Java array of a different one. The row priced a crossing the binder cannot produce; the real cost of `number[]` to `int[]` is that it does not typecheck, and the caller writes `Int32Array` |
...
declare module "java:com.example" {
  /** com.example.Session Final: cannot be extended. */
  export class Session {
    constructor(a0: string, a1: number);
    static open(a0: string): Session | null;
    getName(): string | null;
    getRetries(): number;
    setRetries(a0: number): void;
    tags(): string[] | null;
    headers(): java.util.HashMap<string, string> | null;
    cookies(): java.util.List<string> | null;
    /** Throws java.io.IOException. Propagates as the Java exception itself, terminating the program; not catchable by a TypeScript `try` yet. */
    read(a0: number): Uint8Array | null;
    id(): bigint;
    score(): number;
    isOpen(): boolean;
    close(): void;
...
**Regenerated 2026-09-15 by running it**, and what it replaced was aspirational
in seven ways at once: a `declare namespace com.example` where the generator
emits `declare module "java:com.example"`; `import type { int } from "nts:jvm"`
and `: int` throughout, where brands were removed and `find$int` replaced them;
`getName(): string` where an unannotated reference return is `string | null`;
`implements java.lang.AutoCloseable`, which is not emitted; and parameter names
the class file does not carry, so they are `a0` and `a1`.

None of that was invented -- it is what the design intended when the block was
written, and the generator went another way on each point for a reason recorded
elsewhere in this document. The block simply was never re-run. The paragraph
above it says a command nobody has run is a claim; **so is its output.**
...
| `List<String>` / `string[]` | ● | ● | |
| `int[]` / `byte[]` / subarray | ● | | ● |
| Generics, wildcards, raw types | ● | ● | |  <!-- ts-from-java corrected again 2026-09-15: it published a raw `NtsMap` and now publishes `NtsMap<String, String>` through a `Signature` attribute, which is the only way generics reach a Java caller at all. `NtsMap` and `NtsSet` carry type parameters so the signature erases to the descriptor; `Ljava/util/Map<...>;` over `Lnts/rt/NtsMap;` would be malformed. `Main.java` casts nothing. -->
