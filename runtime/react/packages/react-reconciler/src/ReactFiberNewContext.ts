...
// Development only: which renderer is rendering each context, to detect two
// renderers rendering the same provider concurrently.
const rendererCursorDEV: StackCursor<unknown> = createCursor<unknown>(null);
const renderer2CursorDEV: StackCursor<unknown> = createCursor<unknown>(null);
...
export function scheduleContextWorkOnParentPath(parent: Fiber | null, renderLanes: Lanes, propagationRoot: Fiber): void {
...
function propagateContextChanges(
  ...
  while (fiber !== null) {
    ...
    if (list !== null) {
      ...
      (fiber.memoizedState as SuspenseState).dehydrated === null
    ) {
    ...
    if (nextFiber !== null) {
      ...
      while (nextFiber !== null) {
        ...
        const sibling: Fiber | null = nextFiber.sibling;
        if (sibling !== null) {
...
export function lazilyPropagateParentContextChanges(current: Fiber, workInProgress: Fiber, renderLanes: Lanes): boolean {
  const forcePropagateEntireTree = false;
  return propagateParentContextChanges(current, workInProgress, renderLanes, forcePropagateEntireTree);
}
...
// have changed.
export function propagateParentContextChangesToDeferredTree(current: Fiber, workInProgress: Fiber, renderLanes: Lanes): void {
  const forcePropagateEntireTree = true;
...
function propagateParentContextChanges(
  ...
  while (parent !== null) {
    ...
    if (parent.tag === ContextProvider) {
      ...
      const oldStateHook = currentParent.memoizedState as Hook;
      const oldState = oldStateHook.memoizedState as TransitionStatus;

      const newStateHook = parent.memoizedState as Hook;
      const newState = newStateHook.memoizedState as TransitionStatus;
...
function recordContextDependency(consumer: Fiber | null, context: ReactContextBase, value: unknown): void {
  ...
  if (lastContextDependency === null) {
    ...
    consumer.dependencies = isDevelopment
      ? {
          lanes: NoLanes,
