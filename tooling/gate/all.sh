...
llvm_rc() { ( NTS_BACKEND=llvm NTS_RC=1; export NTS_BACKEND NTS_RC
  ...
  backend_examples 325 "through the LLVM backend, counting" "" 10 ); }
...
llvm() { ( NTS_BACKEND=llvm; export NTS_BACKEND
  ...
  backend_examples 326 "through the LLVM backend" "" 10 ); }
...
jvm() { ( NTS_BACKEND=jvm; export NTS_BACKEND
  ...
  #
  # 322 -> 323, the same example.
  backend_examples 326 "through the JVM backend" exact 10 ); }
corpus() {
...
interop() {
  ...
  # 26: `macos-blocks` (A2's blocks), TypeScript closures as Objective-C
