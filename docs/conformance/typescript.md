...
| ✗ | `in` does not consult the **prototype chain** — `"valueOf" in {}` answers `false`, node answers `true` | **Measured 2026-09-18** by running test262 `language/expressions/in/S8.12.6_A2_T1.js`, one of four files in the 2,527-file slice-1 population that compiled, ran, and gave a wrong answer. Own keys are right in both directions (`"a" in {a:1}` and `"b" in {a:1}`); what is missing is that every object inherits `valueOf`, `toString`, `hasOwnProperty` and the rest from `Object.prototype`, and `in` is defined over the whole chain. It emits no diagnostic, so nothing that ranks refusals can see it. Note that answering `true` would be correct and still not make the member *callable* — `Object.prototype.hasOwnProperty` is itself refused as `a global member with no definition here` — so the two are separate pieces of work and the `in` half is the one the specification is unambiguous about. |
