...
6. **A program's binding is derived from its imports (decided and built
   2026-09-26).** Swift's programmer writes `import AppKit`. Here the program
   writes `import { NSWindow } from "objc:AppKit"`, and that is all: `nts
   build` generates the binding of the names it imports. The choice was made
   from a measurement: all of AppKit typechecks, but costs every check 4.17 s
   against 0.97 s for macos-window's thirteen classes, and a class list kept
   in the config names every class twice.
   - `objc_imports` reads the import clauses of the project's files, as
     written, before the checker runs: the binding is what lets the checker
     resolve them. `bind-objc --name` binds each by the name Swift gives it,
     found in Swift's graphs: a class, a protocol, a function, or a Core
     Foundation class, with the Core Foundation classes their signatures
     name.
   - The files go to the project's `.nts/objc/<key>`, keyed by the SDK, the
     deployment target, the names and the compiler. A changed import is a
     cache miss; nothing is regenerated by hand, and nothing is committed.
   - The frontend opens a config that `extends` the project's and adds them
     (`tsgo::generated`). Everything else keyed on the project's config is
     unchanged.
   - What an import does not name: a class reached through a signature
     (`navigation.navigationBar.topItem`) is bound as a stub. The checker
   ...
   - Migrated: the four iOS fixtures, macos-notes and macos-draw. Each
     ...
     macOS fixtures declare their modules by hand, which the generator
     leaves alone.
