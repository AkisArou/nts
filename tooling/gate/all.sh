...
llvm_rc() { ( NTS_BACKEND=llvm NTS_RC=1; export NTS_BACKEND NTS_RC
  ...
  #
  # 266 -> 267 on 2026-09-19 for `a-library-iterator-result`.
  backend_examples 267 "through the LLVM backend, counting" "" 14 ); }
...
#
# 267 -> 268 on 2026-09-19 for `a-library-iterator-result`, which the JVM's
# `exact` floor below took in the same run and at the same number.
llvm() { ( NTS_BACKEND=llvm; export NTS_BACKEND
  backend_examples 268 "through the LLVM backend" "" 14 ); }
...
jvm() { ( NTS_BACKEND=jvm; export NTS_BACKEND
  ...
  backend_examples 268 "through the JVM backend" exact 10 ); }
corpus() {
